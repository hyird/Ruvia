...
Task<void> Http2WebSocketConnection<Session>::write(WebSocketOpcode opcode, std::string_view payload) {
...
template <typename Session>
Task<void> Http2WebSocketConnection<Session>::close(std::uint16_t code, std::string_view reason) {
    if (closeSent_) {
        co_return;
    }
...
template <typename Session>
Task<void> Http2WebSocketConnection<Session>::detachAndDrainBackgroundWrites() {
...
template <typename Session>
Task<void> Http2WebSocketConnection<Session>::waitForHeartbeatWrite() {
...
template <typename Session>
Task<void> Http2WebSocketConnection<Session>::writeExclusive(
...
template <typename Session>
Task<void> Http2WebSocketConnection<Session>::writeFrameNow(
    WebSocketOpcode opcode,
    std::string_view payload,
    bool endStream) {
    ...
    co_await session_.writeData(stream_, std::string_view(header.data(), headerSize), payload, endStream);
    scannerEntry_.touch();
}
