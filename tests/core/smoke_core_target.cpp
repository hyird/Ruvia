...
template <typename T>
concept ExposesRvalueMemoryBorrow = requires(T&& memory) {
    std::move(memory).resource();
} || requires(T&& memory) { std::move(memory).template allocator<>(); };

template <typename T>
concept ExposesRvalueNativePathBorrow =
    requires(T&& path) { ruvia::detail::nativePathView(std::move(path)); };
...
static_assert(
    std::string_view(RUVIA_VERSION_STRING) == std::string_view(RUVIA_EXPECTED_VERSION_STRING));
...
int main() {
    ruvia::MoveOnlyFunction<int(int)> add(
        [offset = std::make_unique<int>(4)](int value) { return value + *offset; });
