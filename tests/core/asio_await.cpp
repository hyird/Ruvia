...
ruvia::Task<void> exerciseSynchronousCompletion(bool& success) {
    ...
    success = result.errorCode() == std::make_error_code(std::errc::connection_reset) &&
              result.result() == 42;
}
...
ruvia::Task<void> exerciseDeferredCompletion(asio::io_context& ioContext, bool& success) {
    const auto result =
        co_await ruvia::detail::asyncAsio<std::size_t>([&ioContext](auto handler) mutable {
            asio::post(ioContext, [handler = std::move(handler)]() mutable {
                handler(std::make_error_code(std::errc::timed_out), std::size_t{7});
            });
        });
    success =
        result.errorCode() == std::make_error_code(std::errc::timed_out) && result.result() == 7;
}
...
ruvia::Task<void> exerciseInitiateFailure(bool& caught) {
    try {
        (void)(co_await ruvia::detail::asyncAsio<void>(
...
ruvia::Task<void> exerciseAll(asio::io_context& ioContext, bool& syncOk, bool& voidOk,
    bool& deferredOk, bool& initiateCaught) {
...
int main() {
    ...
    asio::co_spawn(ioContext,
        ruvia::detail::taskAsAwaitable(
            exerciseAll(ioContext, syncOk, voidOk, deferredOk, initiateCaught)),
        asio::detached);
    ioContext.run();
