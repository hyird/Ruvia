...
ruvia::Task<void> exerciseSynchronousCompletion(bool& success) {
    const auto result = co_await ruvia::detail::asyncAsio<std::size_t>([](auto handler) mutable { handler(std::make_error_code(std::errc::connection_reset), std::size_t{42}); });
...
ruvia::Task<void> exerciseSynchronousVoid(bool& success) {
    (void)(co_await ruvia::detail::asyncAsio([](auto handler) mutable { handler(std::error_code{}); }));
    success = true;
}
...
ruvia::Task<void> exerciseDeferredCompletion(asio::io_context& ioContext, bool& success) {
    const auto result = co_await ruvia::detail::asyncAsio<std::size_t>([&ioContext](auto handler) mutable { asio::post(ioContext, [handler = std::move(handler)]() mutable { handler(std::make_error_code(std::errc::timed_out), std::size_t{7}); }); });
...
ruvia::Task<void> exerciseInitiateFailure(bool& caught) {
    try {
        (void)(co_await ruvia::detail::asyncAsio<void>([](auto) { throw std::runtime_error("initiate failure"); }));
