...
int main() {
    ...
    edge.start();
    check(edge.addOrigin("front.local", OriginSettings{"127.0.0.1", origin.port(), false}), "addOrigin maps the front host at runtime");
    ...
    // First request: a proxied cache miss reaches the origin.
    {
        ...
        check(contains(seen, "User-Agent: probe-agent"), "a forwardable client header reaches the origin");
        check(contains(seen, "Accept-Encoding: gzip"), "Accept-Encoding is forwarded so the origin may compress");
        check(contains(seen, "X-Forwarded-For: 127.0.0.1"), "X-Forwarded-For carries the client address");
        check(contains(seen, "X-Forwarded-Host: front.local"), "X-Forwarded-Host carries the front host");
        check(contains(seen, "X-Forwarded-Proto: http"), "X-Forwarded-Proto is set");
    ...
    // one cache key rather than fragmenting the cache.
    {
        ...
        check(statusOf(upper) == 200 && contains(upper, "X-Cache: MISS"), "uppercase Host resolves the configured origin");
        const auto lower = httpGet(edgePort, "front.local", "/host-case");
        check(contains(lower, "X-Cache: HIT"), "host case variants share a cache entry");
        check(origin.hits() == before + 1, "equivalent host case reaches the origin only once");
    }
    ...
    // must not restart the shared-cache age clock at zero.
    {
        ...
        check(age && *age >= 40, "cache hit preserves upstream Age instead of resetting it");
    }
    ...
    {
        const int before = origin.hits();
        const auto port80 = httpGet(edgePort, "front.local:80", "/authority-port");
        const auto port81 = httpGet(edgePort, "front.local:81", "/authority-port");
        check(contains(port80, "X-Cache: MISS") && contains(port81, "X-Cache: MISS"), "different Host ports do not collide in the cache");
        check(origin.hits() == before + 2, "different target authorities each reach the origin");
        check(contains(origin.lastRequest(), "X-Forwarded-Host: front.local:81"), "X-Forwarded-Host preserves the request authority port");
        const auto port80Hit = httpGet(edgePort, "FRONT.LOCAL:80", "/authority-port");
        check(contains(port80Hit, "X-Cache: HIT"), "authority host case remains canonicalized");

        check(edge.purge("front.local", "/authority-port"), "route-level purge removes authority variants");
        const int purgedBefore = origin.hits();
        check(contains(httpGet(edgePort, "front.local:80", "/authority-port"), "X-Cache: MISS"), "purge removes the first port variant");
        check(contains(httpGet(edgePort, "front.local:81", "/authority-port"), "X-Cache: MISS"), "purge removes the second port variant");
        check(origin.hits() == purgedBefore + 2, "both purged authority variants are refetched");
    }
    ...
    // Range: partial content served from the cached full body (no origin hit).
    {
        ...
        check(contains(r1, "Content-Range: bytes 0-1/5"), "Content-Range reflects the served slice");
        ...
        check(contains(r4, "Content-Range: bytes */5"), "416 reports the full representation length");
    ...
    {
        const auto response = httpRaw(edgePort,
            "GET /hop HTTP/1.1\r\nHost: front.local\r\n"
        ...
        const auto seen = origin.lastRequest();
        check(!contains(seen, "X-Client-Hop: client-secret"), "Connection-nominated request field is not forwarded");
        check(contains(seen, "X-End-To-End: retained"), "end-to-end request field is forwarded");
        check(!contains(response, "X-Origin-Hop: origin-secret"), "Connection-nominated origin field is not sent to the client");
        check(contains(response, "X-End-To-End: retained"), "end-to-end origin field is sent to the client");

        const auto cached = httpRaw(edgePort,
            "GET /hop HTTP/1.1\r\nHost: front.local\r\n"
            "Connection: close\r\n\r\n");
        check(contains(cached, "X-Cache: HIT"), "sanitized origin metadata remains cacheable");
        check(!contains(cached, "X-Origin-Hop: origin-secret"), "nominated origin field is absent from cached responses");
    }
    ...
    // treating a longer target with the same text prefix as a match.
    {
        ...
        check(contains(neighbor, "X-Cache: MISS"), "neighboring target is cached before purge");
        ...
        check(contains(retained, "X-Cache: HIT"), "purge target boundary retains a longer neighboring URI");
        check(origin.hits() == before, "retained neighboring URI does not contact the origin");
        const auto r = httpGet(edgePort, "front.local", "/page");
    ...
    // invalidates the cached GET for the same target on success.
    {
        ...
        check(contains(origin.lastRequest(), "payload=42"), "request body is forwarded to the origin");

        const auto get = httpGet(edgePort, "front.local", "/page");
        check(contains(get, "X-Cache: MISS"), "the unsafe method invalidated the cached GET");
    }
    ...
    // cached 200 or replacing them with its own revalidation validator.
    {
        ...
        const auto conditional = httpRaw(edgePort,
            "GET /page HTTP/1.1\r\nHost: front.local\r\n"
            "If-None-Match: \"v1\"\r\nConnection: close\r\n\r\n");
        check(statusOf(conditional) == 304, "client If-None-Match reaches the origin and returns 304");
        check(contains(conditional, "X-Cache: BYPASS"), "conditional retrieval bypasses local cache evaluation");
        check(bodyOf(conditional).empty(), "304 carries no response body");
        check(origin.notModified() == before + 1, "origin evaluated the client's validator");
    }
    ...
    // must not invalidate the cached GET representation.
    {
        ...
        check(contains(fill, "X-Cache: MISS"), "safe-method invalidation fixture is cached");
        const int before = origin.hits();
        const auto optionsResponse = httpRaw(edgePort,
            "OPTIONS /safe-options HTTP/1.1\r\nHost: front.local\r\n"
            "Connection: close\r\n\r\n");
        check(statusOf(optionsResponse) == 200 && contains(optionsResponse, "X-Cache: BYPASS"), "OPTIONS writes through to the origin");
        check(origin.hits() == before + 1, "OPTIONS contacted the origin exactly once");
        const auto retained = httpGet(edgePort, "front.local", "/safe-options");
        check(contains(retained, "X-Cache: HIT"), "safe OPTIONS does not invalidate cached GET");
        check(origin.hits() == before + 1, "retained GET cache entry avoids another origin request");
    }
    ...
    {
        const int before = origin.hits();
        const auto authenticated = httpRaw(edgePort,
            "GET /auth-private HTTP/1.1\r\nHost: front.local\r\n"
            "Authorization: Bearer secret\r\nConnection: close\r\n\r\n");
        check(contains(authenticated, "X-Cache: BYPASS"), "authenticated retrieval bypasses the shared cache");
        const auto anonymous = httpRaw(edgePort,
            "GET /auth-private HTTP/1.1\r\nHost: front.local\r\n"
            "Connection: close\r\n\r\n");
        check(contains(anonymous, "X-Cache: MISS"), "authenticated response was not stored for anonymous reuse");
        check(origin.hits() == before + 2, "both authenticated and first anonymous requests reach origin");
        const auto anonymousHit = httpRaw(edgePort,
            "GET /auth-private HTTP/1.1\r\nHost: front.local\r\n"
            "Connection: close\r\n\r\n");
        check(contains(anonymousHit, "X-Cache: HIT"), "ordinary anonymous retrieval remains cacheable");
    }
    ...
    {
        const auto fill = httpRaw(edgePort,
            "GET /directives HTTP/1.1\r\nHost: front.local\r\n"
            "Connection: close\r\n\r\n");
        check(contains(fill, "X-Cache: MISS"), "request-directive fixture is initially cached");

        int before = origin.hits();
        const auto revalidate = httpRaw(edgePort,
            "GET /directives HTTP/1.1\r\nHost: front.local\r\n"
            "Cache-Control: no-cache\r\nConnection: close\r\n\r\n");
        check(contains(revalidate, "X-Cache: BYPASS"), "request no-cache does not reuse a stored response directly");
        check(origin.hits() == before + 1, "request no-cache reaches the origin");

        before = origin.hits();
        const auto pragma = httpRaw(edgePort,
            "GET /directives HTTP/1.1\r\nHost: front.local\r\n"
            "Pragma: no-cache\r\nConnection: close\r\n\r\n");
        check(contains(pragma, "X-Cache: BYPASS"), "legacy Pragma no-cache is honored without Cache-Control");
        check(origin.hits() == before + 1, "legacy Pragma no-cache reaches the origin");

        before = origin.hits();
        const auto cachedOnlyHit = httpRaw(edgePort,
            "GET /directives HTTP/1.1\r\nHost: front.local\r\n"
            "Cache-Control: only-if-cached\r\nConnection: close\r\n\r\n");
        check(contains(cachedOnlyHit, "X-Cache: HIT"), "only-if-cached can use an existing fresh response");
        check(origin.hits() == before, "only-if-cached hit does not contact the origin");

        const auto cachedOnlyMiss = httpRaw(edgePort,
            "GET /only-if-cached-miss HTTP/1.1\r\nHost: front.local\r\n"
            "Cache-Control: only-if-cached\r\nConnection: close\r\n\r\n");
        check(statusOf(cachedOnlyMiss) == 504, "only-if-cached miss returns 504");
        check(origin.hits() == before, "only-if-cached miss does not contact the origin");

        before = origin.hits();
        const auto noStore = httpRaw(edgePort,
            "GET /request-no-store HTTP/1.1\r\nHost: front.local\r\n"
            "Cache-Control: no-store\r\nConnection: close\r\n\r\n");
        check(contains(noStore, "X-Cache: BYPASS"), "request no-store bypasses storage");
        const auto afterNoStore = httpRaw(edgePort,
            "GET /request-no-store HTTP/1.1\r\nHost: front.local\r\n"
            "Connection: close\r\n\r\n");
        check(contains(afterNoStore, "X-Cache: MISS"), "response to request no-store was not retained");
        check(origin.hits() == before + 2, "no-store exchange and later fill both reach the origin");
    }
    ...
    {
        check(edge.addOrigin("admin.local", OriginSettings{"127.0.0.1", origin.port(), false}), "addOrigin maps a second host at runtime");
    ...
    // Keep-alive: two requests are served on one persistent connection.
    {
        ...
        check(contains(second, "Connection: close"), "response 2 closes the connection as requested");
    }
    ...
    {
        const auto [first, second] = httpKeepAliveTwo(edgePort, "front.local", "/page", "HTTP/1.1", "disclose");
        check(statusOf(first) == 200 && statusOf(second) == 200, "an unrelated Connection token does not disable HTTP/1.1 reuse");

        const auto [http10First, http10Second] = httpKeepAliveTwo(edgePort, "front.local", "/page", "HTTP/1.0", "keep-alive");
        check(http10First.starts_with("HTTP/1.0 200"), "HTTP/1.0 request receives an HTTP/1.0 response");
        check(contains(http10First, "Connection: keep-alive"), "HTTP/1.0 keep-alive token enables reuse");
        check(http10Second.starts_with("HTTP/1.0 200"), "a second HTTP/1.0 request is served on the reused connection");
    }
    ...
    // stream is therefore close-delimited even if the request asked to persist.
    {
        ...
        check(response.starts_with("HTTP/1.0 200"), "close-delimited streaming preserves HTTP/1.0");
        check(!contains(response, "Transfer-Encoding: chunked"), "HTTP/1.0 streaming never emits chunked framing");
        check(contains(response, "Connection: close"), "unknown-length HTTP/1.0 stream closes for delimiting");
        check(bodyOf(response) == "hello world", "close-delimited HTTP/1.0 body is forwarded without chunk frames");
    }

    // Parse failures retain the status chosen by the shared protocol core.
    {
        const auto tooLarge = httpRaw(edgePort,
            "POST /oversize HTTP/1.1\r\nHost: front.local\r\n"
            "Content-Length: 1048577\r\nConnection: close\r\n\r\n");
        check(statusOf(tooLarge) == 413, "edge rejects a declared body above its 1 MB buffered limit");

        const auto unsupported = httpRaw(edgePort, "GET / HTTP/9.9\r\nHost: front.local\r\nConnection: close\r\n\r\n");
        check(statusOf(unsupported) == 505, "unsupported request version uses the protocol core's 505 status");
    }
    ...
    // then served buffered.
    {
        ...
        check(contains(r, "Transfer-Encoding: chunked"), "unknown-length body is re-chunked to the client");
        check(contains(r, "X-Cache: MISS"), "first chunked request is a MISS");
        check(dechunk(bodyOf(r)) == "hello world", "streamed chunked body decodes to the origin content");
    ...
    // separately, and a response that Varies on an uncovered field is not cached.
    {
        ...
        check(origin.hits() == base + 2, "distinct Accept-Encoding values fetch distinct variants");
        ...
        (void)httpGetEnc(edgePort, "front.local", "/vary-q", "gzip;q=1, br;q=0.1");
        (void)httpGetEnc(edgePort, "front.local", "/vary-q", "gzip;q=0.1, br;q=1");
        check(origin.hits() == weightedBase + 2, "different Accept-Encoding weights select distinct variants");

        const int repeatedBase = origin.hits();
        (void)httpRaw(edgePort,
        ...
        (void)httpRaw(edgePort,
            ...
            "Connection: close\r\n\r\n");
        check(origin.hits() == repeatedBase + 2, "every repeated Accept-Encoding field line participates in the key");
        ...
        (void)httpRaw(edgePort,
            "GET /vary-empty HTTP/1.1\r\nHost: front.local\r\n"
            "Accept-Encoding:\r\nConnection: close\r\n\r\n");
        check(origin.hits() == emptyBase + 2, "absent and present-empty Accept-Encoding remain distinct");
        ...
        check(contains(cookie2, "X-Cache: MISS"), "a Vary: Cookie response is not cached");
    ...
    // is then a fresh hit.
    {
        constexpr std::string_view kWeightedEncoding = "gzip;q=0.2, br;q=0.8";
        const auto r1 = httpGetEnc(edgePort, "front.local", "/swr", kWeightedEncoding);
        ...
        const auto r2 = httpGetEnc(edgePort, "front.local", "/swr", kWeightedEncoding);
        check(contains(r2, "X-Cache: STALE"), "stale-while-revalidate serves the stale copy immediately");
        ...
        check(origin.notModified() == notModifiedBefore + 1, "the background job revalidated with the origin");
        check(contains(origin.lastRequest(), "Accept-Encoding: gzip;q=0.2, br;q=0.8"), "background revalidation preserves the exact variant selector");

        const auto r3 = httpGetEnc(edgePort, "front.local", "/swr", kWeightedEncoding);
    ...
    // collapse into a single origin fetch, and every client still gets served.
    {
        ...
        for (int i = 0; i < kClients; ++i) {
            clients.emplace_back([&, i] { responses[i] = httpGet(edgePort, "front.local", "/slow"); });
        }
        for (auto& t : clients) {
            t.join();
        }
        ...
        check(origin.hits() == before + 1, "concurrent misses coalesced into a single origin fetch");
    }
    ...
    // the lease held by a request already suspended in origin I/O.
    {
        ...
        check(leaseEdge.addOrigin("lease.local", OriginSettings{"127.0.0.1", origin.port(), false}), "lease fixture origin is registered");
        leaseEdge.start();
        const auto port = leaseEdge.localEndpoint().port;
        const int before = origin.hits();
        std::string inFlightResponse;
        std::thread inFlight([&] { inFlightResponse = httpGet(port, "lease.local", "/slow"); });
        for (int attempt = 0; attempt < 100 && origin.hits() == before; ++attempt) {
            std::this_thread::sleep_for(std::chrono::milliseconds(2));
        }
        check(leaseEdge.removeOrigin("lease.local"), "control-plane removal is applied while a request is suspended");
        inFlight.join();
        check(statusOf(inFlightResponse) == 200, "in-flight request retains a stable origin lease after removal");
        check(statusOf(httpGet(port, "lease.local", "/slow")) == 502, "a later request observes the removed mapping");
        leaseEdge.stop();
    }
    ...
    // giving an embedding app the raw material to build metrics of its own.
    {
        ...
        obsOptions.accessLog = [&](const ruvia::edge::AccessLogEntry& e) {
            ...
            if (!callbackAddedOrigin) {
                callbackAddedOrigin = observed->addOrigin("callback.local", OriginSettings{"127.0.0.1", origin.port(), false});
            }
        };
        EdgeServer obsEdge(ruvia::edge::EdgeEndpoint{"0.0.0.0", 0}, std::move(obsOptions));
        observed = &obsEdge;
        obsEdge.start();
        check(obsEdge.addOrigin("front.local", OriginSettings{"127.0.0.1", origin.port(), false}), "observability fixture origin is registered");
        ...
        check(statusOf(httpGet(obsPort, "callback.local", "/callback")) == 200, "worker callback may invoke the serialized control plane directly");

        {
            std::lock_guard<std::mutex> guard(logMutex);
            check(logResults.size() == 3, "each request produced an access-log entry");
            check(logResults.size() == 3 && logResults[0] == "MISS", "first request logged as a MISS");
            check(logResults.size() == 3 && logResults[1] == "HIT", "second request logged as a HIT");
    ...
    // process), truncate the response, or poison later requests on the worker.
    {
        ...
        logFailureOptions.taskFailure = [&](const ruvia::edge::EdgeTaskFailure& failure) {
        ...
        EdgeServer logFailureEdge(ruvia::edge::EdgeEndpoint{"127.0.0.1", 0}, std::move(logFailureOptions));
        logFailureEdge.start();
        check(logFailureEdge.addOrigin("front.local", OriginSettings{"127.0.0.1", origin.port(), false}), "throwing-log fixture origin is registered");
        const auto port = logFailureEdge.localEndpoint().port;
        check(statusOf(httpGet(port, "front.local", "/log-failure-one")) == 200, "throwing access log does not truncate its request");
        check(statusOf(httpGet(port, "front.local", "/log-failure-two")) == 200, "worker remains usable after an access-log exception");
        check(logAttempts.load(std::memory_order_relaxed) == 2, "each completed request still attempts access logging");
        check(logFailureEdge.stats().accessLogFailures == 2, "contained failures are counted as well as reported");
        logFailureEdge.stop();
        {
            // The contained exception is reported, not dropped.
            std::lock_guard<std::mutex> guard(failureMutex);
            check(failureKinds.size() == 2, "each access-log exception reached the failure callback");
            check(std::ranges::all_of(failureKinds, [](ruvia::edge::EdgeTaskKind kind) { return kind == ruvia::edge::EdgeTaskKind::kAccessLog; }), "an access-log exception is reported as kAccessLog");
            check(std::ranges::all_of(failureMessages, [](const std::string& message) { return message == "access log failed"; }), "the original exception is delivered intact");
        }
    }
    ...
    // into the request-record destructor behind it.
    {
        ...
        doubleFailureOptions.accessLog = [](const ruvia::edge::AccessLogEntry&) { throw std::runtime_error("access log failed"); };
        doubleFailureOptions.taskFailure = [&](const ruvia::edge::EdgeTaskFailure&) {
            reportAttempts.fetch_add(1, std::memory_order_relaxed);
            throw std::runtime_error("failure callback failed");
        };
        EdgeServer doubleFailureEdge(ruvia::edge::EdgeEndpoint{"127.0.0.1", 0}, std::move(doubleFailureOptions));
        doubleFailureEdge.start();
        check(doubleFailureEdge.addOrigin("front.local", OriginSettings{"127.0.0.1", origin.port(), false}), "throwing-report fixture origin is registered");
        const auto port = doubleFailureEdge.localEndpoint().port;
        check(statusOf(httpGet(port, "front.local", "/report-failure-one")) == 200, "a throwing failure callback does not truncate its request");
        check(statusOf(httpGet(port, "front.local", "/report-failure-two")) == 200, "worker remains usable after a failure-callback exception");
        check(reportAttempts.load(std::memory_order_relaxed) == 2, "every contained failure still reaches the failure callback");
    ...
    // origin serves an HTTPS request end to end.
    {
        ...
        tlsOptions.tls = ruvia::edge::EdgeTlsConfig{std::string(edge_test_tls::kCertPem), std::string(edge_test_tls::kKeyPem)};
        ...
        tlsEdge.start();
        check(tlsEdge.addOrigin("front.local", OriginSettings{"127.0.0.1", origin.port(), false}), "TLS fixture origin is registered");
        ...
        check(contains(origin.lastRequest(), "X-Forwarded-Proto: https"), "TLS termination forwards the https scheme to the origin");

        // Rotate the certificate through the member API; HTTPS keeps working.
        const ruvia::edge::EdgeTlsConfig fresh{std::string(edge_test_tls::kCertPem), std::string(edge_test_tls::kKeyPem)};
        ...
        check(!tlsEdge.setTlsCertificate(ruvia::edge::EdgeTlsConfig{"not a cert", "not a key"}), "setTlsCertificate rejects invalid PEM");
        {
            std::lock_guard<std::mutex> guard(tlsFailureMutex);
            check(tlsFailures.size() == 1 && tlsFailures.front() == ruvia::edge::EdgeTaskKind::kControl, "a rejected certificate is reported as kControl");
            check(tlsFailureMessages.size() == 1 && !tlsFailureMessages.front().empty(), "the rejection carries the reason the PEM was refused");
        }
        tlsEdge.stop();

        // A plaintext edge has no certificate to rotate.
        check(!edge.setTlsCertificate(fresh), "setTlsCertificate fails when TLS is not enabled");
    }
    ...
    // keeps draining.
    {
        ...
        EdgeServer cappedEdge(ruvia::edge::EdgeEndpoint{"127.0.0.1", 0}, std::move(capOptions));
        cappedEdge.start();
        check(cappedEdge.addOrigin("front.local", OriginSettings{"127.0.0.1", origin.port(), false}), "capped fixture origin is registered");
        ...
        {
            const std::string request = "GET /page HTTP/1.1\r\nHost: front.local\r\n\r\n";
            asio::write(held, asio::buffer(request));
            char buffer[4096];
            asio::error_code ec;
            const std::size_t n = held.read_some(asio::buffer(buffer), ec);
            check(!ec && n > 0 && std::string_view(buffer, n).starts_with("HTTP/1.1 200"), "the connection holding the only slot is served normally");
        }

        check(cappedEdge.stats().activeConnections == 1, "the held connection is counted against the budget");

        // The budget is full: the next connection is closed without a response.
        check(httpGet(port, "front.local", "/page").empty(), "a connection beyond maxConnections is closed immediately");
        check(cappedEdge.stats().connectionsRefused == 1, "shedding a connection is counted, so it is visible without a callback");
        ...
        for (int attempt = 0; attempt < 150 && !servedAfterRelease; ++attempt) {
            servedAfterRelease = statusOf(httpGet(port, "front.local", "/page")) == 200;
            if (!servedAfterRelease) {
                std::this_thread::sleep_for(std::chrono::milliseconds(20));
            }
        }
        check(servedAfterRelease, "closing a connection returns its slot to the budget");
        cappedEdge.stop();
        check(cappedEdge.stats().activeConnections == 0, "every slot is returned once the node stops");
    }
    ...
    // Direct shutdown: stop() does not wait for an in-flight origin request.
    {
        ...
        stoppingEdge.start();
        check(stoppingEdge.addOrigin("front.local", OriginSettings{"127.0.0.1", origin.port(), false}), "shutdown fixture origin is registered");
        ...
        check(elapsed < std::chrono::milliseconds(200), "stop does not drain an in-flight request");
    ...
    // inside io_context until after the cache/config members are destroyed.
    {
        ...
        check(refreshEdge.addOrigin("front.local", OriginSettings{"127.0.0.1", origin.port(), false}), "refresh fixture origin is registered");
        refreshEdge.start();
        const auto port = refreshEdge.localEndpoint().port;

        const auto initial = httpGet(port, "front.local", "/swrslow");
        check(contains(initial, "X-Cache: MISS"), "slow-refresh fixture is initially cached");
        std::this_thread::sleep_for(std::chrono::milliseconds(1100));

        const int refreshesBefore = origin.slowRevalidations();
        const auto stale = httpGet(port, "front.local", "/swrslow");
        check(contains(stale, "X-Cache: STALE"), "slow background revalidation serves stale immediately");
        for (int attempt = 0; attempt < 100 && origin.slowRevalidations() == refreshesBefore; ++attempt) {
            std::this_thread::sleep_for(std::chrono::milliseconds(2));
        }
        check(origin.slowRevalidations() == refreshesBefore + 1, "slow background revalidation entered origin I/O");

        const auto started = std::chrono::steady_clock::now();
        refreshEdge.stop();
        const auto elapsed = std::chrono::steady_clock::now() - started;
        check(elapsed < std::chrono::milliseconds(200), "stop cancels and joins a tracked background refresh promptly");
    }
    ...
    {
        const std::filesystem::path diskDir = std::filesystem::temp_directory_path() / "ruvia_edge_disk_test";
        std::error_code ec;
...

        {
            ...
            diskEdge.start();
            check(diskEdge.addOrigin("front.local", OriginSettings{"127.0.0.1", origin.port(), false}), "disk fixture origin is registered");
        ...
        const int before = origin.hits();
        {
            ...
            EdgeServer reopened(ruvia::edge::EdgeEndpoint{"0.0.0.0", 0}, std::move(reopenOptions));
            reopened.start();
            check(reopened.addOrigin("front.local", OriginSettings{"127.0.0.1", origin.port(), false}), "reopened disk fixture origin is registered");
            const std::uint16_t reopenPort = reopened.localEndpoint().port;
            const auto hit = httpGet(reopenPort, "front.local", "/diskpage");
            check(contains(hit, "X-Cache: HIT"), "a fresh edge serves the entry from the disk tier");
            ...
            check(reopened.purge("front.local", "/diskpage"), "disk-backed purge waits for durable removal");
            reopened.stop();
        }
        ...
        const int afterPurge = origin.hits();
        {
            ...
            EdgeServer purged(ruvia::edge::EdgeEndpoint{"0.0.0.0", 0}, std::move(purgedOptions));
            purged.start();
            check(purged.addOrigin("front.local", OriginSettings{"127.0.0.1", origin.port(), false}), "purged disk fixture origin is registered");
            const auto refetched = httpGet(purged.localEndpoint().port, "front.local", "/diskpage");
            check(contains(refetched, "X-Cache: MISS"), "a reopened edge observes the completed disk purge");
            check(origin.hits() == afterPurge + 1, "durably purged entry is refetched from the origin");
            check(purged.clearCache(), "disk-backed clear reports complete durable removal");
            const int afterClear = origin.hits();
            const auto afterClearResponse = httpGet(purged.localEndpoint().port, "front.local", "/diskpage");
            check(contains(afterClearResponse, "X-Cache: MISS"), "clear removes both memory and disk tiers");
            check(origin.hits() == afterClear + 1, "request after clear is refetched from the origin");
            purged.stop();
        }
        std::filesystem::remove_all(diskDir, ec);
    }
...
#if !defined(_WIN32) && defined(RUVIA_NGHTTP_EXECUTABLE) && defined(RUVIA_H2LOAD_EXECUTABLE)
    {
        ...
        h2Options.tls = ruvia::edge::EdgeTlsConfig{std::string(edge_test_tls::kCertPem), std::string(edge_test_tls::kKeyPem)};
        EdgeServer h2Edge(ruvia::edge::EdgeEndpoint{"0.0.0.0", 0}, std::move(h2Options));
        h2Edge.start();
        check(h2Edge.addOrigin("front.local", OriginSettings{"127.0.0.1", origin.port(), false}), "HTTP/2 fixture authority is registered");
        // h2load addresses the edge by IP, so map that authority to the origin too.
        check(h2Edge.addOrigin("127.0.0.1", OriginSettings{"127.0.0.1", origin.port(), false}), "HTTP/2 IP authority is registered");
        ...
        const std::string page = runShell("\"" RUVIA_NGHTTP_EXECUTABLE "\" -y " + base + "/page");
        check(page == "hello", "h2 request served over ALPN and proxied the origin body");

        // 2. A chunked (unknown-length) origin response is streamed over h2.
        const std::string chunked = runShell("\"" RUVIA_NGHTTP_EXECUTABLE "\" -y " + base + "/chunked");
        check(chunked == "hello world", "h2 streamed body reassembles to the origin content");
        ...
        const std::string big = runShell("\"" RUVIA_NGHTTP_EXECUTABLE "\" -y " + base + "/bigchunk");
        check(big.size() == 200000, "h2 delivered the full flow-controlled body");

        // 4. True multiplexing: many concurrent streams on one connection all
        //    complete without head-of-line blocking or deadlock.
        const std::string mux = runShell("\"" RUVIA_H2LOAD_EXECUTABLE "\" -n16 -c1 -m8 " + base + "/bigchunk");
        check(contains(mux, "16 succeeded"), "16 concurrent h2 streams all succeeded");
        ...
        const auto uploadPath = std::filesystem::temp_directory_path() / "ruvia_edge_h2_upload.bin";
        ...
        const std::string uploads = runShell("\"" RUVIA_H2LOAD_EXECUTABLE "\" -n3 -c1 -m1 -d \"" + uploadPath.string() + "\" " + base + "/upload");
        check(contains(uploads, "3 succeeded"), "sequential h2 request bodies return connection window credit");
        ...
        check(writeUpload(1024 * 1024 + 1), "oversized h2 upload fixture was written");
        const std::string oversized = runShell("\"" RUVIA_H2LOAD_EXECUTABLE "\" -n1 -c1 -m1 -d \"" + uploadPath.string() + "\" " + base + "/oversized-upload");
        check(contains(oversized, "1 failed"), "oversized h2 request body is reset before proxying");
    ...
    // transfer -- then a follow-up is a plain hit again.
    {
        ...
        check(contains(b, "Content-Type: text/plain"), "revalidated response keeps the stored representation headers");
        check(origin.notModified() == notModifiedBefore + 1, "origin answered the conditional with 304");
        ...
        check(contains(c, "Content-Type: text/plain"), "the refreshed cache entry retained its headers");
    }
    ...
    {
        const auto initial = httpGet(edgePort, "front.local", "/nostore-transition");
        check(contains(initial, "X-Cache: MISS") && bodyOf(initial) == "hello", "no-store transition fixture starts as a cacheable response");
        std::this_thread::sleep_for(std::chrono::milliseconds(1100));

        const auto withdrawn = httpGet(edgePort, "front.local", "/nostore-transition");
        check(statusOf(withdrawn) == 200 && bodyOf(withdrawn) == "newer", "origin replaces the stale representation with no-store content");

        const auto failure = httpGet(edgePort, "front.local", "/nostore-transition");
        check(statusOf(failure) == 500, "withdrawn stale entry cannot reappear through stale-if-error");
        check(!contains(failure, "X-Cache: STALE"), "no-store replacement removed the previous cached response");
    }
    ...
    // stale-if-error window is served instead of a gateway error.
    {
        ...
        check(contains(b, "X-Cache: STALE"), "unreachable origin falls back to the stale copy");
