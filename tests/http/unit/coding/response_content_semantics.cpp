...
RUVIA_TEST(response_content_semantics_owns_method_status_precedence) {
    const auto informational =
        httpResponseContentSemantics(HttpKnownMethod::kGet, ruvia::http_status::kEarlyHints);
    ...
    const auto tunnel =
        httpResponseContentSemantics(HttpKnownMethod::kConnect, ruvia::http_status::kNoContent);
    ...
    const auto rejectedConnect =
        httpResponseContentSemantics(HttpKnownMethod::kConnect, ruvia::http_status::kNotFound);
    ...
    const auto resetContent =
        httpResponseContentSemantics(HttpKnownMethod::kGet, ruvia::http_status::kResetContent);
