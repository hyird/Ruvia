...
concept AcceptsAnyTemporaryHttpClientRequestViewText =
    requires(String&& value) { HttpClientRequestView{.method = std::forward<String>(value)}; } ||
    requires(String&& value) { HttpClientRequestView{.target = std::forward<String>(value)}; } ||
    requires(HttpClientRequestView& request, String&& value) {
...
concept AcceptsHttp1ConnectHeaders = requires(Http1ClientRequestWriter& writer,
    const HttpOriginView& origin, std::array<char, 512>& buffer,
...
static_assert(std::same_as<decltype(std::declval<HttpClientRequestView&>().headers),
...
struct PreparedFixture final {
    ...
    PreparedFixture(const HttpOriginView& origin, const HttpClientRequestView& request,
        Http1ClientRequestWirePolicy policy = Http1ClientRequestWirePolicy{})
...
[[nodiscard]] Http1ClientRequestPrepareError prepareError(const HttpClientRequestView& request,
    ...
    const auto result = Http1ClientRequestWriter().prepare(
        HttpOriginView::https({.host = "example.test"}), request, buffer, policy);
...
RUVIA_TEST(http1_client_request_writer_emits_one_canonical_scatter_gather_plan) {
    ...
    PreparedFixture fixture(HttpOriginView::https({.host = "example.test"}), request,
...
RUVIA_TEST(http1_client_request_content_distinguishes_absent_from_explicit_empty) {
    ...
    PreparedFixture absentFixture(HttpOriginView::https({.host = "example.test"}), absent);
    ...
    PreparedFixture emptyFixture(HttpOriginView::https({.host = "example.test"}), empty);
...
RUVIA_TEST(http1_client_request_writer_owns_request_target_forms_and_host) {
    ...
    PreparedFixture extensionFixture(
        HttpOriginView::https({.host = "example.test", .port = 8443}), extension);
    ...
    options.target = "*";
    PreparedFixture optionsFixture(HttpOriginView::http({.host = "example.test"}), options);
    ...
    RUVIA_CHECK(
        prepareError(connect) == Http1ClientRequestPrepareError::kConnectRequiresDedicatedEntry);
}
...
RUVIA_TEST(http1_client_connect_entry_generates_authority_form_atomically) {
    ...
    const auto result = Http1ClientRequestWriter().prepareConnect(
        HttpOriginView::https({.host = "example.test"}), {}, buffer);
    ...
    const auto ipv6 = Http1ClientRequestWriter().prepareConnect(
        HttpOriginView::https({.host = "[::1]"}), {}, ipv6Buffer);
    ...
    const auto invalid = Http1ClientRequestWriter().prepareConnect(
        HttpOriginView::https({.host = "example.test", .port = 0}), {}, invalidBuffer);
    RUVIA_CHECK(invalid.failure() != nullptr);
    RUVIA_CHECK(
        invalid.failure()->error() == Http1ClientRequestPrepareError::kInvalidConnectOrigin);
}
...
RUVIA_TEST(http1_client_request_writer_is_the_only_host_and_framing_owner) {
    ...
    const Case cases[] = {
        ...
        {"Transfer-Encoding", "chunked",
            Http1ClientRequestPrepareError::kTransferEncodingUnsupported},
...
RUVIA_TEST(http1_client_request_writer_validates_cors_fields) {
    ...
    PreparedFixture fixture(HttpOriginView::https({.host = "example.test"}), valid);
...
RUVIA_TEST(http1_client_request_writer_validates_te_capabilities_and_weights) {
    const auto prepareWithTe =
        [&ruvia_ctx](std::string_view value) -> std::optional<Http1ClientRequestPrepareError> {
        ...
        std::array<char, 512> buffer;
        const auto result = Http1ClientRequestWriter().prepare(
            HttpOriginView::https({.host = "example.test"}), request, buffer);
...
RUVIA_TEST(http1_client_request_writer_enforces_expect_content_semantics) {
    ...
    RUVIA_CHECK(prepareError(rawExpectation) ==
    ...
    if (absentFixture.result.failure() != nullptr) {
        RUVIA_CHECK(absentFixture.result.failure()->error() ==
    ...
    if (emptyFixture.result.failure() != nullptr) {
        RUVIA_CHECK(emptyFixture.result.failure()->error() ==
                    Http1ClientRequestPrepareError::kExpectationWithoutContent);
    }
    ...
    PreparedFixture fixture(HttpOriginView::https({.host = "example.test"}), nonempty,
        Http1ClientRequestWirePolicy{
            .expectation = ruvia::HttpClientRequestExpectation::kContinue});
    RUVIA_CHECK(fixture.result.prepared() != nullptr);
...
RUVIA_TEST(http1_client_request_writer_rejects_invalid_close_policy) {
    ...
    const auto connect =
        Http1ClientRequestWriter().prepareConnect(HttpOriginView::https({.host = "example.test"}),
            std::span<const ruvia::HttpHeaderView>{}, buffer, invalidWithoutExpectation);
...
RUVIA_TEST(http1_client_request_writer_enforces_method_content_semantics) {
    ...
    RUVIA_CHECK(
        prepareError(options) == Http1ClientRequestPrepareError::kOptionsContentTypeRequired);
    ...
    PreparedFixture fixture(HttpOriginView::https({.host = "example.test"}), options);
    RUVIA_CHECK(fixture.result.prepared() != nullptr);
...
RUVIA_TEST(http1_client_request_writer_returns_exact_buffer_requirement_without_partial_output) {
    ...
    const auto tooSmall = Http1ClientRequestWriter().prepare(
        HttpOriginView::https({.host = "example.test"}), request, small);
    ...
    const auto prepared = Http1ClientRequestWriter().prepare(
        HttpOriginView::https({.host = "example.test"}), request, enough);
    RUVIA_CHECK(prepared.prepared() != nullptr);
    if (prepared.prepared() != nullptr && tooSmall.bufferTooSmall() != nullptr) {
        RUVIA_CHECK_EQ(
            tooSmall.bufferTooSmall()->requiredHeadBytes(), prepared.prepared()->head().size());
    }
}
...
RUVIA_TEST(http1_client_upgrade_state_allocation_failure_leaves_head_buffer_untouched) {
    ...
    try {
        (void)Http1ClientRequestWriter({.resource = &rejecting})
            .prepare(HttpOriginView::https({.host = "example.test"}), request, output);
...
RUVIA_TEST(http1_client_request_writer_enforces_header_count_and_size_transactionally) {
    ...
    const auto invalid = Http1ClientRequestWriter().prepare(
        HttpOriginView::https({.host = "example.test"}), invalidMethod, untouched);
    RUVIA_CHECK(invalid.failure() != nullptr);
...
RUVIA_TEST(http1_client_request_context_binds_the_actual_close_signal) {
    ...
    PreparedFixture explicitClose(HttpOriginView::https({.host = "example.test"}), request);
    ...
    if (explicitPrepared != nullptr) {
        ...
        if (response.parsed() != nullptr) {
            ...
            if (withoutContent != nullptr) {
                RUVIA_CHECK(
                    withoutContent->persistence() == ruvia::Http1ClosePolicy::kCloseAfterResponse);
            }
        }
    }

    HttpClientRequestView generatedRequest;
    PreparedFixture generatedClose(HttpOriginView::https({.host = "example.test"}),
        generatedRequest,
        Http1ClientRequestWirePolicy{.closePolicy = Http1ClosePolicy::kCloseAfterResponse});
