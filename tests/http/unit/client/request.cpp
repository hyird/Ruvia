...
concept AcceptsAnyTemporaryHttpClientRequestText = requires(String&& value) { HttpClientRequest{.method = std::forward<String>(value)}; } || requires(String&& value) { HttpClientRequest{.target = std::forward<String>(value)}; } || requires(HttpClientRequest& request, String&& value) { request.method = std::forward<String>(value); } || requires(HttpClientRequest& request, String&& value) { request.target = std::forward<String>(value); };

template <typename String>
concept AcceptsLvalueHttpClientRequestText = requires(HttpClientRequest& request, String& value) {
...
concept AcceptsHttp1ConnectHeaders = requires(Http1ClientRequestWriter& writer, const HttpOrigin& origin, std::array<char, 512>& buffer, Headers&& headers) { writer.prepareConnect(origin, std::forward<Headers>(headers), buffer); };
...
constexpr HttpClientRequest kLiteralHttpClientRequest{.method = "POST", .target = "/items"};
...
static_assert(!std::is_constructible_v<HttpClientRequest::HeaderInit, std::array<ruvia::HttpHeaderView, 1>&&>);
static_assert(!std::is_assignable_v<HttpClientRequest::HeaderInit&, std::array<ruvia::HttpHeaderView, 1>&&>);
...
struct PreparedFixture final {
    ...
    PreparedFixture(const HttpOrigin& origin, const HttpClientRequest& request, Http1ClientRequestWirePolicy policy = Http1ClientRequestWirePolicy::withoutExpectation())
        : result(Http1ClientRequestWriter().prepare(origin, request, buffer, policy)) {}
};

[[nodiscard]] Http1ClientRequestPrepareError prepareError(const HttpClientRequest& request) {
    std::array<char, 512> buffer;
    const auto result = Http1ClientRequestWriter().prepare(HttpOrigin::https("example.test"), request, buffer);
...
RUVIA_TEST(http1_client_request_writer_emits_one_canonical_scatter_gather_plan) {
    ...
    PreparedFixture fixture(HttpOrigin::https("example.test"), request, Http1ClientRequestWirePolicy::expectContinue());
...
RUVIA_TEST(http1_client_request_writer_owns_request_target_forms_and_host) {
    ...
    PreparedFixture extensionFixture(HttpOrigin::https("example.test", 8443), extension);
...
RUVIA_TEST(http1_client_connect_entry_generates_authority_form_atomically) {
    std::array<char, 512> buffer;
    const auto result = Http1ClientRequestWriter().prepareConnect(HttpOrigin::https("example.test"), {}, buffer);
    ...
    const auto ipv6 = Http1ClientRequestWriter().prepareConnect(HttpOrigin::https("[::1]"), {}, ipv6Buffer);
    ...
    const auto invalid = Http1ClientRequestWriter().prepareConnect(HttpOrigin::https("example.test", 0), {}, invalidBuffer);
...
RUVIA_TEST(http1_client_request_writer_enforces_expect_content_semantics) {
    ...
    HttpClientRequest absent;
    absent.method = "POST";
    PreparedFixture absentFixture(HttpOrigin::https("example.test"), absent, Http1ClientRequestWirePolicy::expectContinue());
    ...
    empty.content = HttpClientRequestContent::bytes("");
    PreparedFixture emptyFixture(HttpOrigin::https("example.test"), empty, Http1ClientRequestWirePolicy::expectContinue());
    ...
    PreparedFixture fixture(HttpOrigin::https("example.test"), nonempty, Http1ClientRequestWirePolicy::expectContinue());
    RUVIA_CHECK(fixture.result.prepared() != nullptr);
...
RUVIA_TEST(http1_client_request_writer_rejects_invalid_content_encoding_syntax) {
    ...
    for (const std::string_view value : {"deflate", "gzip, br"}) {
        ...
        request.headers = std::span<const ruvia::HttpHeaderView>(&contentEncoding, 1);
        request.content = HttpClientRequestContent::bytes("body");
        PreparedFixture fixture(HttpOrigin::https("example.test"), request);
        RUVIA_CHECK(fixture.result.prepared() != nullptr);
    }
}
...
RUVIA_TEST(http1_client_request_writer_returns_exact_buffer_requirement_without_partial_output) {
    ...
    small.fill('z');
    const auto tooSmall = Http1ClientRequestWriter().prepare(HttpOrigin::https("example.test"), request, small);
    ...
    const auto prepared = Http1ClientRequestWriter().prepare(HttpOrigin::https("example.test"), request, enough);
...
RUVIA_TEST(http1_client_request_writer_enforces_header_count_and_size_transactionally) {
    ...
    untouched.fill('q');
    const auto invalid = Http1ClientRequestWriter().prepare(HttpOrigin::https("example.test"), invalidMethod, untouched);
    RUVIA_CHECK(invalid.failure() != nullptr);
...
RUVIA_TEST(http1_client_request_context_binds_the_actual_close_signal) {
    ...
    PreparedFixture generatedClose(HttpOrigin::https("example.test"), generatedRequest, Http1ClientRequestWirePolicy::withoutExpectation(Http1ClientRequestClosePolicy::kCloseAfterResponse));
