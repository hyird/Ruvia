...
RUVIA_TEST(http_client_rejects_invalid_or_repeated_content_type) {
    const auto invalid = parseResult("GET",
        "HTTP/1.1 200 OK\r\n"
        "Content-Type: not a media type\r\n"
        "Content-Length: 0");
    RUVIA_CHECK(invalid.failure() != nullptr);
    if (invalid.failure() != nullptr) {
        RUVIA_CHECK(invalid.failure()->error() == Http1ClientResponseParseError::kInvalidHeader);
    }

    const auto repeated = parseResult("GET",
        "HTTP/1.1 200 OK\r\n"
    ...
    if (repeated.failure() != nullptr) {
        RUVIA_CHECK(repeated.failure()->error() == Http1ClientResponseParseError::kInvalidHeader);
    }
...
RUVIA_TEST(http_client_response_parser_owns_exact_head_boundary) {
    ...
    RUVIA_CHECK(parsed->head().protocolVersion() == HttpProtocolVersion::kHttp11);
