...
concept CanParseSetCookie = requires(Input&& input) { ruvia::parseSetCookie(std::forward<Input>(input)); };
...
RUVIA_TEST(set_cookie_parser_exposes_client_storage_fields) {
    ...
    RUVIA_CHECK(!parsed->has(static_cast<ruvia::HttpSetCookieAttribute>(static_cast<std::uint8_t>(ruvia::HttpSetCookieAttribute::kSecure) | static_cast<std::uint8_t>(ruvia::HttpSetCookieAttribute::kPath))));
}
...
RUVIA_TEST(set_cookie_parser_treats_empty_domain_as_host_only) {
    ...
    const auto overridesEarlierDomain = ruvia::parseSetCookie("sid=abc; Domain=example.com; Domain=");
...
RUVIA_TEST(set_cookie_parser_saturates_max_age_overflow) {
    const auto positive = ruvia::parseSetCookie("sid=abc; Max-Age=999999999999999999999999999999999999");
    RUVIA_CHECK(positive.has_value());
    RUVIA_CHECK(positive->maxAgeSeconds() == std::chrono::duration_cast<std::chrono::seconds>(std::chrono::days(400)).count());

    const auto negative = ruvia::parseSetCookie("sid=abc; Max-Age=-999999999999999999999999999999999999");
...
RUVIA_TEST(set_cookie_parser_ignores_invalid_later_expires_attribute) {
    const auto parsed = ruvia::parseSetCookie("sid=abc; Expires=Sun, 06 Nov 1994 08:49:37 GMT; Expires=not-a-date");
    RUVIA_CHECK(parsed.has_value());
...
RUVIA_TEST(set_cookie_parser_uses_cookie_date_token_grammar) {
    ...
    const auto shortYear = ruvia::parseSetCookie("sid=abc; Expires=Thursday, 01-Jan-70 00:00:00 GMT");
