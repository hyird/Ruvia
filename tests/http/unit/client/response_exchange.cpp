...
RUVIA_TEST(http_client_limits_informational_responses_per_exchange) {
    ...
    const auto prepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), request, requestHead);
...
RUVIA_TEST(http_client_expect_continue_is_one_stateful_exchange_contract) {
    ...
    std::array<char, 512> requestHead;
    const auto preparedResult = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), request, requestHead, Http1ClientRequestWirePolicy{.expectation = ruvia::HttpClientRequestExpectation::kContinue});
    ...
    Http1ClientResponseParser parser(prepared->exchangeState());
...
RUVIA_TEST(http_client_closing_informational_response_ends_exchange) {
    ...
    const auto getPrepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), getRequest, getHead);
    RUVIA_CHECK(getPrepared.prepared() != nullptr);
    if (getPrepared.prepared() == nullptr) {
        return;
    }

    Http1ClientResponseParser getParser(getPrepared.prepared()->exchangeState());
    ...
    const auto postPrepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), postRequest, postHead, Http1ClientRequestWirePolicy{.expectation = ruvia::HttpClientRequestExpectation::kContinue});
    RUVIA_CHECK(postPrepared.prepared() != nullptr);
    if (postPrepared.prepared() == nullptr) {
        return;
    }

    Http1ClientResponseParser postParser(postPrepared.prepared()->exchangeState());
    ...
    if (closingContinue.parsed() != nullptr) {
        RUVIA_CHECK(closingContinue.parsed()->plan().requestContentSignal() == HttpClientRequestContentSignal::kExchangeComplete);
    }

    Http1ClientResponseParser closingHintsParser(postPrepared.prepared()->exchangeState());
    ...
    if (closingHints.parsed() != nullptr) {
        RUVIA_CHECK(closingHints.parsed()->plan().requestContentSignal() == HttpClientRequestContentSignal::kExchangeComplete);
    }

    std::array<char, 512> requestCloseHead;
    const auto requestClosePrepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), getRequest, requestCloseHead, Http1ClientRequestWirePolicy{.closePolicy = Http1ClosePolicy::kCloseAfterResponse});
    RUVIA_CHECK(requestClosePrepared.prepared() != nullptr);
    if (requestClosePrepared.prepared() == nullptr) {
        return;
    }

    Http1ClientResponseParser requestCloseParser(requestClosePrepared.prepared()->exchangeState());
...
RUVIA_TEST(http_client_expect_final_cancels_only_pending_request_content) {
    ...
    request.content = ruvia::HttpClientRequestContentView::bytes("payload");
    std::array<char, 512> requestHead;
    const auto prepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), request, requestHead, Http1ClientRequestWirePolicy{.expectation = ruvia::HttpClientRequestExpectation::kContinue});
    RUVIA_CHECK(prepared.prepared() != nullptr);
    if (prepared.prepared() == nullptr) {
        return;
    }
    ...
    RUVIA_CHECK(finalResponse.parsed() != nullptr);
    if (finalResponse.parsed() != nullptr) {
        RUVIA_CHECK(finalResponse.parsed()->plan().requestContentSignal() == HttpClientRequestContentSignal::kExchangeComplete);
    }

    Http1ClientResponseParser completedParser(prepared.prepared()->exchangeState());
...
RUVIA_TEST(http_client_upgrade_after_expect_requires_prior_continue) {
    ...
    const auto rejectedPrepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), request, rejectedHead, Http1ClientRequestWirePolicy{.expectation = ruvia::HttpClientRequestExpectation::kContinue});
    RUVIA_CHECK(rejectedPrepared.prepared() != nullptr);
    if (rejectedPrepared.prepared() == nullptr) {
        return;
    }
    Http1ClientResponseParser rejectedParser(rejectedPrepared.prepared()->exchangeState());
    ...
    const auto completedWithoutContinuePrepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), request, completedWithoutContinueHead, Http1ClientRequestWirePolicy{.expectation = ruvia::HttpClientRequestExpectation::kContinue});
    RUVIA_CHECK(completedWithoutContinuePrepared.prepared() != nullptr);
    if (completedWithoutContinuePrepared.prepared() == nullptr) {
        return;
    }
    Http1ClientResponseParser completedWithoutContinueParser(completedWithoutContinuePrepared.prepared()->exchangeState());
    ...
    const auto lateContinuePrepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), request, lateContinueHead, Http1ClientRequestWirePolicy{.expectation = ruvia::HttpClientRequestExpectation::kContinue});
    RUVIA_CHECK(lateContinuePrepared.prepared() != nullptr);
    if (lateContinuePrepared.prepared() == nullptr) {
        return;
    }
    Http1ClientResponseParser lateContinueParser(lateContinuePrepared.prepared()->exchangeState());
    ...
    const auto pendingPrepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), request, pendingHead, Http1ClientRequestWirePolicy{.expectation = ruvia::HttpClientRequestExpectation::kContinue});
    RUVIA_CHECK(pendingPrepared.prepared() != nullptr);
    if (pendingPrepared.prepared() == nullptr) {
        return;
    }
    Http1ClientResponseParser pendingParser(pendingPrepared.prepared()->exchangeState());
    ...
    const auto acceptedPrepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), request, acceptedHead, Http1ClientRequestWirePolicy{.expectation = ruvia::HttpClientRequestExpectation::kContinue});
    RUVIA_CHECK(acceptedPrepared.prepared() != nullptr);
    if (acceptedPrepared.prepared() == nullptr) {
        return;
    }
    Http1ClientResponseParser acceptedParser(acceptedPrepared.prepared()->exchangeState());
...
RUVIA_TEST(http_client_upgrade_requires_complete_request_content) {
    ...
    const auto incompletePrepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), request, incompleteHead);
    RUVIA_CHECK(incompletePrepared.prepared() != nullptr);
    if (incompletePrepared.prepared() == nullptr) {
        return;
    }
    Http1ClientResponseParser incompleteParser(incompletePrepared.prepared()->exchangeState());
    ...
    const auto completePrepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), request, completeHead);
    RUVIA_CHECK(completePrepared.prepared() != nullptr);
    if (completePrepared.prepared() == nullptr) {
        return;
    }
    Http1ClientResponseParser completeParser(completePrepared.prepared()->exchangeState());
...
RUVIA_TEST(http1_client_exchange_state_owns_every_long_lived_response_fact) {
    std::optional<ruvia::Http1ClientExchangeState> headExchange;
    std::array<char, 512> headWire{};
    std::string method = "HEAD";
    {
        ruvia::HttpClientRequestView request{.method = method, .target = "/resource"};
        const auto prepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), request, headWire);
        if (prepared.prepared() == nullptr) {
    ...
    std::array<char, 512> upgradeWire{};
    std::string connection = "Upgrade";
    std::string protocol = "websocket";
    {
        const std::array headers{
            ruvia::HttpHeaderView{"Connection", connection},
            ruvia::HttpHeaderView{"Upgrade", protocol},
        };
        ruvia::HttpClientRequestView request{.method = "GET", .target = "/socket", .headers = headers};
        const auto prepared = ruvia::Http1ClientRequestWriter().prepare(ruvia::HttpOriginView::https({.host = "example.test"}), request, upgradeWire);
        if (prepared.prepared() == nullptr) {
    ...
    auto upgradeResult = upgradeParser.parse("HTTP/1.1 101 Switching Protocols\r\nConnection: Upgrade\r\nUpgrade: websocket\r\n\r\n");
