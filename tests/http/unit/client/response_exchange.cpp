...
RUVIA_TEST(http_client_expect_continue_is_one_stateful_exchange_contract) {
    ...
    RUVIA_CHECK(parser.completeRequestContent() ==
...
RUVIA_TEST(http_client_closing_informational_response_ends_exchange) {
    ...
    const auto getPrepared = ruvia::Http1ClientRequestWriter().prepare(
        ruvia::HttpOriginView::https({.host = "example.test"}), getRequest, getHead);
    ...
    const auto earlyHints =
        getParser.parse("HTTP/1.1 103 Early Hints\r\nConnection: close\r\n\r\n");
    RUVIA_CHECK(earlyHints.parsed() != nullptr);
    if (earlyHints.parsed() != nullptr) {
    ...
    const auto postPrepared = ruvia::Http1ClientRequestWriter().prepare(
        ruvia::HttpOriginView::https({.host = "example.test"}), postRequest, postHead,
        Http1ClientRequestWirePolicy{
            .expectation = ruvia::HttpClientRequestExpectation::kContinue});
    ...
    const auto closingContinue =
        postParser.parse("HTTP/1.1 100 Continue\r\nConnection: close\r\n\r\n");
    RUVIA_CHECK(closingContinue.parsed() != nullptr);
    if (closingContinue.parsed() != nullptr) {
        RUVIA_CHECK(closingContinue.parsed()->plan().requestContentSignal() ==
    ...
    const auto closingHints =
        closingHintsParser.parse("HTTP/1.1 103 Early Hints\r\nConnection: close\r\n\r\n");
    RUVIA_CHECK(closingHints.parsed() != nullptr);
    if (closingHints.parsed() != nullptr) {
        RUVIA_CHECK(closingHints.parsed()->plan().requestContentSignal() ==
                    HttpClientRequestContentSignal::kExchangeComplete);
    }

    std::array<char, 512> requestCloseHead;
    const auto requestClosePrepared = ruvia::Http1ClientRequestWriter().prepare(
        ruvia::HttpOriginView::https({.host = "example.test"}), getRequest, requestCloseHead,
    ...
    if (requestCloseFinal.parsed() != nullptr) {
        RUVIA_CHECK(requireWithoutContent(requestCloseFinal.parsed()->plan()).persistence() ==
...
RUVIA_TEST(http_client_expect_final_cancels_only_pending_request_content) {
    ...
    RUVIA_CHECK(completedParser.completeRequestContent() ==
                Http1ClientRequestContentCompletionStatus::kCompleted);
    const auto completedFinal =
        completedParser.parse("HTTP/1.1 417 Expectation Failed\r\nContent-Length: 0\r\n\r\n");
...
RUVIA_TEST(http_client_upgrade_after_expect_requires_prior_continue) {
    ...
    const auto rejectedPrepared = ruvia::Http1ClientRequestWriter().prepare(
        ruvia::HttpOriginView::https({.host = "example.test"}), request, rejectedHead,
        Http1ClientRequestWirePolicy{
            .expectation = ruvia::HttpClientRequestExpectation::kContinue});
    ...
    if (rejected.failure() != nullptr) {
        RUVIA_CHECK(
            rejected.failure()->error() == Http1ClientResponseParseError::kInvalidProtocolSwitch);
    }
    ...
    const auto completedWithoutContinuePrepared = ruvia::Http1ClientRequestWriter().prepare(
        ruvia::HttpOriginView::https({.host = "example.test"}), request,
    ...
    Http1ClientResponseParser completedWithoutContinueParser(
        completedWithoutContinuePrepared.prepared()->exchangeState());
    RUVIA_CHECK(completedWithoutContinueParser.completeRequestContent() ==
                Http1ClientRequestContentCompletionStatus::kCompleted);
    ...
    if (completedWithoutContinue.failure() != nullptr) {
        RUVIA_CHECK(completedWithoutContinue.failure()->error() ==
                    Http1ClientResponseParseError::kInvalidProtocolSwitch);
    }

    std::array<char, 512> lateContinueHead;
    const auto lateContinuePrepared = ruvia::Http1ClientRequestWriter().prepare(
        ruvia::HttpOriginView::https({.host = "example.test"}), request, lateContinueHead,
        Http1ClientRequestWirePolicy{
            .expectation = ruvia::HttpClientRequestExpectation::kContinue});
    ...
    RUVIA_CHECK(lateContinueParser.completeRequestContent() ==
                Http1ClientRequestContentCompletionStatus::kCompleted);
    ...
    const auto pendingPrepared = ruvia::Http1ClientRequestWriter().prepare(
        ruvia::HttpOriginView::https({.host = "example.test"}), request, pendingHead,
        Http1ClientRequestWirePolicy{
            .expectation = ruvia::HttpClientRequestExpectation::kContinue});
    ...
    if (pendingUpgrade.failure() != nullptr) {
        RUVIA_CHECK(pendingUpgrade.failure()->error() ==
                    Http1ClientResponseParseError::kInvalidProtocolSwitch);
    }

    std::array<char, 512> acceptedHead;
    const auto acceptedPrepared = ruvia::Http1ClientRequestWriter().prepare(
        ruvia::HttpOriginView::https({.host = "example.test"}), request, acceptedHead,
        Http1ClientRequestWirePolicy{
            .expectation = ruvia::HttpClientRequestExpectation::kContinue});
    ...
    RUVIA_CHECK(continueResponse.parsed() != nullptr);
    RUVIA_CHECK(acceptedParser.completeRequestContent() ==
                Http1ClientRequestContentCompletionStatus::kCompleted);
...
RUVIA_TEST(http_client_upgrade_requires_complete_request_content) {
    ...
    const auto incompletePrepared = ruvia::Http1ClientRequestWriter().prepare(
        ruvia::HttpOriginView::https({.host = "example.test"}), request, incompleteHead);
    ...
    if (incomplete.failure() != nullptr) {
        RUVIA_CHECK(
            incomplete.failure()->error() == Http1ClientResponseParseError::kInvalidProtocolSwitch);
    }

    std::array<char, 512> completeHead;
    const auto completePrepared = ruvia::Http1ClientRequestWriter().prepare(
        ruvia::HttpOriginView::https({.host = "example.test"}), request, completeHead);
    ...
    RUVIA_CHECK(completeParser.completeRequestContent() ==
                Http1ClientRequestContentCompletionStatus::kCompleted);
...
RUVIA_TEST(http_client_rejects_end_to_end_connection_options) {
    for (const std::string_view option :
        ...
        response.append("\r\nContent-Length: 0");
        RUVIA_CHECK(parseFailureError("GET", response) ==
...
RUVIA_TEST(http_client_response_plan_owns_version_and_connection_persistence) {
    ...
    const auto http10KeepAlive =
        parseHead("GET", "HTTP/1.0 200 OK\r\nConnection: keep-alive\r\nContent-Length: 3");
    RUVIA_CHECK(
        requireKnownLength(http10KeepAlive.plan()).persistence() == Http1ClosePolicy::kAllowReuse);

    const auto responseClose =
        parseHead("GET", "HTTP/1.1 200 OK\r\nConnection: close\r\nContent-Length: 3");
    RUVIA_CHECK(requireKnownLength(responseClose.plan()).persistence() ==
                Http1ClosePolicy::kCloseAfterResponse);

    const auto requestClose = parseHead(
        "GET", "HTTP/1.1 200 OK\r\nContent-Length: 3", Http1ClosePolicy::kCloseAfterResponse);
    RUVIA_CHECK(requireKnownLength(requestClose.plan()).persistence() ==
                Http1ClosePolicy::kCloseAfterResponse);
}
...
RUVIA_TEST(http_client_successful_connect_transitions_to_tunnel) {
    ...
    const auto rejected =
        parseHead("CONNECT", "HTTP/1.1 407 Proxy Authentication Required\r\nContent-Length: 3");
