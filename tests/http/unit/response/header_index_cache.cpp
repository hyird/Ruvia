...
std::array<HttpResponseHeader, 3> sampleHeaders() {
    return {
        makeResponseHeader(
            "content-typetext/html", 12, 9, ruvia::detail::kResponseHeaderContentType, false),
        makeResponseHeader("x-customval", 8, 3, 0, false),
        makeResponseHeader(
            "locationhttps://x", 8, 9, ruvia::detail::kResponseHeaderLocation, false),
    };
}
...
RUVIA_TEST(find_response_header_indexed_cache_hit) {
    ...
    const auto* found =
        findResponseHeaderIndexed(headers.data(), headers.data() + headers.size(), cache, 5, "", 0);
...
RUVIA_TEST(find_response_header_indexed_cache_miss_is_authoritative) {
    ...
    const auto* found = findResponseHeaderIndexed(headers.data(), headers.data() + headers.size(),
        cache, 5, "location", ruvia::detail::kResponseHeaderLocation);
...
RUVIA_TEST(find_response_header_indexed_overflow_scans_by_bit) {
    ...
    const auto* found = findResponseHeaderIndexed(headers.data(), headers.data() + headers.size(),
        cache, 5, "", ruvia::detail::kResponseHeaderLocation);
