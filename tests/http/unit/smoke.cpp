...
int main() {
    ...
    if (interimResult.prepared() == nullptr ||
        interimResult.prepared()->head() != "HTTP/1.1 100 Continue\r\n\r\n") {
        return 5;
    }
    const auto result =
        ruvia::Http1RequestParser().parse("GET / HTTP/1.1\r\nHost: example.test\r\n\r\n");
    ...
    const auto requestPlan =
        ruvia::Http1ClientRequestWriter().prepare(origin, outboundRequest, requestHead);
    ...
    return origin.host().empty() || origin.port() != 443 ||
                   origin.scheme() != ruvia::HttpScheme::kHttps || responseHead == nullptr ||
