...
RUVIA_TEST(http1_request_body_plan_has_one_framing_truth) {
    ...
    const auto emptyExpectationPlan =
        emptyLength.expectationPlan(HttpUnsupportedExpectationPolicy::kReject);
    ...
    const auto compressedExpectationPlan =
...
RUVIA_TEST(request_body_gzip_bomb_rejected) {
    ...
    RUVIA_CHECK(decodeError(HttpContentCoding::kGzip, gz, 1024) ==
...
RUVIA_TEST(request_body_gzip_truncated_rejected) {
    ...
    RUVIA_CHECK(
        decodeError(HttpContentCoding::kGzip, gz) == HttpContentDecodeError::kInvalidContent);
}
...
RUVIA_TEST(request_body_gzip_decodes_every_rfc1952_member) {
    ...
    RUVIA_CHECK_EQ(decoded(HttpContentCoding::kGzip, first + second, kDecodedBodyLimit),
...
RUVIA_TEST(request_body_gzip_rejects_bytes_after_the_last_member) {
    ...
    RUVIA_CHECK(
        decodeError(HttpContentCoding::kGzip, encoded) == HttpContentDecodeError::kInvalidContent);
}
...
RUVIA_TEST(request_body_brotli_bomb_rejected) {
    ...
    RUVIA_CHECK(!br.empty());
    RUVIA_CHECK(decodeError(HttpContentCoding::kBrotli, br, 1024) ==
                HttpContentDecodeError::kDecodedSizeExceeded);
}
...
RUVIA_TEST(request_body_brotli_rejects_trailing_bytes) {
    ...
    RUVIA_CHECK(decodeError(HttpContentCoding::kBrotli, encoded) ==
...
RUVIA_TEST(request_body_zstd_bomb_rejected) {
    ...
    RUVIA_CHECK(decodeError(HttpContentCoding::kZstd, zz, 1024) ==
                HttpContentDecodeError::kDecodedSizeExceeded);
}
...
RUVIA_TEST(request_body_zstd_decodes_every_rfc8878_frame) {
    ...
    RUVIA_CHECK(!first.empty());
    RUVIA_CHECK(!second.empty());
    RUVIA_CHECK_EQ(decoded(HttpContentCoding::kZstd, first + second, kDecodedBodyLimit),
        std::string("first-second"));
}
...
RUVIA_TEST(request_body_zstd_rejects_bytes_after_the_last_frame) {
    ...
    RUVIA_CHECK(
        decodeError(HttpContentCoding::kZstd, encoded) == HttpContentDecodeError::kInvalidContent);
}
...
RUVIA_TEST(http_request_content_decoder_owns_protocol_failure_status) {
    ...
    RUVIA_CHECK_EQ(unsupported.protocolFailure()->protocolError().status(),
        ruvia::http_status::kUnsupportedMediaType);
}
