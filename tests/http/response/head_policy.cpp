...
RUVIA_TEST(response_write_plan_unifies_method_status_and_body_size) {
    ...
    const auto getPlan = ruvia::detail::httpBufferedResponseWritePlan(ruvia::HttpKnownMethod::kGet, response);
    ...
    RUVIA_CHECK(getPlan.bodyPlan().contentSemantics() == ruvia::detail::HttpResponseContentSemantics::kWithContent);
    ...
    const auto headPlan = ruvia::detail::httpBufferedResponseWritePlan(ruvia::HttpKnownMethod::kHead, response);
    RUVIA_CHECK_EQ(headPlan.responseStatus(), ruvia::http_status::kOk);
    RUVIA_CHECK(headPlan.bodyPlan().statusAllowsBody());
    RUVIA_CHECK(headPlan.bodyPlan().contentSemantics() == ruvia::detail::HttpResponseContentSemantics::kWithoutContent);
    ...
    const auto noContentPlan = ruvia::detail::httpBufferedResponseWritePlan(ruvia::HttpKnownMethod::kGet, response);
    ...
    RUVIA_CHECK(noContentPlan.bodyPlan().contentSemantics() == ruvia::detail::HttpResponseContentSemantics::kWithoutContent);
    ...
    RUVIA_CHECK(resetContentPlan.bodyPlan().contentSemantics() == ruvia::detail::HttpResponseContentSemantics::kWithContent);
    ...
    response.status(ruvia::http_status::kOk);
    const auto connectPlan = ruvia::detail::httpBufferedResponseWritePlan(ruvia::HttpKnownMethod::kConnect, response);
...
RUVIA_TEST(response_policy_normal_status_allows_everything) {
    for (const ruvia::HttpStatusCode status : {ruvia::http_status::kOk, ruvia::http_status::kPartialContent, ruvia::http_status::kNotFound, ruvia::http_status::kInternalServerError}) {
...
RUVIA_TEST(response_policy_bodyless_statuses_forbid_all_framing) {
    ...
    for (const ruvia::HttpStatusCode status : {ruvia::http_status::kContinue, ruvia::http_status::kSwitchingProtocols, ruvia::HttpStatusCode::fromValue(199), ruvia::http_status::kNoContent}) {
        const auto policy = responseWritePolicy(status);
        RUVIA_CHECK(policy.normal() == nullptr);
        RUVIA_CHECK(policy.bodyForbidden() != nullptr);
        RUVIA_CHECK(policy.zeroLength() == nullptr);
...
RUVIA_TEST(http1_response_head_framing_is_an_exclusive_plan) {
    ...
    const auto chunked = ruvia::detail::http1ChunkedResponseStreamHeadPlan(bodyPlan, connectionPlan);
    const auto closeDelimited = ruvia::detail::http1CloseDelimitedResponseStreamHeadPlan(bodyPlan, connectionPlan);
    ...
    RUVIA_CHECK(closeDelimited.bodyPlan().contentSemantics() == ruvia::detail::HttpResponseContentSemantics::kWithContent);
