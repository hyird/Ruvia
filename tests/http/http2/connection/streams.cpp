...
RUVIA_TEST(http2_connection_peer_stream_limit_waits_for_both_half_closes) {
    ...
    const auto first = client.submitRegularRequestHead("POST", "https", "example.test", "/upload", {}, Http2RequestContent::streaming());
    ...
    const auto whileOpen = client.submitRegularRequestHead("GET", "https", "example.test", "/second", {}, Http2RequestContent::none());
    RUVIA_CHECK(whileOpen.submitted() == nullptr);
    RUVIA_CHECK(requestHeadSubmitError(whileOpen) == Http2RequestHeadSubmitError::kPeerStreamLimitReached);
    ...
    const auto peerHalfOnly = client.submitRegularRequestHead("GET", "https", "example.test", "/second", {}, Http2RequestContent::none());
    RUVIA_CHECK(peerHalfOnly.submitted() == nullptr);
    RUVIA_CHECK(requestHeadSubmitError(peerHalfOnly) == Http2RequestHeadSubmitError::kPeerStreamLimitReached);
    RUVIA_CHECK(client.submitData(firstStreamId, {}, Http2EndStream::kEndStream) == Http2DataSubmitStatus::kAccepted);
    client.consumeOutput(client.pendingOutput().size());

    const auto afterBothHalves = client.submitRegularRequestHead("GET", "https", "example.test", "/second", {}, Http2RequestContent::none());
...
RUVIA_TEST(http2_connection_peer_reset_releases_peer_stream_limit_slot) {
    ...
    const auto firstStreamId = submittedRequestStreamId(first);
    client.consumeOutput(client.pendingOutput().size());
    RUVIA_CHECK(requestHeadSubmitError(client.submitRegularRequestHead("GET", "https", "example.test", "/second", {}, Http2RequestContent::none())) == Http2RequestHeadSubmitError::kPeerStreamLimitReached);
    ...
    RUVIA_CHECK(client.feed(std::string_view(reset, sizeof(reset))) == ruvia::detail::Http2FeedResult::kAccepted);

    const auto afterReset = client.submitRegularRequestHead("GET", "https", "example.test", "/second", {}, Http2RequestContent::none());
...
RUVIA_TEST(http2_connection_feed_priority_payload_is_ignored) {
    ...
    live[13] = 0;
    RUVIA_CHECK(conn.feed(std::string_view(live, sizeof(live))) == ruvia::detail::Http2FeedResult::kAccepted);
    ...
    idle[13] = 0;
    RUVIA_CHECK(conn.feed(std::string_view(idle, sizeof(idle))) == ruvia::detail::Http2FeedResult::kAccepted);
    RUVIA_CHECK(!conn.connectionError().has_value());
...
RUVIA_TEST(http2_connection_malformed_priority_is_stream_frame_size_error) {
    ...
    RUVIA_CHECK(conn.feed(std::string_view(malformed, sizeof(malformed))) == Http2FeedResult::kAccepted);
    RUVIA_CHECK(!conn.connectionError().has_value());
    ...
    RUVIA_CHECK_EQ(ruvia::detail::http2Read32(reinterpret_cast<const unsigned char*>(out.data() + 9)), static_cast<std::uint32_t>(Http2ErrorCode::kFrameSizeError));
...
RUVIA_TEST(http2_connection_malformed_idle_priority_is_connection_error) {
    ...
    malformed[9] = 0;
    RUVIA_CHECK(conn.feed(std::string_view(malformed, sizeof(malformed))) == Http2FeedResult::kProtocolFailure);
    ...
    RUVIA_CHECK_EQ(ruvia::detail::http2Read32(reinterpret_cast<const unsigned char*>(out.data() + 13)), static_cast<std::uint32_t>(Http2ErrorCode::kFrameSizeError));
}
...
RUVIA_TEST(http2_connection_rejects_non_increasing_new_peer_stream_id) {
    ...
    const auto first = headersFrame(&resource, 5, ruvia::detail::kHttp2FlagEndHeaders | ruvia::detail::kHttp2FlagEndStream, std::string_view(firstBlock.data(), firstBlock.size()));
    ...
    const auto lower = headersFrame(&resource, 3, ruvia::detail::kHttp2FlagEndHeaders | ruvia::detail::kHttp2FlagEndStream, std::string_view(lowerBlock.data(), lowerBlock.size()));
    RUVIA_CHECK(conn.feed(std::string_view(lower.data(), lower.size())) == Http2FeedResult::kProtocolFailure);
    RUVIA_CHECK(conn.connectionError() == Http2ErrorCode::kProtocolError);
}
...
RUVIA_TEST(http2_connection_client_reset_detaches_partial_response_head) {
    ...
    const auto firstHead = client.submitRegularRequestHead("GET", "https", "example.test", "/", {}, Http2RequestContent::none());
    ...
    const auto first = headersFrame(&resource, firstStream, 0, std::string_view(responseBlock.data(), statusBytes));
    RUVIA_CHECK(client.feed(std::string_view(first.data(), first.size())) == ruvia::detail::Http2FeedResult::kAccepted);
    RUVIA_CHECK(client.headerBlockInProgress());

    RUVIA_CHECK(client.submitReset(firstStream, Http2ErrorCode::kCancel) == Http2SubmitStatus::kAccepted);
    RUVIA_CHECK(client.stream(firstStream) == nullptr);
    client.consumeOutput(client.pendingOutput().size());

    const auto continuation = continuationFrame(&resource, firstStream, ruvia::detail::kHttp2FlagEndHeaders, std::string_view(responseBlock.data() + statusBytes, responseBlock.size() - statusBytes));
    RUVIA_CHECK(client.feed(std::string_view(continuation.data(), continuation.size())) == ruvia::detail::Http2FeedResult::kAccepted);
    ...
    const auto nextHead = client.submitRegularRequestHead("GET", "https", "example.test", "/next", {}, Http2RequestContent::none());
    ...
    const auto final = headersFrame(&resource, nextStream, ruvia::detail::kHttp2FlagEndHeaders | ruvia::detail::kHttp2FlagEndStream, std::string_view(nextResponse.data(), nextResponse.size()));
    RUVIA_CHECK(client.feed(std::string_view(final.data(), final.size())) == ruvia::detail::Http2FeedResult::kAccepted);
...
RUVIA_TEST(http2_connection_pinned_stream_survives_peer_reset) {
    ...
    while (const auto event = conn.nextEvent()) {
        if (const auto* closed = event->streamClosed(); closed != nullptr && closed->streamId() == 1) {
            sawClosed = true;
...
RUVIA_TEST(http2_connection_client_rejects_reset_content_payload) {
    ...
    RUVIA_CHECK(client.feed(std::string_view(forbidden.data(), forbidden.size())) == Http2FeedResult::kAccepted);
    ...
    RUVIA_CHECK_EQ(ruvia::detail::http2Read32(reinterpret_cast<const unsigned char*>(reset.data() + 9)), static_cast<std::uint32_t>(Http2ErrorCode::kProtocolError));
...
RUVIA_TEST(http2_connection_client_accepts_empty_reset_content_terminal) {
    ...
    RUVIA_CHECK(client.feed(std::string_view(terminal.data(), terminal.size())) == Http2FeedResult::kAccepted);
...
RUVIA_TEST(http2_connection_client_role_rejects_unexpected_streams) {
    std::pmr::monotonic_buffer_resource resource;
    {
        ...
        const auto idle = headersFrame(&resource, 5, ruvia::detail::kHttp2FlagEndHeaders, std::string_view(head.data(), head.size()));
    ...
    }
    {
        ...
        std::pmr::string head(&resource);
        HpackEncoder::encodeHeader(head, ":status", "200");
        const auto even = headersFrame(&resource, 2, ruvia::detail::kHttp2FlagEndHeaders, std::string_view(head.data(), head.size()));
...
RUVIA_TEST(http2_connection_repeated_peer_reset_is_connection_error) {
    for (const bool pinned : {false, true}) {
        ...
        RUVIA_CHECK(conn.feed(std::string_view(rst, sizeof(rst))) == ruvia::detail::Http2FeedResult::kProtocolFailure);
        RUVIA_CHECK(conn.connectionError() == Http2ErrorCode::kStreamClosed);
        RUVIA_CHECK_EQ(firstGoawayError(conn.pendingOutput()), static_cast<std::uint32_t>(Http2ErrorCode::kStreamClosed));
...
RUVIA_TEST(http2_connection_malformed_priority_after_peer_reset_is_connection_error) {
    for (const bool pinned : {false, true}) {
        ...
        RUVIA_CHECK(conn.feed(std::string_view(priority, sizeof(priority))) == ruvia::detail::Http2FeedResult::kProtocolFailure);
        RUVIA_CHECK(conn.connectionError() == Http2ErrorCode::kFrameSizeError);
        RUVIA_CHECK_EQ(firstGoawayError(conn.pendingOutput()), static_cast<std::uint32_t>(Http2ErrorCode::kFrameSizeError));
...
RUVIA_TEST(http2_connection_rapid_reset_flood_trips_enhance_your_calm) {
    ...
    for (std::uint32_t sid = 1; sid < 1U + 2U * 1200U; sid += 2) {
        const auto h = headersFrame(&resource, sid, ruvia::detail::kHttp2FlagEndHeaders | ruvia::detail::kHttp2FlagEndStream, std::string_view(block.data(), block.size()));
        if (conn.feed(std::string_view(h.data(), h.size())) == ruvia::detail::Http2FeedResult::kProtocolFailure) {
            tripped = true;
            break;
        }
        while (conn.nextEvent().has_value()) {
        ...
        if (conn.feed(std::string_view(rst, sizeof(rst))) == ruvia::detail::Http2FeedResult::kProtocolFailure) {
            tripped = true;
