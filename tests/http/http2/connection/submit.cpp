...
RUVIA_TEST(http2_connection_request_head_requires_started_preface_without_consuming_id) {
    ...
    const auto beforeStart = client.submitRegularRequestHead("GET", "https", "example.test", "/", {}, Http2RequestContent::none());
    RUVIA_CHECK(beforeStart.submitted() == nullptr);
    RUVIA_CHECK(requestHeadSubmitError(beforeStart) == Http2RequestHeadSubmitError::kConnectionNotStarted);
...
RUVIA_TEST(http2_connection_websocket_handshake_clears_staged_block_on_encoding_failure) {
    ...
    auto negotiation = ruvia::detail::makeWebSocketServerNegotiation(negotiationRequest.request, {.supportedSubprotocols = supportedProtocols, .resource = &resource});
    ...
    resource.rejectAllocations(false);
    auto retryNegotiation = ruvia::detail::makeWebSocketServerNegotiation(negotiationRequest.request, {.supportedSubprotocols = supportedProtocols, .resource = &resource});
...
RUVIA_TEST(http2_connection_buffered_response_length_is_transactional) {
    ...
    RUVIA_CHECK(conn.finishResponse(1, validatedTrailers({})) == Http2FinishSubmitStatus::kContentLengthIncomplete);
    RUVIA_CHECK(conn.submitData(1, "four", Http2EndStream::kEndStream) == Http2DataSubmitStatus::kContentLengthIncomplete);
    RUVIA_CHECK(conn.submitData(1, "sixsix", Http2EndStream::kKeepOpen) == Http2DataSubmitStatus::kContentLengthExceeded);
    RUVIA_CHECK(conn.pendingOutput().empty());
    RUVIA_CHECK_EQ(stream->localContent().acceptedBytes(), std::uint64_t{0});
    RUVIA_CHECK_EQ(stream->localContent().committedBytes(), std::uint64_t{0});
    RUVIA_CHECK(stream->localSend().responseContentOpen() != nullptr);

    RUVIA_CHECK(conn.submitData(1, "he", Http2EndStream::kKeepOpen) == Http2DataSubmitStatus::kAccepted);
    ...
    RUVIA_CHECK(conn.submitData(1, "llo", Http2EndStream::kEndStream) == Http2DataSubmitStatus::kAccepted);
...
RUVIA_TEST(http2_connection_response_head_submit_result_is_discriminated) {
    ...
    const auto streamingSubmitted = streaming.submitStreamingResponseHead(1, std::move(streamingHead), ruvia::detail::ResponseStreamKind::kGeneric, ResponseTrailerIntent::kNone);
...
RUVIA_TEST(http2_connection_buffered_response_requires_matching_prepared_plan) {
    ...
    const auto wrongMethodPlan = ruvia::detail::httpBufferedResponseWritePlan(ruvia::HttpKnownMethod::kHead, response);
    const auto wrongMethod = connection.submitResponseHead(1, response, wrongMethodPlan);
    RUVIA_CHECK(responseHeadSubmitFailureMessage(wrongMethod) == "HTTP/2 response head does not match its write plan");
    RUVIA_CHECK(connection.pendingOutput().empty());

    const auto staleRepresentationPlan = ruvia::detail::httpBufferedResponseWritePlan(ruvia::HttpKnownMethod::kGet, response);
    response.body("longer");
    const auto staleRepresentation = connection.submitResponseHead(1, response, staleRepresentationPlan);
    RUVIA_CHECK(responseHeadSubmitFailureMessage(staleRepresentation) == "HTTP/2 response head does not match its write plan");
    RUVIA_CHECK(connection.pendingOutput().empty());

    const auto staleStatusPlan = ruvia::detail::httpBufferedResponseWritePlan(ruvia::HttpKnownMethod::kGet, response);
    ...
    RUVIA_CHECK(responseHeadSubmitFailureMessage(staleStatus) == "HTTP/2 response head does not match its write plan");
    RUVIA_CHECK(connection.pendingOutput().empty());

    const auto submitted = connection.submitResponseHead(1, response, ruvia::detail::httpBufferedResponseWritePlan(ruvia::HttpKnownMethod::kGet, response));
...
RUVIA_TEST(http2_connection_rejects_duplicate_response_head_without_output) {
    ...
    const auto firstResult = conn.submitStreamingResponseHead(1, std::move(first), ruvia::detail::ResponseStreamKind::kGeneric, ResponseTrailerIntent::kNone);
    ...
    RUVIA_CHECK(responseHeadSubmitFailureMessage(duplicateResult) == "invalid HTTP/2 response head submission state");
    RUVIA_CHECK(conn.pendingOutput().empty());
}
...
RUVIA_TEST(http2_connection_rejects_head_api_for_wrong_role) {
    ...
    RUVIA_CHECK(requestHeadSubmitError(server.submitRegularRequestHead("GET", "https", "example.test", "/", {}, Http2RequestContent::none())) == Http2RequestHeadSubmitError::kInvalidState);
    ...
    RUVIA_CHECK(responseHeadSubmitFailureMessage(result) == "invalid HTTP/2 response head submission state");
    RUVIA_CHECK(client.pendingOutput().empty());
}
...
RUVIA_TEST(http2_connection_request_content_alternatives_own_wire_framing) {
    ...
    const auto check = [&resource, &ruvia_ctx](std::string_view method, Http2RequestContent content, bool expectEndStream, std::string_view expectedContentLength, auto&& verifyLocalContent) {
        Http2Connection client(&resource, ruvia::detail::Http2Role::kClient);
        beginClient(client);
        const auto submit = client.submitRegularRequestHead(method, "https", "example.test", "/upload", {}, content);
        ...
        const auto decodeResult = decoder.decode(out.substr(9, frame.length), &observation, &observeRequestContentLength);
    ...
    check("GET", Http2RequestContent::none(), true, {}, [&ruvia_ctx](const Http2LocalContentState& localContent) {
        RUVIA_CHECK(localContent.forbidden() != nullptr);
        RUVIA_CHECK(localContent.knownLength() == nullptr);
    });
    check("POST", Http2RequestContent::knownLength(0), true, "0", [&ruvia_ctx](const Http2LocalContentState& localContent) {
    ...
    check("POST", Http2RequestContent::knownLength(5), false, "5", [&ruvia_ctx](const Http2LocalContentState& localContent) {
        const auto* knownLength = localContent.knownLength();
        RUVIA_CHECK(knownLength != nullptr);
        if (knownLength != nullptr) {
    ...
    check("POST", Http2RequestContent::streaming(), false, {}, [&ruvia_ctx](const Http2LocalContentState& localContent) {
...
RUVIA_TEST(http2_connection_enforces_request_method_content_semantics_transactionally) {
    ...
    const auto checkRejected = [&resource, &ruvia_ctx](std::string_view method, std::span<const ruvia::HttpHeaderView> headers, Http2RequestContent content) {
        Http2Connection client(&resource, ruvia::detail::Http2Role::kClient);
        beginClient(client);

        const auto rejected = client.submitRegularRequestHead(method, "https", "example.test", "/diagnostics", headers, content);
    ...
    const auto trace = traceClient.submitRegularRequestHead("TRACE", "https", "example.test", "/diagnostics", {}, Http2RequestContent::none());
    ...
    const auto accepted = client.submitRegularRequestHead("OPTIONS", "https", "example.test", "/diagnostics", contentType, Http2RequestContent::knownLength(1));
    RUVIA_CHECK(accepted.submitted() != nullptr);
    RUVIA_CHECK_EQ(submittedRequestStreamId(accepted), std::uint32_t{1});
}
...
RUVIA_TEST(http2_connection_rejects_repeated_websocket_identity_and_user_agent_fields_transactionally) {
    ...
    for (const auto& test : cases) {
        ...
        const auto rejected = client.submitRegularRequestHead("GET", "https", "example.test", "/resource", headers, Http2RequestContent::none());
        RUVIA_CHECK(rejected.submitted() == nullptr);
        RUVIA_CHECK(requestHeadSubmitError(rejected) == Http2RequestHeadSubmitError::kInvalidMessage);
        RUVIA_CHECK(client.pendingOutput().empty());
        RUVIA_CHECK(client.stream(1) == nullptr);
...
RUVIA_TEST(http2_connection_rejects_oversized_outbound_request_heads_transactionally) {
    ...
    const auto checkRejected = [&resource, &ruvia_ctx](std::string_view path, std::span<const ruvia::HttpHeaderView> headers, Http2RequestContent content) {
        Http2Connection client(&resource, ruvia::detail::Http2Role::kClient);
        beginClient(client);
        const auto rejected = client.submitRegularRequestHead("GET", "https", "example.test", path, headers, content);
        RUVIA_CHECK(rejected.submitted() == nullptr);
        RUVIA_CHECK(requestHeadSubmitError(rejected) == Http2RequestHeadSubmitError::kInvalidMessage);
        RUVIA_CHECK(client.pendingOutput().empty());
        RUVIA_CHECK(client.stream(1) == nullptr);
    ...
    const auto rejectedConnect = connectClient.submitConnectRequestHead("example.test:443", oversized);
...
RUVIA_TEST(http2_connection_rejects_oversized_response_heads_transactionally) {
    ...
    const auto checkRejected = [&resource, &ruvia_ctx](ruvia::HttpResponse response) {
        ...
        RUVIA_CHECK(rejected.submitted() == nullptr);
        RUVIA_CHECK(responseHeadSubmitFailureMessage(rejected) == "invalid HTTP/2 response head message");
        RUVIA_CHECK(connection.pendingOutput().empty());
...
RUVIA_TEST(http2_connection_rejects_raw_request_content_length_transactionally) {
    std::pmr::monotonic_buffer_resource resource;
    const auto checkRejected = [&resource, &ruvia_ctx](std::span<const ruvia::HttpHeaderView> headers) {
        Http2Connection client(&resource, ruvia::detail::Http2Role::kClient);
        beginClient(client);
        const auto rejected = client.submitRegularRequestHead("POST", "https", "example.test", "/upload", headers, Http2RequestContent::knownLength(5));
        RUVIA_CHECK(rejected.submitted() == nullptr);
        RUVIA_CHECK(requestHeadSubmitError(rejected) == Http2RequestHeadSubmitError::kInvalidMessage);
        RUVIA_CHECK(client.pendingOutput().empty());
        RUVIA_CHECK(client.stream(1) == nullptr);

        const auto accepted = client.submitRegularRequestHead("POST", "https", "example.test", "/upload", {}, Http2RequestContent::knownLength(5));
        RUVIA_CHECK(accepted.submitted() != nullptr);
        RUVIA_CHECK_EQ(submittedRequestStreamId(accepted), std::uint32_t{1});
        RUVIA_CHECK(!client.pendingOutput().empty());
    };
...
RUVIA_TEST(http2_connection_encodes_non_http_request_without_authority) {
    ...
    const auto submitted = client.submitRegularRequestHead("GET", "git+ssh", std::nullopt, "", {}, Http2RequestContent::none());
    RUVIA_CHECK(submitted.submitted() != nullptr);
    ...
    RequestContentLengthObservation observation;
    HpackDecoder decoder({.resource = &resource});
    const auto decoded = decoder.decode(out.substr(9, frame.length), &observation, &observeRequestContentLength);
...
RUVIA_TEST(http2_connection_encodes_non_http_userinfo_authority) {
    ...
    const auto submitted = client.submitRegularRequestHead("GET", "git+ssh", "deploy:secret@example.test:9418", "/repository", {}, Http2RequestContent::none());
    RUVIA_CHECK(submitted.submitted() != nullptr);
    RUVIA_CHECK_EQ(submittedRequestStreamId(submitted), std::uint32_t{1});
...
RUVIA_TEST(http2_connection_encodes_options_asterisk_path) {
    ...
    const auto submitted = client.submitRegularRequestHead("OPTIONS", "https", std::nullopt, "*", {}, Http2RequestContent::none());
    RUVIA_CHECK(submitted.submitted() != nullptr);
    RUVIA_CHECK_EQ(submittedRequestStreamId(submitted), std::uint32_t{1});
    const auto out = client.pendingOutput();
    const auto frame = ruvia::detail::http2ParseFrameHeader(out.substr(0, 9));
...
RUVIA_TEST(http2_connection_exposes_negotiated_extended_connect_capability) {
    ...
    RUVIA_CHECK(client.feed(std::string_view(settings, sizeof(settings))) == ruvia::detail::Http2FeedResult::kAccepted);
    RUVIA_CHECK(client.peerExtendedConnectEnabled());

    Http2Connection server(&resource);
    beginPeerInput(server);
    RUVIA_CHECK(server.feed(std::string_view(settings, sizeof(settings))) == ruvia::detail::Http2FeedResult::kAccepted);
...
RUVIA_TEST(http2_connection_request_known_length_is_exact_and_transactional) {
    ...
    Http2Connection client(&resource, ruvia::detail::Http2Role::kClient);
    beginClient(client);
    const auto request = client.submitRegularRequestHead("POST", "https", "example.test", "/upload", {}, Http2RequestContent::knownLength(5));
    RUVIA_CHECK(request.submitted() != nullptr);
    const auto streamId = submittedRequestStreamId(request);
    client.consumeOutput(client.pendingOutput().size());
    ...
    RUVIA_CHECK(stream != nullptr);
    RUVIA_CHECK(client.submitData(streamId, "four", Http2EndStream::kEndStream) == Http2DataSubmitStatus::kContentLengthIncomplete);
    RUVIA_CHECK(client.submitData(streamId, "sixsix", Http2EndStream::kKeepOpen) == Http2DataSubmitStatus::kContentLengthExceeded);
    RUVIA_CHECK(client.pendingOutput().empty());
    RUVIA_CHECK_EQ(stream->localContent().acceptedBytes(), std::uint64_t{0});
    RUVIA_CHECK_EQ(stream->localContent().committedBytes(), std::uint64_t{0});
    RUVIA_CHECK(stream->localSend().requestContentOpen() != nullptr);

    RUVIA_CHECK(client.submitData(streamId, "he", Http2EndStream::kKeepOpen) == Http2DataSubmitStatus::kAccepted);
    ...
    RUVIA_CHECK(client.submitData(streamId, "llo", Http2EndStream::kEndStream) == Http2DataSubmitStatus::kAccepted);
    ...
    RUVIA_CHECK(client.submitData(streamId, "again", Http2EndStream::kEndStream) == Http2DataSubmitStatus::kInvalidState);
    RUVIA_CHECK(client.pendingOutput().empty());
}
...
RUVIA_TEST(http2_connection_request_streaming_content_has_no_length_contract) {
    ...
    RUVIA_CHECK(client.submitData(streamId, "chunk-a", Http2EndStream::kKeepOpen) == Http2DataSubmitStatus::kAccepted);
    client.consumeOutput(client.pendingOutput().size());
    RUVIA_CHECK(client.submitData(streamId, "chunk-b", Http2EndStream::kEndStream) == Http2DataSubmitStatus::kAccepted);
    const auto out = client.pendingOutput();
...
RUVIA_TEST(http2_connection_interim_head_preserves_final_head_phase) {
    ...
    const ruvia::HttpInterimResponseHead invalidEarlyHints(ruvia::http_status::kEarlyHints, invalidHeaders);
    RUVIA_CHECK(conn.submitInterimResponseHead(1, invalidEarlyHints) == Http2SubmitStatus::kInvalidMessage);
    RUVIA_CHECK(conn.pendingOutput().empty());

    const ruvia::HttpHeaderView earlyHintHeaders[] = {
        {"Link", "</style.css>; rel=preload"},
    };
    const ruvia::HttpInterimResponseHead earlyHints(ruvia::http_status::kEarlyHints, earlyHintHeaders);
...
RUVIA_TEST(http2_connection_submit_streaming_response_head_and_chunks) {
    ...
    const auto headResult = conn.submitStreamingResponseHead(1, std::move(resp), ruvia::detail::ResponseStreamKind::kGeneric, ResponseTrailerIntent::kNone);
    ...
    RUVIA_CHECK(conn.submitData(1, "chunk1", Http2EndStream::kKeepOpen) == Http2DataSubmitStatus::kAccepted);
    RUVIA_CHECK(conn.submitData(1, "chunk2", Http2EndStream::kEndStream) == Http2DataSubmitStatus::kAccepted);
...
RUVIA_TEST(http2_connection_streaming_rejects_invalid_content_length_before_head) {
    ...
    for (const std::string_view invalid : {std::string_view{"x"}, std::string_view{"-1"}, std::string_view{"5,5"}, std::string_view{"18446744073709551616"}}) {
        ruvia::HttpResponse response({.resource = &resource});
        response.status(ruvia::http_status::kOk);
        response.header("Content-Length", invalid);
        const auto result = conn.submitStreamingResponseHead(1, std::move(response), ruvia::detail::ResponseStreamKind::kGeneric, ResponseTrailerIntent::kNone);
    ...
    RUVIA_CHECK(responseHeadSubmitted(conn.submitStreamingResponseHead(1, std::move(valid), ruvia::detail::ResponseStreamKind::kGeneric, ResponseTrailerIntent::kNone)));
    auto* stream = conn.stream(1);
    RUVIA_CHECK(stream != nullptr);
    RUVIA_CHECK_EQ(requireLocalKnownLength(*stream).declaredLength(), std::uint64_t{5});
}
...
RUVIA_TEST(http2_connection_client_role_surfaces_early_hints_separately) {
    ...
    std::pmr::string bytes(&resource);
    {
        ...
        const auto interimFrame = headersFrame(&resource, streamId, ruvia::detail::kHttp2FlagEndHeaders, std::string_view(interim.data(), interim.size()));
        ...
        const auto finalFrame = headersFrame(&resource, streamId, ruvia::detail::kHttp2FlagEndHeaders, std::string_view(final_.data(), final_.size()));
        bytes.append(finalFrame.data(), finalFrame.size());
        char data[9 + 2];
        ruvia::detail::http2EncodeFrameHeader(data, 2, Http2FrameType::kData, ruvia::detail::kHttp2FlagEndStream, streamId);
...
RUVIA_TEST(http2_connection_client_rejects_te_in_every_response_head) {
    ...
    for (const auto status : statuses) {
        ...
        const auto responseHead = headersFrame(&resource, streamId, static_cast<std::uint8_t>(ruvia::detail::kHttp2FlagEndHeaders | (status == "200" ? ruvia::detail::kHttp2FlagEndStream : 0)), std::string_view(response.data(), response.size()));
