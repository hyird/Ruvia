...
void appendFrame(std::pmr::string& wire, ruvia::Http2FrameType type, std::uint8_t flags, std::uint32_t streamId, std::string_view payload) {
    std::array<char, ruvia::kHttp2FrameHeaderBytes> header{};
    if (!ruvia::encodeHttp2FrameHeader(header, static_cast<std::uint32_t>(payload.size()), type, flags, streamId)) {
...
std::pmr::string clientResponseWire(std::pmr::memory_resource* resource, std::string_view body, bool includeHeader = false, bool endStream = true) {
...
std::pmr::string clientWindowThresholdResponseWire(std::pmr::memory_resource* resource, bool endStream) {
    ...
    constexpr auto kFrameCount = ruvia::detail::kHttp2ReceiveWindowUpdateThreshold / kFramePayloadBytes;
    std::pmr::string block(resource);
    ruvia::HpackEncoder::encodeStatus(block, ruvia::http_status::kOk);
    ...
    for (std::size_t index = 0; index < kFrameCount; ++index) {
        const auto flags = static_cast<std::uint8_t>(endStream && index + 1 == kFrameCount ? 0x1U : 0U);
...
ruvia::Http2Connection preparedClient(std::pmr::memory_resource* resource) {
    ...
    const auto submitted = client.submitRequestHead(ruvia::Http2RegularRequestHeadView{.method = "GET", .scheme = "https", .authority = "example.test", .target = "/"});
...
RUVIA_TEST(http2_public_client_terminal_event_preserves_unacknowledged_data_credit) {
    ...
    RUVIA_CHECK(client.acknowledge(std::move(credit)) == ruvia::Http2ReceivedDataAcknowledgeStatus::kInvalidCredit);
}
...
RUVIA_TEST(http2_public_dropped_data_credit_returns_debt_and_releases_closed_stream) {
    ...
    RUVIA_CHECK(client.submitReset(1, ruvia::Http2ErrorCode::kCancel) == ruvia::Http2SubmitStatus::kClosed);
}
...
RUVIA_TEST(http2_public_terminal_waits_for_exact_credit_before_window_update) {
    ...
    for (std::size_t index = 0; index + 1 < credits.size(); ++index) {
        RUVIA_CHECK(client.acknowledge(std::move(credits[index])) == ruvia::Http2ReceivedDataAcknowledgeStatus::kAcknowledged);
        RUVIA_CHECK(client.pendingOutput().empty());
    }
    ...
    const auto update = ruvia::parseHttp2FrameHeader(std::span<const char>(output.data(), output.size()));
    RUVIA_CHECK(update.has_value());
    RUVIA_CHECK(update && update->type == static_cast<std::uint8_t>(ruvia::Http2FrameType::kWindowUpdate));
...
RUVIA_TEST(http2_public_client_reset_preserves_outstanding_data_credit) {
    ...
    RUVIA_CHECK(client.submitReset(1, ruvia::Http2ErrorCode::kCancel) == ruvia::Http2SubmitStatus::kAccepted);
    RUVIA_CHECK(client.acknowledge(std::move(credit)) == ruvia::Http2ReceivedDataAcknowledgeStatus::kAcknowledged);
}
...
RUVIA_TEST(http2_public_peer_reset_preserves_outstanding_data_credit) {
    ...
    appendFrame(wire, ruvia::Http2FrameType::kRstStream, 0, 1, std::string_view(kCancelPayload.data(), kCancelPayload.size()));
    RUVIA_CHECK(client.feed(wire) == ruvia::Http2FeedResult::kAccepted);
...
RUVIA_TEST(http2_public_dropped_credit_retries_failed_window_update_once) {
    ...
    const auto connectionUpdate = ruvia::parseHttp2FrameHeader(std::span<const char>(output.data(), output.size()));
    const auto streamOffset = ruvia::kHttp2FrameHeaderBytes + 4;
    const auto streamUpdate = ruvia::parseHttp2FrameHeader(std::span<const char>(output.data() + streamOffset, output.size() - streamOffset));
    RUVIA_CHECK(connectionUpdate && connectionUpdate->type == static_cast<std::uint8_t>(ruvia::Http2FrameType::kWindowUpdate));
    RUVIA_CHECK(connectionUpdate && connectionUpdate->streamId == 0);
    RUVIA_CHECK(streamUpdate && streamUpdate->type == static_cast<std::uint8_t>(ruvia::Http2FrameType::kWindowUpdate));
...
RUVIA_TEST(http2_public_server_release_preserves_outstanding_data_credit) {
    ...
    RUVIA_CHECK(server.release(std::move(*requestHead)) == ruvia::Http2ServerRequestReleaseStatus::kReleased);
    RUVIA_CHECK(server.acknowledge(std::move(credit)) == ruvia::Http2ReceivedDataAcknowledgeStatus::kAcknowledged);
}
...
RUVIA_TEST(http2_public_dropped_request_retries_failed_abandonment) {
    ...
    const auto reset = ruvia::parseHttp2FrameHeader(std::span<const char>(output.data() + kSettingsAckBytes, output.size() - kSettingsAckBytes));
    RUVIA_CHECK(reset && reset->type == static_cast<std::uint8_t>(ruvia::Http2FrameType::kRstStream));
...
RUVIA_TEST(http2_public_request_endpoint_survives_connection_destruction_without_aba) {
    ...
    RUVIA_CHECK(other.release(std::move(*escaped->requestHead())) == ruvia::Http2ServerRequestReleaseStatus::kInvalidLease);
    escaped.reset();
}
...
RUVIA_TEST(http2_public_data_credit_endpoint_survives_connection_destruction_without_aba) {
    ...
    RUVIA_CHECK(other.acknowledge(std::move(escapedCredit)) == ruvia::Http2ReceivedDataAcknowledgeStatus::kInvalidCredit);
    RUVIA_CHECK(escapedCredit.valid());
}
