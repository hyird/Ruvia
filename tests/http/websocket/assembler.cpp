...
concept AcceptsWebSocketMessagePayload = requires(Payload&& payload) { WebSocketMessageAccess::make(WebSocketOpcode::kText, std::forward<Payload>(payload)); };
...
WebSocketFrameView frame(WebSocketOpcode opcode, std::string_view payload, bool fin, bool continuation = false, bool rsv1 = false) {
    if (continuation) {
...
template <typename T>
concept HasAnyRvalueInboundAccessor = requires(T&& result) { std::move(result).continueReading(); } || requires(T&& result) { std::move(result).controlFrame(); } || requires(T&& result) { std::move(result).message(); } || requires(T&& result) { std::move(result).failure(); };
...
static_assert(std::same_as<decltype(std::declval<const WebSocketInboundResult&>().continueReading()), const ruvia::detail::WebSocketInboundContinue*>);
static_assert(std::same_as<decltype(std::declval<const WebSocketInboundResult&>().controlFrame()), const ruvia::detail::WebSocketInboundControlFrame*>);
static_assert(std::same_as<decltype(std::declval<const WebSocketInboundResult&>().message()), const ruvia::detail::WebSocketInboundMessage*>);
static_assert(std::same_as<decltype(std::declval<const WebSocketInboundResult&>().failure()), const ruvia::detail::WebSocketInboundFailure*>);
...
static_assert(std::same_as<decltype(std::declval<const ruvia::detail::WebSocketInboundFragmented&>().opcode()), WebSocketOpcode>);
static_assert(std::same_as<decltype(std::declval<const ruvia::detail::WebSocketInboundFragmented&>().encoding()), WebSocketInboundContentEncoding>);
...
RUVIA_TEST(ws_assembler_single_frame_messages) {
    WebSocketInboundAssembler assembler(std::pmr::get_default_resource());
    const auto text = assembler.accept(frame(WebSocketOpcode::kText, "hello", true), byteLimit(1000));
    ...
    const auto binaryResult = assembler.accept(frame(WebSocketOpcode::kBinary, binary, true), byteLimit(1000));
    ...
    const auto compressed = assembler.accept(frame(WebSocketOpcode::kText, "z", true, false, true), byteLimit(1000));
    RUVIA_CHECK(compressed.message() != nullptr);
    RUVIA_CHECK(compressed.message()->contentEncoding() == WebSocketInboundContentEncoding::kPerMessageDeflate);
}

RUVIA_TEST(ws_assembler_invalid_utf8_text) {
    WebSocketInboundAssembler assembler(std::pmr::get_default_resource());
    const std::string overlong("\xc0\x80", 2);  // overlong encoding of NUL
    const auto result = assembler.accept(frame(WebSocketOpcode::kText, overlong, true), byteLimit(1000));
...
RUVIA_TEST(ws_assembler_fragmented_message) {
    WebSocketInboundAssembler assembler(std::pmr::get_default_resource());
    const auto first = assembler.accept(frame(WebSocketOpcode::kText, "hel", false), byteLimit(1000));
    RUVIA_CHECK(first.continueReading() != nullptr);
    const auto second = assembler.accept(frame(WebSocketOpcode::kText, "lo ", false, true), byteLimit(1000));
...
RUVIA_TEST(ws_assembler_control_frame_interleaved_in_fragments) {
    ...
    const auto complete = assembler.accept(frame(WebSocketOpcode::kText, "lo", true, true), byteLimit(1000));
    RUVIA_CHECK(complete.message() != nullptr);
...
RUVIA_TEST(ws_assembler_fragmented_compressed_defers_validation) {
    ...
    WebSocketInboundAssembler assembler(std::pmr::get_default_resource());
    const auto first = assembler.accept(frame(WebSocketOpcode::kText, std::string_view("\x01\x02", 2), false, false, true), byteLimit(1000));
    RUVIA_CHECK(first.continueReading() != nullptr);
    const auto complete = assembler.accept(frame(WebSocketOpcode::kText, std::string_view("\x03", 1), true, true), byteLimit(1000));
    RUVIA_CHECK(complete.message() != nullptr);
    RUVIA_CHECK(complete.message()->contentEncoding() == WebSocketInboundContentEncoding::kPerMessageDeflate);
...
RUVIA_TEST(ws_assembler_protocol_errors) {
    ...
    const auto noStartResult = noStart.accept(frame(WebSocketOpcode::kText, "x", true, true), byteLimit(1000));
    ...
    const auto started = interleaved.accept(frame(WebSocketOpcode::kText, "start", false), byteLimit(1000));
    RUVIA_CHECK(started.continueReading() != nullptr);
    const auto interleavedResult = interleaved.accept(frame(WebSocketOpcode::kText, "new", true), byteLimit(1000));
    ...
    const auto withinLimit = tooBig.accept(frame(WebSocketOpcode::kText, "12345", false), byteLimit(10));
    RUVIA_CHECK(withinLimit.continueReading() != nullptr);
    const auto tooBigResult = tooBig.accept(frame(WebSocketOpcode::kText, "678901", true, true), byteLimit(10));
    ...
    const auto firstFrameResult = firstFrameTooBig.accept(frame(WebSocketOpcode::kBinary, "123456", false), byteLimit(5));
...
RUVIA_TEST(ws_assembler_violations_carry_rfc_close_code) {
    ...
    WebSocketInboundAssembler noStart(std::pmr::get_default_resource());
    RUVIA_CHECK_EQ(acceptCloseCode(noStart, frame(WebSocketOpcode::kText, "x", true, true), byteLimit(1000)), std::uint16_t{1002});  // continuation with no message open

    WebSocketInboundAssembler interleaved(std::pmr::get_default_resource());
    (void)interleaved.accept(frame(WebSocketOpcode::kText, "start", false), byteLimit(1000));
    RUVIA_CHECK_EQ(acceptCloseCode(interleaved, frame(WebSocketOpcode::kText, "new", true), byteLimit(1000)), std::uint16_t{1002});  // interleaved non-continuation data frame

    WebSocketInboundAssembler tooBig(std::pmr::get_default_resource());
    (void)tooBig.accept(frame(WebSocketOpcode::kText, "12345", false), byteLimit(10));
    RUVIA_CHECK_EQ(acceptCloseCode(tooBig, frame(WebSocketOpcode::kText, "678901", true, true), byteLimit(10)), std::uint16_t{1009});  // per-message size limit exceeded
}
