...
[[nodiscard]] std::uint64_t parseMilliseconds(std::string_view argument, std::string_view option) {
    std::uint64_t value = 0;
    const auto* begin = argument.data();
    const auto* end = begin + argument.size();
    const auto parsed = std::from_chars(begin, end, value);
    if (parsed.ec != std::errc{} || parsed.ptr != end || value == 0 || value > 60'000) {
        throw std::invalid_argument(std::string(option) + " must be an integer in [1, 60000]");
    }
    return value;
}
...
public:
    ...
    [[nodiscard]] std::uint64_t operator()() {
        ...
        for (std::size_t i = 0; i < kOperations; ++i) {
            ...
            if (ready == nullptr) {
                throw std::runtime_error("HTTP/1 benchmark request head did not parse");
            }
...
public:
    ...
    [[nodiscard]] std::uint64_t operator()() noexcept {
        std::uint64_t checksum = 0;
        for (std::size_t i = 0; i < kOperations; ++i) {
            const auto streamId = static_cast<std::uint32_t>(((sequence_++ & 0x3fffU) * 2U) + 1U);
...
public:
    ...
    [[nodiscard]] std::uint64_t operator()() {
        std::uint64_t checksum = 0;
        for (std::size_t i = 0; i < kOperations; ++i) {
            ping_[ruvia::detail::kHttp2FrameHeaderBytes] = static_cast<char>(sequence_++);
            const auto status = connection_.feed(std::string_view(ping_.data(), ping_.size()));
            if (status != ruvia::detail::Http2FeedResult::kAccepted) {
                throw std::runtime_error("HTTP/2 PING fast path was not accepted");
            }
...
public:
    ...
    Http2RequestHeadFeedBatch()
        ...
        ruvia::detail::HpackEncoder::encodeHeader(block, ":method", "GET");
        ruvia::detail::HpackEncoder::encodeHeader(block, ":scheme", "https");
        ruvia::detail::HpackEncoder::encodeHeader(block, ":path", "/items/42?expand=owner");
        ruvia::detail::HpackEncoder::encodeHeader(block, ":authority", "benchmark.example");
        ruvia::detail::HpackEncoder::encodeHeader(block, "accept", "application/json");
        ruvia::detail::HpackEncoder::encodeHeader(block, "accept-encoding", "gzip, br");
        ruvia::detail::HpackEncoder::encodeHeader(block, "user-agent", "ruvia-benchmark");
...
[[nodiscard]] std::string renderJson(const Options& options, const std::vector<Result>& results) {
