...
int main() {
    ...
    try {
        (void)hpackDecoder.decode(std::string_view(indexedLiteral.data(), indexedLiteral.size()), [](std::string_view, std::string_view) -> bool { throw std::runtime_error("callback failure"); });
    ...
    const auto dynamicResult = hpackDecoder.decode(std::string_view(&dynamicIndex, 1), [&](std::string_view name, std::string_view value) {
    ...
    if (!parsedPublicFrame || parsedPublicFrame->length != 7 || parsedPublicFrame->type != static_cast<std::uint8_t>(ruvia::Http2FrameType::kData) || parsedPublicFrame->flags != 1 || parsedPublicFrame->streamId != 3) {
        return 60;
    }
    ...
    const auto publicRequest = publicHttp2.submitRequestHead(ruvia::Http2RegularRequestHeadView{.method = "GET", .scheme = "https", .authority = "example.test", .target = "/"});
    ...
    for (std::size_t index = 0; index < 256; ++index) {
        const auto submitted = publicResetClient.submitRequestHead(ruvia::Http2RegularRequestHeadView{.method = "GET", .scheme = "https", .authority = "example.test", .target = "/cancelled"});
    ...
    const auto publicConnect = publicConnectClient.submitRequestHead(ruvia::Http2ConnectRequestHeadView{.authority = "example.test:443"});
    if (publicConnect.submitted() == nullptr) {
        return 63;
    }
    ...
    const auto unavailableExtendedConnect = publicExtendedConnectClient.submitRequestHead(ruvia::Http2ExtendedConnectRequestHeadView{.protocol = "websocket", .scheme = "https", .authority = "example.test", .target = "/ws"});
    if (unavailableExtendedConnect.failure() == nullptr || unavailableExtendedConnect.failure()->error() != ruvia::Http2RequestHeadSubmitError::kPeerCapabilityUnavailable) {
        return 64;
    }
    ...
    (void)ruvia::encodeHttp2FrameHeader(enableConnectHeader, static_cast<std::uint32_t>(enableConnectProtocol.size()), ruvia::Http2FrameType::kSettings, 0, 0);
    ...
    const auto publicExtendedConnect = publicExtendedConnectClient.submitRequestHead(ruvia::Http2ExtendedConnectRequestHeadView{.protocol = "custom", .scheme = "https", .authority = "example.test", .target = "/tunnel"});
    if (publicExtendedConnect.submitted() == nullptr) {
        return 66;
    }
    ...
    if (publicServer.feed(publicRequestWire) != ruvia::Http2FeedResult::kAccepted) {
        return 62;
    }
    ...
    if (publicRequestHeadEvent == nullptr || publicRequestHeadEvent->request().method() != "POST" || firstChunk == nullptr || firstChunk->bytes() != "a" || secondChunk == nullptr || secondChunk->bytes() != "b" || !publicRequestEnd || publicRequestEnd->messageEnd() == nullptr) {
        return 62;
    }
    ...
    if (otherPublicServer.feed(publicRequestWire) != ruvia::Http2FeedResult::kAccepted) {
        return 62;
    }
    ...
    if (publicServer.acknowledge(std::move(foreignCredit)) != ruvia::Http2ReceivedDataAcknowledgeStatus::kInvalidCredit || otherPublicServer.acknowledge(std::move(foreignCredit)) != ruvia::Http2ReceivedDataAcknowledgeStatus::kAcknowledged || otherPublicServer.acknowledge(otherSecondChunk->takeCredit()) != ruvia::Http2ReceivedDataAcknowledgeStatus::kAcknowledged) {
        return 62;
    }
    if (publicServer.release(std::move(*otherRequestHead)) != ruvia::Http2ServerRequestReleaseStatus::kInvalidLease || otherPublicServer.release(std::move(*otherRequestHead)) != ruvia::Http2ServerRequestReleaseStatus::kReleased) {
        return 62;
    }

    auto firstCredit = firstChunk->takeCredit();
    if (publicServer.acknowledge(std::move(firstCredit)) != ruvia::Http2ReceivedDataAcknowledgeStatus::kAcknowledged || publicServer.acknowledge(std::move(firstCredit)) != ruvia::Http2ReceivedDataAcknowledgeStatus::kInvalidCredit) {
        return 62;
    }
    if (publicServer.acknowledge(secondChunk->takeCredit()) != ruvia::Http2ReceivedDataAcknowledgeStatus::kAcknowledged) {
        return 62;
    }
    if (publicServer.release(std::move(*publicRequestHeadEvent)) != ruvia::Http2ServerRequestReleaseStatus::kReleased || publicServer.release(std::move(*publicRequestHeadEvent)) != ruvia::Http2ServerRequestReleaseStatus::kInvalidLease) {
        return 62;
    }
    if (!publicRequestHeadEvent->request().method().empty()) {
        return 62;
    }
    ...
    if (!invalidWebSocketCompressionRejected) {
        return 60;
    }
    ...
    for (std::size_t index = 0; index < websocketPayload.size(); ++index) {
        websocketFrame[6 + index] = static_cast<char>(websocketPayload[index] ^ websocketMask[index % websocketMask.size()]);
    }
    if (publicWebSocket.feed(std::string_view(websocketFrame.data(), websocketFrame.size())) != ruvia::WebSocketFeedStatus::kAccepted) {
        return 60;
    }
    const auto websocketEvent = publicWebSocket.nextEvent();
    if (!websocketEvent || websocketEvent->message() == nullptr || websocketEvent->message()->payload() != "public") {
        return 60;
    }
    ...
    const auto sseFrame = ruvia::formatSseMessage(ruvia::SseMessage{.data = "public", .retry = std::chrono::milliseconds::zero()}, {.resource = &publicProtocolResource});
    bool negativeRetryRejected = false;
    try {
        (void)ruvia::formatSseMessage(ruvia::SseMessage{.retry = std::chrono::milliseconds{-1}}, {.resource = &publicProtocolResource});
    } catch (const std::invalid_argument&) {
    ...
    const auto decodedUrl = ruvia::detail::decodeUrlComponent("installed%20decoder", {.mode = ruvia::detail::UrlDecodeMode::kPercent, .resource = std::pmr::get_default_resource()});
    const auto malformedUrl = ruvia::detail::decodeUrlComponent("prefix%2", {.mode = ruvia::detail::UrlDecodeMode::kPercent, .resource = std::pmr::get_default_resource()});
    if (!decodedUrl.has_value() || std::string_view(*decodedUrl) != "installed decoder" || malformedUrl.has_value()) {
        return 51;
    }
    auto encodedContent = ruvia::encodeHttpContent(ruvia::HttpContentCoding::kGzip, "installed content encoder", {.maxEncodedBytes = 1024, .resource = std::pmr::get_default_resource()});
    if (encodedContent.encoded() == nullptr || encodedContent.failure() != nullptr || encodedContent.encoded()->bytes().empty()) {
        return 50;
    }
    const auto identityDecode = ruvia::decodeHttpContent(ruvia::HttpContentCoding::kIdentity, "identity", {.maxDecodedBytes = 8, .resource = std::pmr::get_default_resource()});
    ...
    const auto redirectRequestPlan = ruvia::planHttpClientRedirectRequest(outboundRequest, {.status = ruvia::http_status::kTemporaryRedirect, .resource = std::pmr::get_default_resource()});
    ...
    const auto redirectTarget = ruvia::resolveHttpClientRedirectTarget(outboundOrigin, {.currentTarget = "/base/current", .location = "../next?x=1", .resource = std::pmr::get_default_resource()});
    ...
    const auto crossOrigin = ruvia::resolveHttpClientRedirectTarget(outboundOrigin, {.currentTarget = "/base/current", .location = "https://other.test/next", .resource = std::pmr::get_default_resource()});
    ...
    if (!transferCoded.messageReady() || transferCodedBody == nullptr || transferCodedBody->transferCodings().count != 1 || transferCodedBody->transferCodings().values[0] != ruvia::HttpTransferCoding::kGzip) {
        return 14;
    }
