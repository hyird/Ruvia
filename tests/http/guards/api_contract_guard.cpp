...
concept HasPositionalSseFormatResource = requires(const Message& message, std::pmr::memory_resource* resource) { ruvia::formatSseMessage(message, resource); };
...
static_assert(std::same_as<decltype(ruvia::SseFormatOptions{}.resource), std::pmr::memory_resource*>);
static_assert(std::same_as<decltype(ruvia::formatSseMessage(ruvia::SseMessage{}, ruvia::SseFormatOptions{})), std::pmr::string>);
...
template <typename T>
concept HasHttp2ConnectionPositionalResourceFactory = requires(std::pmr::memory_resource* resource) { T::server(resource); } || requires(std::pmr::memory_resource* resource) { T::client(resource); };

static_assert(!HasAmbiguousQueryLookup<ruvia::HttpRequest>);
static_assert(std::same_as<decltype(std::declval<const ruvia::HttpRequest&>().lastRawQueryValue(std::string_view{})), std::optional<std::string_view>>);
static_assert(!std::copy_constructible<ruvia::Http2Connection>);
static_assert(std::move_constructible<ruvia::Http2Connection>);
static_assert(!std::constructible_from<ruvia::Http2Connection, std::pmr::memory_resource*, ruvia::Http2Role>);
static_assert(std::is_aggregate_v<ruvia::Http2ConnectionOptions>);
static_assert(std::same_as<decltype(ruvia::Http2ConnectionOptions{}.resource), std::pmr::memory_resource*>);
...
static_assert(std::same_as<decltype(ruvia::Http2Connection::server(ruvia::Http2ConnectionOptions{})), ruvia::Http2Connection>);
static_assert(std::same_as<decltype(ruvia::Http2Connection::client(ruvia::Http2ConnectionOptions{})), ruvia::Http2Connection>);
...
static_assert(std::same_as<decltype(std::declval<const ruvia::Http2RequestHeadEvent&>().expectationPlan(ruvia::HttpUnsupportedExpectationPolicy::kReject)), ruvia::HttpServerExpectationPlan>);
static_assert(std::same_as<decltype(std::declval<const ruvia::Http2ResponseHeadEvent&>().head()), const ruvia::HttpClientResponseHead&>);
static_assert(std::same_as<decltype(std::declval<ruvia::Http2MessageBodyChunkEvent&>().takeCredit()), ruvia::Http2ReceivedDataCredit>);
...
template <typename T>
concept HasLegacyHttp2RequestSubmission = requires(T& connection) { connection.submitRegularRequestHead("GET", "https", "example.test", "/", std::span<const ruvia::HttpHeaderView>{}, ruvia::Http2RequestContent::none()); };

template <typename T>
concept HasTypedHttp2RequestSubmission = requires(T& connection, const ruvia::Http2RegularRequestHeadView& regular, const ruvia::Http2ConnectRequestHeadView& connect, const ruvia::Http2ExtendedConnectRequestHeadView& extended) {
    { connection.submitRequestHead(regular) } -> std::same_as<ruvia::Http2RequestHeadSubmitResult>;
    { connection.submitRequestHead(connect) } -> std::same_as<ruvia::Http2RequestHeadSubmitResult>;
    { connection.submitRequestHead(extended) } -> std::same_as<ruvia::Http2RequestHeadSubmitResult>;
};

template <typename T>
concept HasBareHttp2ServerRequestRelease = requires(T& connection) { connection.releaseServerRequest(std::uint32_t{1}); } || requires(T& connection) { connection.release(std::uint32_t{1}); };
...
static_assert(std::same_as<decltype(ruvia::HpackDecoderOptions{}.resource), std::pmr::memory_resource*>);
...
template <typename T>
concept HasPositionalHpackHeaderWithNameIndexFlag = requires(std::pmr::string& output, std::uint32_t index, std::string_view value) { T::encodeHeaderWithNameIndex(output, index, value, true); };

static_assert(!HasPositionalHpackHeaderWithNameIndexFlag<ruvia::HpackEncoder>);

template <typename T>
concept HasRawHpackCallback = requires(T& decoder) { decoder.decode(std::string_view{}, nullptr, static_cast<bool (*)(void*, std::string_view, std::string_view)>(nullptr)); };
...
static_assert(requires(ruvia::HpackDecoder& decoder) {
    { decoder.decode(std::string_view{}, hpackFunctionCallback) } -> std::same_as<ruvia::HpackDecodeResult>;
});
...
static_assert(std::same_as<decltype(ruvia::WebSocketServerConnectionOptions{}.resource), std::pmr::memory_resource*>);
static_assert(std::same_as<decltype(ruvia::WebSocketServerConnectionOptions{}.messageLimit), ruvia::ProtocolByteLimit>);
static_assert(std::same_as<decltype(ruvia::WebSocketServerConnectionOptions{}.compression), ruvia::WebSocketCompression>);

template <typename T>
concept HasPositionalWebSocketServerConnectionResource = requires(std::pmr::memory_resource* resource) { T(resource); };
...
template <typename Input>
concept AcceptsMultipartBorrowedInput = requires(Input&& input) { ruvia::parseMultipartBody(std::forward<Input>(input), {.boundary = ruvia::MultipartBoundary("x")}); };

template <typename T>
concept HasPositionalMultipartParserConstructor = requires(std::pmr::memory_resource* resource) { T(ruvia::MultipartBoundary("x"), resource); };

template <typename T>
concept HasPositionalMultipartBodyParse = requires(ApiGuardDependent<T, std::string_view> body, ApiGuardDependent<T, ruvia::MultipartBoundary> boundary, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::parseMultipartBody(body, boundary); } || requires(ApiGuardDependent<T, std::string_view> body, ApiGuardDependent<T, ruvia::MultipartBoundary> boundary, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::parseMultipartBody(body, boundary, resource); };

template <typename Input>
concept AcceptsSetCookieBorrowedInput = requires(Input&& input) { ruvia::parseSetCookie(std::forward<Input>(input)); };

template <typename T>
concept HasSetCookiePublicField = requires(T& cookie) { cookie.name = std::string_view{}; } || requires(T& cookie) { cookie.value = std::string_view{}; } || requires(T& cookie) { cookie.path = std::string_view{}; } || requires(T& cookie) { cookie.domain = std::string_view{}; } || requires(T& cookie) { cookie.expires = std::optional<std::time_t>{}; } || requires(T& cookie) { cookie.maxAgeSeconds = std::optional<std::int64_t>{}; } || requires(T& cookie) { cookie.secure = true; } || requires(T& cookie) { cookie.hasPathAttribute = true; } || requires(T& cookie) { cookie.sameSiteNone = true; };
...
template <typename T>
concept HasAnyCacheControlBooleanField = requires(T& directives) { directives.noStore; } || requires(T& directives) { directives.noCache; } || requires(T& directives) { directives.noTransform; } || requires(T& directives) { directives.mustRevalidate; } || requires(T& directives) { directives.proxyRevalidate; } || requires(T& directives) { directives.isPrivate; } || requires(T& directives) { directives.isPublic; } || requires(T& directives) { directives.immutable; } || requires(T& directives) { directives.onlyIfCached; } || requires(T& directives) { directives.maxStaleAny; };

template <typename T>
concept HasAnyCacheControlDeltaSecondsField = requires(T& directives) { directives.maxAge; } || requires(T& directives) { directives.maxStale; } || requires(T& directives) { directives.minFresh; } || requires(T& directives) { directives.sMaxAge; } || requires(T& directives) { directives.staleWhileRevalidate; } || requires(T& directives) { directives.staleIfError; };
...
template <typename T>
concept HasPositionalHttpContentDecode = requires(ApiGuardDependent<T, std::string_view> input, ApiGuardDependent<T, std::size_t> maxDecodedBytes, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::decodeHttpContent(ruvia::HttpContentCoding::kGzip, input, maxDecodedBytes); } || requires(ApiGuardDependent<T, std::string_view> input, ApiGuardDependent<T, std::size_t> maxDecodedBytes, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::decodeHttpContent(ruvia::HttpContentCoding::kGzip, input, maxDecodedBytes, resource); };
...
template <typename T>
concept HasPositionalHttpContentEncode = requires(ApiGuardDependent<T, std::string_view> input, ApiGuardDependent<T, std::size_t> maxEncodedBytes, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::encodeHttpContent(ruvia::HttpContentCoding::kGzip, input, maxEncodedBytes); } || requires(ApiGuardDependent<T, std::string_view> input, ApiGuardDependent<T, std::size_t> maxEncodedBytes, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::encodeHttpContent(ruvia::HttpContentCoding::kGzip, input, maxEncodedBytes, resource); };
...
concept HasPositionalUrlDecodeOptions = requires(Mode mode, std::pmr::memory_resource* resource) { ruvia::detail::decodeUrlComponent(std::string_view{}, mode, resource); };
...
concept AcceptsResponseTrailerRange = requires(Headers&& headers) { ruvia::detail::httpResponseTrailerSection(std::forward<Headers>(headers)); };

template <typename Headers>
concept AcceptsValidatedResponseTrailerRange = requires(Headers&& headers) { ruvia::detail::validatedResponseTrailerSection(std::forward<Headers>(headers)); };
...
template <typename T>
concept AcceptsRvalueWebSocketResponsePartVisitor = requires(T&& handshake) { std::move(handshake).forEachResponsePart([](std::string_view) {}); };
...
template <typename T>
concept HasPositionalHttpRequestContentDecode = requires(ApiGuardDependent<T, std::string_view> input, ApiGuardDependent<T, std::size_t> maxDecodedBytes, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::detail::decodeHttpRequestContent(ruvia::HttpContentCoding::kGzip, input, maxDecodedBytes, resource); };
...
template <typename T>
concept HasPositionalHttpClientRedirectRequestPlan = requires(const ruvia::HttpClientRequestView& request, ApiGuardDependent<T, ruvia::HttpStatusCode> status, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::planHttpClientRedirectRequest(request, status); } || requires(const ruvia::HttpClientRequestView& request, ApiGuardDependent<T, ruvia::HttpStatusCode> status, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::planHttpClientRedirectRequest(request, status, resource); };

template <typename T>
concept HasPositionalHttpClientRedirectTargetResolution = requires(const ruvia::HttpOriginView& origin, ApiGuardDependent<T, std::string_view> currentTarget, ApiGuardDependent<T, std::string_view> location, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::resolveHttpClientRedirectTarget(origin, currentTarget, location); } || requires(const ruvia::HttpOriginView& origin, ApiGuardDependent<T, std::string_view> currentTarget, ApiGuardDependent<T, std::string_view> location, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::resolveHttpClientRedirectTarget(origin, currentTarget, location, resource); };
...
template <typename T>
concept HasRawResponseCodingSelector = requires(const T& qualities) { httpSelectResponseCodingFromQualities(qualities); };

template <typename T>
concept HasRawResponseCodingScore = requires(const T& quality) { httpAcceptedEncodingScore(quality); };

template <typename T>
concept HasRawResponseIdentityScore = requires(const T& quality) { httpAcceptedIdentityScore(quality); };
...
static_assert(std::same_as<decltype(ruvia::HttpResponse::Options{}.resource), std::pmr::memory_resource*>);
...
static_assert(std::same_as<decltype(std::declval<const ruvia::CacheControl&>().has(ruvia::CacheControlDirective::kNoTransform)), bool>);
static_assert(std::same_as<decltype(std::declval<const ruvia::CacheControl&>().sMaxAge()), std::optional<std::uint64_t>>);
...
static_assert(ruvia::httpClientExpectationIsValid(false, ruvia::HttpRequestContentIndication::kNoContent));
static_assert(!ruvia::httpClientExpectationIsValid(true, ruvia::HttpRequestContentIndication::kNoContent));
static_assert(ruvia::httpClientExpectationIsValid(true, ruvia::HttpRequestContentIndication::kWillFollow));
...
concept HasTypedHttpServerExpectationPlan = requires(const T& state) {
    { state.expectationPlan(ruvia::HttpUnsupportedExpectationPolicy::kReject) } -> std::same_as<ruvia::HttpServerExpectationPlan>;
};
...
static_assert(std::same_as<decltype(std::declval<const ruvia::Http1RequestBodyPlan&>().expectations()), ruvia::HttpRequestExpectations>);
static_assert(std::same_as<decltype(std::declval<const ruvia::Http1RequestBodyPlan&&>().expectations()), ruvia::HttpRequestExpectations>);
...
static_assert(std::constructible_from<ruvia::Http1ClientResponseParser, ruvia::Http1ClientExchangeState&&, ruvia::Http1ClientResponseParser::Options>);
static_assert(!std::constructible_from<ruvia::Http1ClientResponseParser, ruvia::Http1ClientExchangeState&&, std::pmr::memory_resource*>);
static_assert(!std::constructible_from<ruvia::Http1ClientResponseParser, ruvia::Http1ClientExchangeState&>);
...
static_assert(std::same_as<decltype(std::declval<const ruvia::HttpUnsupportedExpectationRejection&>().protocolError()), ruvia::HttpProtocolError>);
...
using WebSocketServerNegotiator = ruvia::detail::WebSocketServerNegotiation (*)(const ruvia::HttpRequest&, ruvia::detail::WebSocketServerNegotiationOptions);
using WebSocketServerHandshakeFactory = ruvia::WebSocketServerHandshake (*)(const ruvia::HttpRequest&, ruvia::WebSocketServerHandshakeOptions);
...
template <typename T>
concept HasPositionalWebSocketServerNegotiationFactory = requires(const ruvia::HttpRequest& request, ApiGuardDependent<T, std::span<const std::string_view>> supportedSubprotocols, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::detail::makeWebSocketServerNegotiation(request, supportedSubprotocols); } || requires(const ruvia::HttpRequest& request, ApiGuardDependent<T, std::span<const std::string_view>> supportedSubprotocols, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::detail::makeWebSocketServerNegotiation(request, supportedSubprotocols, resource); };

template <typename T>
concept HasPositionalWebSocketServerHandshakeFactory = requires(const ruvia::HttpRequest& request, ApiGuardDependent<T, std::span<const std::string_view>> supportedSubprotocols, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::makeWebSocketServerHandshake(request, supportedSubprotocols); } || requires(const ruvia::HttpRequest& request, ApiGuardDependent<T, std::span<const std::string_view>> supportedSubprotocols, ApiGuardDependent<T, std::pmr::memory_resource*> resource) { ruvia::makeWebSocketServerHandshake(request, supportedSubprotocols, resource); };
...
static_assert(std::same_as<decltype(ruvia::detail::WebSocketServerNegotiationOptions{}.supportedSubprotocols), std::span<const std::string_view>>);
static_assert(std::same_as<decltype(ruvia::detail::WebSocketServerNegotiationOptions{}.resource), std::pmr::memory_resource*>);
static_assert(std::same_as<decltype(ruvia::WebSocketServerHandshakeOptions{}.supportedSubprotocols), std::span<const std::string_view>>);
static_assert(std::same_as<decltype(ruvia::WebSocketServerHandshakeOptions{}.resource), std::pmr::memory_resource*>);
static_assert(!HasPositionalWebSocketServerNegotiationFactory<ruvia::detail::WebSocketServerNegotiation>);
...
static_assert(std::same_as<decltype(std::declval<ruvia::MultipartParseOptions&>().boundary), ruvia::MultipartBoundary>);
static_assert(std::same_as<decltype(ruvia::MultipartParseOptions{.boundary = ruvia::MultipartBoundary("x")}.resource), std::pmr::memory_resource*>);
...
static_assert(std::same_as<decltype(ruvia::parseMultipartBody(std::string_view{}, ruvia::MultipartParseOptions{.boundary = ruvia::MultipartBoundary("x")})), ruvia::MultipartBodyParseResult>);
...
static_assert(std::same_as<decltype(ruvia::planHttpClientRedirectRequest(std::declval<const ruvia::HttpClientRequestView&>(), ruvia::HttpClientRedirectRequestPlanOptions{.status = ruvia::http_status::kTemporaryRedirect})), ruvia::HttpClientRedirectRequestPlan>);
...
static_assert(std::same_as<decltype(ruvia::detail::decodeHttpRequestContent(ruvia::HttpContentCoding::kGzip, std::string_view{}, ruvia::HttpContentDecodeOptions{.maxDecodedBytes = 0, .resource = nullptr})), ruvia::detail::HttpRequestContentDecodeResult>);
static_assert(!HasPositionalHttpRequestContentDecode<ruvia::detail::HttpRequestContentDecodeResult>);
...
static_assert(std::same_as<decltype(ruvia::decodeHttpContent(ruvia::HttpContentCoding::kGzip, std::string_view{}, ruvia::HttpContentDecodeOptions{.maxDecodedBytes = 0, .resource = nullptr})), ruvia::HttpContentDecodeResult>);
static_assert(std::is_aggregate_v<ruvia::HttpContentDecodeOptions>);
static_assert(std::same_as<decltype(ruvia::HttpContentDecodeOptions{}.maxDecodedBytes), std::size_t>);
static_assert(std::same_as<decltype(ruvia::HttpContentDecodeOptions{}.resource), std::pmr::memory_resource*>);
...
static_assert(std::same_as<decltype(ruvia::encodeHttpContent(ruvia::HttpContentCoding::kGzip, std::string_view{}, ruvia::HttpContentEncodeOptions{.maxEncodedBytes = 0, .resource = nullptr})), ruvia::HttpContentEncodeResult>);
static_assert(std::is_aggregate_v<ruvia::HttpContentEncodeOptions>);
static_assert(std::same_as<decltype(ruvia::HttpContentEncodeOptions{}.maxEncodedBytes), std::size_t>);
static_assert(std::same_as<decltype(ruvia::HttpContentEncodeOptions{}.resource), std::pmr::memory_resource*>);
...
static_assert(std::same_as<decltype(ruvia::detail::UrlDecodeOptions{}.mode), ruvia::detail::UrlDecodeMode>);
static_assert(std::same_as<decltype(ruvia::detail::UrlDecodeOptions{}.resource), std::pmr::memory_resource*>);
static_assert(std::same_as<decltype(ruvia::detail::decodeUrlComponent(std::string_view{}, ruvia::detail::UrlDecodeOptions{.mode = ruvia::detail::UrlDecodeMode::kPercent, .resource = nullptr})), std::optional<std::pmr::string>>);
