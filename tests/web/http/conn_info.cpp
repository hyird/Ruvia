...
concept HasBooleanTransportRefinement = requires(const Services& services, std::string_view remoteAddress, std::string_view clientCertificateSubject, bool secure) { services.withTransport(remoteAddress, clientCertificateSubject, secure); };

template <typename Services>
concept AcceptsRvaluePlainTransport = requires(const Services& services) { services.withPlainTransport(std::string("temporary")); };

template <typename Services>
concept AcceptsRvalueTlsAddress = requires(const Services& services) { services.withTlsTransport(std::string("temporary")); };

template <typename Services>
concept AcceptsRvalueTlsCertificate = requires(const Services& services) { services.withTlsTransport(std::string_view("stable"), std::string("temporary")); };
...
static_assert(std::is_same_v<decltype(std::declval<const ConnInfo&>().plain()), const PlainConnectionTransport*>);
...
[[nodiscard]] std::size_t activeTransportCount(const ConnInfo& info) noexcept {
    return static_cast<std::size_t>(info.plain() != nullptr) + static_cast<std::size_t>(info.tls() != nullptr);
}
...
RUVIA_TEST(conn_info_transport_has_one_active_alternative) {
    ...
    RUVIA_CHECK_EQ(mutualTls.connInfo().tls()->clientCertificateSubject(), std::string_view("CN=typed-client"));
...
RUVIA_TEST(context_preserves_typed_connection_info_for_handler) {
    ...
    RUVIA_CHECK_EQ(tlsInfo.tls()->clientCertificateSubject(), std::string_view("CN=request-client"));
}
