...
WebSocketLivenessDecision decide(const WebSocketLifecycleOptions& opts, WebSocketLivenessMode livenessMode,
...
RUVIA_TEST(ws_heartbeat_disabled_stays_idle) {
    ...
    RUVIA_CHECK(decide(options(0, 0), WebSocketLivenessMode::kOpen, WebSocketLivenessIdle{}, false, 0,
                    10000) == WebSocketLivenessDecision::kIdle);
    RUVIA_CHECK(decide(options(1000, 500), WebSocketLivenessMode::kInactive, WebSocketLivenessIdle{},
...
RUVIA_TEST(ws_heartbeat_sends_ping_when_idle) {
    ...
    RUVIA_CHECK(decide(options(1000, 500), WebSocketLivenessMode::kOpen, WebSocketLivenessIdle{}, false, 0,
                    2000) == WebSocketLivenessDecision::kSendPing);
    // Recent activity keeps it idle.
    RUVIA_CHECK(decide(options(1000, 500), WebSocketLivenessMode::kOpen, WebSocketLivenessIdle{}, false,
                    1500, 2000) == WebSocketLivenessDecision::kIdle);
    // A write in flight defers the ping.
    RUVIA_CHECK(decide(options(1000, 500), WebSocketLivenessMode::kOpen, WebSocketLivenessIdle{}, true, 0,
...
RUVIA_TEST(ws_liveness_bounds_local_close_handshake) {
    ...
    RUVIA_CHECK(decide(opts, WebSocketLivenessMode::kAwaitingPeerClose, WebSocketAwaitingPeerClose(1000),
                    false, 0, 2999) == WebSocketLivenessDecision::kIdle);
    RUVIA_CHECK(decide(opts, WebSocketLivenessMode::kAwaitingPeerClose, WebSocketAwaitingPeerClose(1000),
                    false, 0, 3000) == WebSocketLivenessDecision::kAbortTransport);
    RUVIA_CHECK(decide(options(1000, 500, 0), WebSocketLivenessMode::kAwaitingPeerClose,
