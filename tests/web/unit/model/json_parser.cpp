...
RUVIA_TEST(model_json_parser_dispatches_decoded_keys) {
    ...
    const auto parsed = ruvia::fromJson<AccessorSurfaceRequest>(
        R"({"mess\u0061ge":"ready"})", {.resource = &resource});
...
RUVIA_TEST(model_public_parsers_own_string_fields) {
    ...
    auto json = ruvia::fromJson<AccessorSurfaceRequest>(
        std::string(R"({"message":"json-owned"})"), {.resource = &resource});
...
RUVIA_TEST(model_json_parser_keeps_field_errors_separate_from_document_errors) {
    std::pmr::monotonic_buffer_resource resource;
    RUVIA_CHECK(!ruvia::fromJson<NestedModelItem>(
        R"({"id":"wrong","label":"still parsed"})", {.resource = &resource})
            .has_value());
    const auto parsed = ruvia::detail::ModelParseAccess::parseJsonBorrowedPartial<NestedModelItem>(
    ...
    RUVIA_CHECK(ruvia::detail::ModelValidationAccess::fieldState<"id">(*parsed) ==
...
RUVIA_TEST(model_json_parser_fully_validates_unknown_and_duplicate_values) {
    ...
    RUVIA_CHECK(!ruvia::fromJson<AccessorSurfaceRequest>(
        R"({"message":"ready","unknown":[1,]})", {.resource = &resource})
            .has_value());
    RUVIA_CHECK(!ruvia::fromJson<AccessorSurfaceRequest>(
        R"({"message":"ready","unknown":"\ud83d"})", {.resource = &resource})
            .has_value());
    RUVIA_CHECK(!ruvia::fromJson<AccessorSurfaceRequest>(
        R"({"message":"first","message":{"broken":}})", {.resource = &resource})
            .has_value());
    RUVIA_CHECK(!ruvia::fromJson<AccessorSurfaceRequest>(
        R"({"message":"first","message":"\ud83d"})", {.resource = &resource})
            .has_value());

    RUVIA_CHECK(!ruvia::fromJson<AccessorSurfaceRequest>(
        R"({"message":"first","message":"second"})", {.resource = &resource})
            .has_value());
    ...
    RUVIA_CHECK(ruvia::detail::ModelValidationAccess::fieldState<"message">(*duplicate) ==
...
RUVIA_TEST(model_json_parser_enforces_depth_while_skipping_unknown_values) {
    ...
    std::pmr::monotonic_buffer_resource resource;
    RUVIA_CHECK(
        !ruvia::fromJson<AccessorSurfaceRequest>(input, {.resource = &resource}).has_value());
}
