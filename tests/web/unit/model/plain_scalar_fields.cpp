...
RUVIA_REQUEST_MODEL(WrappedScalars, RUVIA_OPTIONAL_FIELD(count, ruvia::UInt32), RUVIA_OPTIONAL_FIELD(ratio, ruvia::Double), RUVIA_OPTIONAL_FIELD(enabled, ruvia::Bool), RUVIA_OPTIONAL_FIELD(delta, ruvia::Int64));

RUVIA_RESPONSE_MODEL(WrappedScalarsResponse, RUVIA_OPTIONAL_FIELD(count, ruvia::UInt32), RUVIA_OPTIONAL_FIELD(ratio, ruvia::Double), RUVIA_OPTIONAL_FIELD(enabled, ruvia::Bool), RUVIA_OPTIONAL_FIELD(delta, ruvia::Int64));

RUVIA_REQUEST_MODEL(WrappedDefaulted, RUVIA_OPTIONAL_FIELD(retries, ruvia::UInt32, RUVIA_DEFAULT(3)));
...
RUVIA_TEST(model_wrapper_scalar_fields_reject_mistyped_values_but_allow_missing_optional_fields) {
    ...
    if (partialForm) {
        RUVIA_CHECK(ruvia::detail::ModelValidationAccess::fieldState<"count">(*partialForm) == ruvia::detail::ModelFieldState::kInvalidType);
    }
}
...
RUVIA_TEST(model_wrapper_scalar_fields_round_trip_through_json) {
    ...
    response.set<"count">(*parsed->get<"count">()).set<"ratio">(*parsed->get<"ratio">()).set<"enabled">(*parsed->get<"enabled">()).set<"delta">(*parsed->get<"delta">());
