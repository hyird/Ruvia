...
RUVIA_TEST(model_wrapper_scalar_fields_parse_json_and_forms) {
    ...
    const auto form = ruvia::FormBody<WrappedScalars>::parse("count=41&ratio=2.5&enabled=true&delta=-9", &formResource);
...
RUVIA_TEST(model_wrapper_scalar_fields_round_trip_through_json) {
    std::pmr::monotonic_buffer_resource resource;
    const auto parsed = ruvia::JsonBody<WrappedScalars>::parse(R"({"count":1,"ratio":2.5,"enabled":false,"delta":-3})", &resource);
...
RUVIA_TEST(model_wrapper_scalar_fields_apply_defaults) {
    ...
    const auto defaulted = ruvia::JsonBody<WrappedDefaulted>::parse("{}", &defaultResource);
