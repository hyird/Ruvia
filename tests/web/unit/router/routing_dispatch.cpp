...
RUVIA_TEST(websocket_route_owns_validated_lifecycle_policy) {
    ...
    bool rejected = false;
    try {
        invalid.registerWebSocketRoute(HttpKnownMethod::kGet, path("/invalid-ws"),
            ruvia::detail::RouteStreamHandler(nullptr, &dummyStreamHandler),
            std::span<const ControllerMiddlewareDescriptor>{},
            std::span<const ControllerMiddlewareDescriptor>{}, invalidOptions);
...
RUVIA_TEST(streaming_get_routes_do_not_gain_head_shadow) {
    ...
    impl.registerResponseStreamRoute(HttpKnownMethod::kGet, path("/events"),
        ruvia::detail::RouteStreamHandler(nullptr, &dummyStreamHandler), {}, {});
    impl.registerSseRoute(HttpKnownMethod::kGet, path("/sse"),
        ruvia::detail::RouteStreamHandler(nullptr, &dummyStreamHandler), {}, {});
    impl.registerWebSocketRoute(HttpKnownMethod::kGet, path("/ws"),
        ruvia::detail::RouteStreamHandler(nullptr, &dummyStreamHandler), {}, {});
    impl.finalize();
...
RUVIA_TEST(dispatch_maps_handler_exceptions_to_error_responses) {
    ...
    const auto application =
        dispatchOne(RouteHandler(nullptr, &throwsHttpErrorHandler), HttpKnownMethod::kGet, "/x");
    ...
    const auto generic =
        dispatchOne(RouteHandler(nullptr, &throwsGenericHandler), HttpKnownMethod::kGet, "/x");
...
RUVIA_TEST(dispatch_defensively_rejects_malformed_request_content_coding) {
    const auto result = dispatchOneToken(
        RouteHandler(nullptr, &readsRequestBodyHandler), "GET", "/x", "gzip;level=9", "encoded");
...
RUVIA_TEST(dispatch_produces_404_and_405_for_unmatched_routes) {
    ...
    RUVIA_CHECK_EQ(
        dispatchOne(RouteHandler(nullptr, &okHandler), HttpKnownMethod::kGet, "/nope").status,
        std::uint16_t{404});
...
RUVIA_TEST(dispatch_uses_custom_not_found_handler) {
    ...
    const auto result = dispatchWithHandlers(RouteHandler(nullptr, &okHandler), nullptr,
        &customNotFound, HttpKnownMethod::kGet, "/nope");
...
RUVIA_TEST(dispatch_uses_custom_error_handler_with_thrown_status) {
    ...
    const auto result = dispatchWithHandlers(RouteHandler(nullptr, &throwsHttpErrorHandler),
        &customError, nullptr, HttpKnownMethod::kGet, "/x");
...
RUVIA_TEST(dispatch_preserves_content_coding_advertisement_with_custom_error_handler) {
    const auto result = dispatchWithHandlersToken(RouteHandler(nullptr, &readsRequestBodyHandler),
...
RUVIA_TEST(dispatch_routes_unimplemented_method_through_custom_error_handler) {
    const auto result = dispatchWithHandlersToken(
        RouteHandler(nullptr, &okHandler), &customError, nullptr, "PROPFIND", "/x");
...
RUVIA_TEST(request_json_if_and_form_if_only_fall_back_on_media_type_mismatch) {
    ...
    const auto wrongType =
        dispatchBodyRequest(RouteHandler(nullptr, &jsonIfEchoHandler), "text/plain", "x");
    RUVIA_CHECK_EQ(wrongType.status, std::uint16_t{200});
    RUVIA_CHECK_EQ(wrongType.body, std::string("no-json"));
    const auto badBody = dispatchBodyRequest(
        RouteHandler(nullptr, &jsonIfEchoHandler), "application/json", "{not-json");
    RUVIA_CHECK_EQ(badBody.status, std::uint16_t{400});
    const auto formWrongType =
    ...
    const auto ok = dispatchBodyRequest(
        RouteHandler(nullptr, &jsonIfEchoHandler), "application/json", R"({"value":"hi"})");
    RUVIA_CHECK_EQ(ok.status, std::uint16_t{200});
    RUVIA_CHECK_EQ(ok.body, std::string("hi"));
    const auto formOk = dispatchBodyRequest(
        RouteHandler(nullptr, &formIfEchoHandler), "application/x-www-form-urlencoded", "value=hi");
    RUVIA_CHECK_EQ(formOk.status, std::uint16_t{200});
    RUVIA_CHECK_EQ(formOk.body, std::string("hi"));
}
...
RUVIA_TEST(prefix_not_found_handler_scopes_by_longest_segment_prefix) {
    ...
    impl.setPrefixNotFoundHandlers(
        std::span<const ruvia::detail::HttpPrefixNotFoundHandler>(scoped, 2));
    impl.finalize();
    const auto& table = impl.routeTable();
...
RUVIA_TEST(prefix_error_handler_scopes_thrown_route_failures) {
    ...
    impl.registerRoute(HttpKnownMethod::kGet, path("/api/boom"),
        RouteHandler(nullptr, &throwsHttpErrorHandler), RequestBodyMode::kBuffered,
        std::span<const ControllerMiddlewareDescriptor>{},
        std::span<const ControllerMiddlewareDescriptor>{});
...
RUVIA_TEST(dispatch_options_asterisk_returns_server_wide_allow) {
    ...
    impl.registerRoute(HttpKnownMethod::kPost, path("/b"), RouteHandler(nullptr, &okHandler),
        RequestBodyMode::kBuffered, std::span<const ControllerMiddlewareDescriptor>{},
        std::span<const ControllerMiddlewareDescriptor>{});
    impl.registerExtensionMethodRoute("PROPFIND", path("/c"), RouteHandler(nullptr, &okHandler),
        RequestBodyMode::kBuffered, std::span<const ControllerMiddlewareDescriptor>{},
        std::span<const ControllerMiddlewareDescriptor>{});
    impl.finalize();
    const auto& table = impl.routeTable();
    ...
    auto future = asio::co_spawn(
        ctx, ruvia::detail::taskAsAwaitable(table.dispatch(request, memory, {})), asio::use_future);
    ctx.run();
