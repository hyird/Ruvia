...
static_assert(!std::constructible_from<ruvia::detail::RedisRegistry, asio::io_context&, std::pmr::memory_resource*, RedisDefinitions>);
static_assert(std::constructible_from<ruvia::detail::RedisRegistry, asio::io_context&, std::pmr::memory_resource*, RedisDefinitions, ruvia::WorkerHandle>);
static_assert(!std::constructible_from<ruvia::detail::RedisPool, asio::io_context&, ruvia::detail::RedisConfigStorage&&, std::optional<std::chrono::milliseconds>, std::size_t, const ruvia::WorkerHandle&, std::pmr::memory_resource*>);
static_assert(!std::constructible_from<ruvia::detail::RedisPool, asio::io_context&, const ruvia::detail::RedisConfigStorage&, std::optional<std::chrono::milliseconds>, std::size_t, std::pmr::memory_resource*>);
static_assert(!std::constructible_from<ruvia::detail::RedisPool, asio::io_context&, const ruvia::detail::RedisConfigStorage&, std::optional<std::chrono::milliseconds>, std::size_t, std::pmr::memory_resource*, const ruvia::WorkerHandle*>);
static_assert(!std::constructible_from<ruvia::detail::RedisPool, asio::io_context&, const ruvia::detail::RedisConfigStorage&, std::optional<std::chrono::milliseconds>, std::size_t, ruvia::WorkerHandle, std::pmr::memory_resource*>);
...
template <typename T>
concept HasRedisPerCallCommandOptions = requires(const T& handle, std::span<const std::string_view> args) { handle.command(args, ruvia::OperationOptions{}); } || requires(const T& handle) { handle.command(ruvia::OperationOptions{}, "PING"); };

template <typename T>
concept HasRedisBlockingPerCallOptions = requires(const T& handle, std::span<const std::string_view> keys) {
...
template <typename T>
concept HasRedisRegistrationPositional = requires(T& app, ruvia::RedisConfig config) { app.redis(config); } || requires(T& app, ruvia::RedisConfig config) { app.redis(std::string_view{}, config); };
...
template <typename T>
concept HasLegacyRedisSetCommands = requires(const T& handle) { handle.setEx("key", std::chrono::seconds(1), "value"); } || requires(const T& handle) { handle.setNx("key", "value"); } || requires(const T& handle) { handle.getSet("key", "value"); };
...
RUVIA_TEST(redis_blocking_commands_ignore_the_ordinary_pool_timeout_and_require_a_cancellation_bound) {
    ...
    ruvia::detail::RedisRegistry registry(ioContext, std::pmr::get_default_resource(), definitions, worker.handle());
    ...
    try {
        (void)redis.withOptions({.timeout = std::chrono::seconds(1)}).command("BLPOP", "queue", "1");
    } catch (...) {
...
RUVIA_TEST(redis_registry_rejects_an_invalid_worker) {
    ...
    RUVIA_CHECK(throwsInvalidArgument([&] { ruvia::detail::RedisRegistry registry(ioContext, std::pmr::get_default_resource(), definitions, worker); }));
}
...
RUVIA_TEST(redis_registry_owns_nested_pmr_configuration) {
    ...
    registry.emplace(ioContext, &targetResource, std::span<const ruvia::detail::RedisDefinition>(&*definition, 1), worker.handle());
    definition.reset();
...
RUVIA_TEST(redis_transaction_errors_preserve_server_diagnostics) {
    const auto reply = ruvia::detail::RedisTypesAccess::errorValue("EXECABORT Transaction discarded because of previous errors.", std::pmr::get_default_resource());
    bool preserved = false;
    try {
        ruvia::detail::throwIfRedisTransactionReplyError(reply, 3);
    } catch (const ruvia::RedisError& error) {
        preserved = error.code() == ruvia::RedisError::Code::kCommandError && std::string_view(error.what()).find("reply 3") != std::string_view::npos && std::string_view(error.what()).find("EXECABORT") != std::string_view::npos;
    }
...
RUVIA_TEST(redis_active_command_reports_pool_closing_instead_of_io_error) {
    ...
    ruvia::detail::RedisPool pool(ioContext, storedConfig, storedConfig.commandTimeout, 1, worker.handle(), resource);
    ...
    auto result = asio::co_spawn(ioContext, ruvia::detail::taskAsAwaitable(exercise()), asio::use_future);
