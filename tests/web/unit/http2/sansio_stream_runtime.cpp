...
static_assert(!std::constructible_from<ruvia::detail::Http2SansIoStreamSignal, ruvia::WorkerHandle&&, ruvia::detail::Http2SansIoTermination&>);
static_assert(!std::constructible_from<ruvia::detail::Http2SansIoSleepAwaiter, ruvia::WorkerHandle&&, ruvia::detail::Http2SansIoTermination&, std::chrono::steady_clock::duration>);
static_assert(!std::constructible_from<ruvia::detail::Http2SansIoSleepAwaiter, ruvia::WorkerHandle&&, ruvia::detail::Http2SansIoTermination&, std::chrono::steady_clock::duration, ruvia::StopToken>);
static_assert(std::constructible_from<Http2SansIoResponseStreamSink, ruvia::detail::Http2Connection&, std::uint32_t, ruvia::detail::ResponseStreamKind, ruvia::detail::WorkerSignal&, ruvia::detail::Http2SansIoStreamSignal&, std::pmr::memory_resource*, ruvia::HttpKnownMethod, HttpResponseCodingSelection, ruvia::detail::HttpResponseCodingAvailability>);
static_assert(!std::constructible_from<Http2SansIoResponseStreamSink, ruvia::detail::Http2Connection&, std::uint32_t, ruvia::detail::ResponseStreamKind, ruvia::detail::WorkerSignal&, ruvia::detail::Http2SansIoStreamSignal&, std::pmr::memory_resource*, ruvia::HttpKnownMethod, HttpResponseCodingSelection>);
...
RUVIA_TEST(http2_stream_sleep_reports_elapsed_result) {
    ...
    const auto waitForElapsed = [&]() -> ruvia::Task<ruvia::TimerSleepResult> { co_return co_await ruvia::detail::Http2SansIoSleepAwaiter(worker, termination, std::chrono::milliseconds(0)); };

    asio::co_spawn(io, [&]() -> asio::awaitable<void> { observed = co_await ruvia::detail::taskAsAwaitable(waitForElapsed()); }, asio::detached);
    io.run();
...
RUVIA_TEST(http2_worker_shutdown_reports_typed_sleep_result) {
    ...
    Http2SansIoTermination termination;
    std::optional<ruvia::TimerSleepResult> observed;
    const auto waitForShutdown = [&]() -> ruvia::Task<ruvia::TimerSleepResult> { co_return co_await ruvia::detail::Http2SansIoSleepAwaiter(worker, termination, std::chrono::hours(1)); };

    asio::co_spawn(io, [&]() -> asio::awaitable<void> { observed = co_await ruvia::detail::taskAsAwaitable(waitForShutdown()); }, asio::detached);
...
RUVIA_TEST(http2_stream_sleep_observes_request_stop_token) {
    ...
    std::optional<ruvia::TimerSleepResult> observed;
    const auto waitForStop = [&]() -> ruvia::Task<ruvia::TimerSleepResult> { co_return co_await ruvia::detail::Http2SansIoSleepAwaiter(worker, termination, std::chrono::hours(1), token); };

    asio::co_spawn(io, [&]() -> asio::awaitable<void> { observed = co_await ruvia::detail::taskAsAwaitable(waitForStop()); }, asio::detached);
...
RUVIA_TEST(http2_session_termination_cancels_stream_sleep_with_exact_error) {
    ...
    const auto waitForTermination = [&]() -> ruvia::Task<ruvia::TimerSleepResult> { co_return co_await ruvia::detail::Http2SansIoSleepAwaiter(worker, termination, std::chrono::hours(1)); };
...
RUVIA_TEST(http2_websocket_transport_abort_remains_noexcept_when_reset_output_allocation_fails) {
    ...
    ruvia::detail::Http2SansIoWsTransport<asio::any_io_executor> transport(connection, 1, queue, streamSignal, writeSignal, asio::any_io_executor(io.get_executor()));

    std::pmr::string scratch(&resource);
    connection.takeOutput(scratch);
    char settings[ruvia::detail::kHttp2FrameHeaderBytes];
    ruvia::detail::http2EncodeFrameHeader(settings, 0, ruvia::detail::Http2FrameType::kSettings, 0, 0);
    RUVIA_CHECK(connection.feed(std::string_view(settings, sizeof(settings))) == ruvia::detail::Http2FeedResult::kAccepted);
    RUVIA_CHECK_EQ(connection.pendingOutput().size(), static_cast<std::size_t>(ruvia::detail::kHttp2FrameHeaderBytes));
...
RUVIA_TEST(http2_buffered_response_writer_reports_failure_when_reset_output_allocation_fails) {
    ...
    asio::co_spawn(
        ...
        [&]() -> asio::awaitable<void> {
            try {
                result = co_await ruvia::detail::taskAsAwaitable(writer.write(1, response, writePlan));
            } catch (const std::bad_alloc&) {
                threw = true;
            }
        },
