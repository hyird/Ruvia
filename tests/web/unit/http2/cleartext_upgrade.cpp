...
RUVIA_TEST(http2_cleartext_startup_accepts_only_prior_knowledge_preface) {
    ...
    RUVIA_CHECK(
        probeCleartextHttp2Preface("PRI * HT", false) == CleartextHttp2Probe::kNeedMorePreface);
    ...
    RUVIA_CHECK(
        probeCleartextHttp2Preface(kHttp2ClientPreface, true) == CleartextHttp2Probe::kHttp1);
}
