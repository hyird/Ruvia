...
ruvia::Task<ruvia::HttpResponse> tlsPongHandler(void* state, ruvia::Context& ctx) {
    ...
    observation.clientCertificateEmpty =
        info.tls() != nullptr && info.tls()->clientCertificateSubject().empty();
...
int serverAlpnSelect(SSL*, const unsigned char** out, unsigned char* outlen,
    const unsigned char* in, unsigned int inlen, void*) {
    static const unsigned char kH2[] = {2, 'h', '2'};
    if (SSL_select_next_proto(const_cast<unsigned char**>(out), outlen, kH2, sizeof(kH2), in,
    ...
    asio::co_spawn(
        ...
        [&]() -> asio::awaitable<void> {
            ...
            impl.registerRoute(ruvia::HttpKnownMethod::kGet,
                ...
                ruvia::detail::RouteHandler(&connectionObservation, &tlsPongHandler),
                ruvia::detail::RequestBodyMode::kBuffered,
                std::span<const ruvia::detail::ControllerMiddlewareDescriptor>{},
                std::span<const ruvia::detail::ControllerMiddlewareDescriptor>{});
            impl.finalize();
            co_await ruvia::detail::taskAsAwaitable(
                ruvia::test::runBareHttp2SansIoSession(tls, impl.routeTable(), worker,
                    ruvia::detail::ContextServices{}.withTlsTransport("127.0.0.1")));
        },
        asio::detached);
...
RUVIA_TEST(http_server_tls_verify_mode_optional_vs_mandatory) {
    const auto optional =
        ruvia::detail::httpServerTlsVerifyMode(ruvia::TlsClientCertificateRequirement::kOptional);
    RUVIA_CHECK(optional == asio::ssl::verify_peer);
    RUVIA_CHECK((optional & asio::ssl::verify_fail_if_no_peer_cert) == 0);

    const auto mandatory =
        ruvia::detail::httpServerTlsVerifyMode(ruvia::TlsClientCertificateRequirement::kRequired);
