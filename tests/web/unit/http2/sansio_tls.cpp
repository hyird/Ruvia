...
ruvia::Task<ruvia::HttpResponse> tlsPongHandler(void* state, ruvia::Context& ctx) {
    ...
    observation.clientCertificateEmpty = info.tls() != nullptr && info.tls()->clientCertificateSubject().empty();
...
int serverAlpnSelect(SSL*, const unsigned char** out, unsigned char* outlen, const unsigned char* in, unsigned int inlen, void*) {
    static const unsigned char kH2[] = {2, 'h', '2'};
    if (SSL_select_next_proto(const_cast<unsigned char**>(out), outlen, kH2, sizeof(kH2), in, inlen) != OPENSSL_NPN_NEGOTIATED) {
...
RUVIA_TEST(sansio_tls_alpn_h2_round_trip) {
    ...
    asio::co_spawn(
        ...
        [&]() -> asio::awaitable<void> {
            ...
            serverCtx.use_certificate(asio::buffer(pem.cert.data(), pem.cert.size()), asio::ssl::context::pem);
            serverCtx.use_private_key(asio::buffer(pem.key.data(), pem.key.size()), asio::ssl::context::pem);
            SSL_CTX_set_alpn_select_cb(serverCtx.native_handle(), &serverAlpnSelect, nullptr);

            asio::ssl::stream<tcp::socket&> tls(sock, serverCtx);
            auto [hsEc] = co_await tls.async_handshake(asio::ssl::stream_base::server, asio::as_tuple(asio::use_awaitable));
            if (hsEc) {
                co_return;
            }
    ...
    asio::co_spawn(
        ...
        [&]() -> asio::awaitable<void> {
            ...
            auto [hsEc] = co_await tls.async_handshake(asio::ssl::stream_base::client, asio::as_tuple(asio::use_awaitable));
            if (hsEc) {
                co_return;
            }

            auto writeAll = [&tls](std::string_view bytes) -> asio::awaitable<bool> {
                auto [ec, n] = co_await asio::async_write(tls, asio::buffer(bytes.data(), bytes.size()), asio::as_tuple(asio::use_awaitable));
                (void)n;
                co_return !ec;
            };
            auto readExact = [&tls](void* data, std::size_t size) -> asio::awaitable<bool> {
                auto [ec, n] = co_await asio::async_read(tls, asio::buffer(data, size), asio::as_tuple(asio::use_awaitable));
                co_return !ec && n == size;
            };
    ...
    RUVIA_CHECK(connectionObservation.remoteAddress == std::string_view("127.0.0.1"));
}
...
RUVIA_TEST(http_server_tls_verify_mode_optional_vs_mandatory) {
    const auto optional = ruvia::detail::httpServerTlsVerifyMode(ruvia::TlsClientCertificateRequirement::kOptional);
    RUVIA_CHECK(optional == asio::ssl::verify_peer);
    RUVIA_CHECK((optional & asio::ssl::verify_fail_if_no_peer_cert) == 0);

    const auto mandatory = ruvia::detail::httpServerTlsVerifyMode(ruvia::TlsClientCertificateRequirement::kRequired);
