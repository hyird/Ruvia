...
RUVIA_TEST(context_preserves_typed_connection_info_for_handler) {
    ...
    const auto plainContext =
        ContextAccess::make(memory, request, ContextServices{}.withPlainTransport("192.0.2.44"));
    ...
    const auto tlsContext = ContextAccess::make(
        memory, request, ContextServices{}.withTlsTransport("198.51.100.55", "CN=request-client"));
