...
RUVIA_TEST(buffered_response_compression_uses_sync_and_bounded_offload_thresholds) {
    ...
    const auto options = Compression{.minBytes = 1024, .syncBytes = 64 * 1024, .maxBytes = 64 * 1024 * 1024};

    auto small = responseWithBody(std::string(32 * 1024, 's'));
    const auto beforeSmall = pool.stats();
    auto smallResult = runCompressionTask(io, ruvia::detail::applyResponseCompressionAsync(coding, HttpKnownMethod::kGet, small, options, &pool, worker));
    ...
    auto largeResult = runCompressionTask(io, ruvia::detail::applyResponseCompressionAsync(coding, HttpKnownMethod::kGet, large, options, &pool, worker));
    ...
    auto synchronousFallbackResult = runCompressionTask(io, ruvia::detail::applyResponseCompressionAsync(coding, HttpKnownMethod::kGet, synchronousFallback, options, nullptr, worker));
    RUVIA_CHECK(synchronousFallbackResult.compressed());
    RUVIA_CHECK_EQ(gzipDecompress(responseBody(synchronousFallback).bytes()), synchronousFallbackPlain);

    pool.stop();
    pool.join();
}

RUVIA_TEST(buffered_response_compression_falls_back_to_identity_when_pool_rejects_or_body_is_too_large) {
    asio::io_context& io = ruvia::test::newTestIoContext();
    auto dispatcher = std::make_shared<ruvia::detail::WorkerDispatcher>(io, 8);
    const auto worker = ruvia::detail::WorkerHandleAccess::make(dispatcher);
    ...
    auto unavailableResult = runCompressionTask(io, ruvia::detail::applyResponseCompressionAsync(coding, HttpKnownMethod::kGet, unavailable, options, &pool, worker));
    ...
    auto oversized = responseWithBody(oversizedPlain);
    const auto capped = Compression{.minBytes = 1024, .syncBytes = 32 * 1024, .maxBytes = 64 * 1024};
    auto oversizedResult = runCompressionTask(io, ruvia::detail::applyResponseCompressionAsync(coding, HttpKnownMethod::kGet, oversized, capped, nullptr, worker));
...
RUVIA_TEST(streaming_compression_owns_one_typed_encoder_lifecycle) {
    ...
    const auto decoded = ruvia::decodeHttpContent(ruvia::HttpContentCoding::kGzip, encoded, {.maxDecodedBytes = kCompressibleBody.size(), .resource = std::pmr::get_default_resource()});
...
RUVIA_TEST(streaming_compression_failure_is_terminal) {
    ...
    auto response = responseWithBody(kCompressibleBody);
    ruvia::detail::HttpStreamingResponseCompression compression(&resource, gzipResponseCoding(), ruvia::detail::HttpResponseCodingAvailability::kIdentityAndCompression);
    compression.prepare(HttpKnownMethod::kGet, response, ruvia::detail::ResponseStreamKind::kGeneric);
    compression.activate(ruvia::detail::httpResponseBodyPlan(HttpKnownMethod::kGet, response.status()));
    RUVIA_CHECK(compression.active());
...
RUVIA_TEST(streaming_compression_precommit_abort_is_terminal) {
    auto response = responseWithBody(kCompressibleBody);
    ruvia::detail::HttpStreamingResponseCompression compression(std::pmr::get_default_resource(), gzipResponseCoding(), ruvia::detail::HttpResponseCodingAvailability::kIdentityAndCompression);
    compression.prepare(HttpKnownMethod::kGet, response, ruvia::detail::ResponseStreamKind::kGeneric);
    compression.abort();
...
RUVIA_TEST(streaming_compression_respects_encoder_availability_at_representation_boundary) {
    ...
    auto response = responseWithBody(kCompressibleBody);
    ruvia::detail::HttpStreamingResponseCompression compression(std::pmr::get_default_resource(), selection, ruvia::detail::HttpResponseCodingAvailability::kIdentityOnly);
    ...
    if (const auto* selected = allowedSelection.selected()) {
        ruvia::detail::HttpStreamingResponseCompression identityFallback(std::pmr::get_default_resource(), *selected, ruvia::detail::HttpResponseCodingAvailability::kIdentityOnly);
...
RUVIA_TEST(response_compression_preflight_rejects_non_transformable_metadata) {
    const auto selection = gzipResponseCoding();
    const auto eligible = [](const HttpResponse& response) { return ruvia::detail::httpResponseCompressionEligibility(gzipResponseCoding(), HttpKnownMethod::kGet, response, ruvia::detail::ResponseStreamKind::kGeneric) == ruvia::detail::HttpResponseCompressionEligibility::kEligible; };
    ...
    RUVIA_CHECK(ruvia::detail::httpResponseCompressionEligibility(selection, HttpKnownMethod::kGet, responseWithBody(kCompressibleBody), ruvia::detail::ResponseStreamKind::kGeneric) == ruvia::detail::HttpResponseCompressionEligibility::kEligible);
}
...
RUVIA_TEST(preencoded_response_must_be_acceptable_to_client) {
    ...
    if (const auto* selected = negotiation.selected()) {
        ...
        ruvia::detail::HttpStreamingResponseCompression streamCompression(std::pmr::get_default_resource(), *selected, ruvia::detail::HttpResponseCodingAvailability::kIdentityAndCompression);
        bool rejected = false;
        try {
        ...
        ruvia::detail::HttpStreamingResponseCompression stackedCompression(std::pmr::get_default_resource(), *selected, ruvia::detail::HttpResponseCodingAvailability::kIdentityAndCompression);
        ...
        if (const auto* identitySelection = identityNegotiation.selected()) {
            ...
            ruvia::detail::HttpStreamingResponseCompression identityCompression(std::pmr::get_default_resource(), *identitySelection, ruvia::detail::HttpResponseCodingAvailability::kIdentityAndCompression);
