...
int main() {
    ...
    ruvia::detail::WebWorkerRuntime server(ruvia::detail::HttpServerListenerDefinition(asio::ip::tcp::endpoint(asio::ip::make_address("127.0.0.1"), 0), std::move(tls)), routerImpl.routeTable(), {}, std::move(options));
    server.start();
