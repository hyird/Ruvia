...
private:
    ...
    static bool readClientControlFrame(asio::ip::tcp::socket& socket, std::uint8_t expectedOpcode) {
        std::array<unsigned char, 2> head{};
        asio::read(socket, asio::buffer(head));
        if ((head[0] & 0x8FU) != (0x80U | expectedOpcode) || (head[1] & 0x80U) == 0 ||
            (head[1] & 0x7FU) > 125) {
            return false;
        }
        const auto size = static_cast<std::size_t>(head[1] & 0x7FU);
...
ruvia::Task<int> exercise(ruvia::WebSocketClient& client, ruvia::WorkerId expectedWorker) {
    ...
    }
    {
        ...
        try {
            ...
            competingReadRejected =
                error.code() == ruvia::WebSocketClientError::Code::kInvalidState;
        }
...
ruvia::Task<int> exerciseHeartbeat(ruvia::WebSocketClient& client, bool expectTimeout) {
    ...
    try {
        const auto message = co_await client.read();
        client.abort();
        result = expectTimeout || !message.has_value() || !message->text() ||
        ...
        client.abort();
        result =
            expectTimeout && error.code() == ruvia::WebSocketClientError::Code::kTimeout ? 0 : 2;
    }
    co_await client.shutdown();
    co_return result;
}
...
ruvia::Task<int> exerciseReadTimeout(ruvia::WebSocketClient& client) {
    co_await client.connect();
    int result = 1;
    try {
        static_cast<void>(
            co_await client.withOptions({.timeout = std::chrono::milliseconds{50}}).read());
    } catch (const ruvia::WebSocketClientError& error) {
        result = error.code() == ruvia::WebSocketClientError::Code::kTimeout ? 0 : 2;
    }
    client.abort();
...
ruvia::Task<int> exerciseShutdownWhileReading(
    ruvia::EventLoop loop, ruvia::WebSocketClient& client) {
    co_await client.connect();
    ruvia::detail::WorkerSignal readStarted(client.worker());
    ruvia::detail::asyncStartTask(consumeUntilShutdown(client, readStarted),
        asio::bind_executor(loop.executor(), [](ruvia::detail::TaskCompletionResult<void> result) {
            if (result.failure()) {
                std::terminate();
            }
        }));
...
ruvia::Task<int> exerciseAll(ruvia::EventLoop loop, ruvia::WebSocketClient& client,
    ...
    const auto readTimeoutResult = co_await exerciseReadTimeout(readTimeoutClient);
    if (readTimeoutResult != 0) {
        co_return 15 + readTimeoutResult;
    }
