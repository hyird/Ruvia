...
std::vector<std::string> logPipelinedBurst(const asio::ip::tcp::endpoint& endpoint,
    const AccessLogListener& listener, std::string_view pipelined, std::size_t expected) {
...
int main() {
    ...
    options.accessLog.callback =
        ruvia::detail::CallbackAccess::bind<void(const ruvia::AccessLogRecord&) noexcept>(listener);
    ...
    // other failing first.
    int failures = 0;
    const auto check = [&failures](const char* label, const std::vector<std::string>& got,
                           std::size_t at, std::string_view wantFirst,
        ...
        std::fprintf(stderr,
            ...
            "\"%s\"}, want {\"%.*s\", \"%.*s\"}\n",
            label, got[at].c_str(), got[at + 1].c_str(), static_cast<int>(wantFirst.size()),
            wantFirst.data(), static_cast<int>(wantSecond.size()), wantSecond.data());
        ++failures;
    };
