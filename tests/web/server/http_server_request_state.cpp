...
template <typename T>
concept HasUncheckedPreparedStreamExtraction =
...
static_assert(
    !HasAnyRvaluePreparedResponseStreamBorrow<ruvia::detail::PreparedHttp1ResponseStream>);
static_assert(
    !HasUncheckedPreparedStreamExtraction<ruvia::detail::PreparedHttp1ResponseStreamResult>);
static_assert(HasValueSemanticResponseBodyPlan<ruvia::detail::ResponseStreamCommitPlan>);

ruvia::detail::PreparedHttp1ResponseStream prepareStream(ruvia::HttpResponse response,
    ResponseStreamKind kind, const ruvia::detail::Http1ResponseStreamPlan& plan,
...
RUVIA_TEST(request_state_route_body_limit_only_tightens) {
    ...
    const auto boundedStream =
        requestBodyByteLimit(RequestBodyMode::kStream, std::nullopt, 1024, 32);
    RUVIA_CHECK(boundedStream.maximum().has_value());
    RUVIA_CHECK_EQ(boundedStream.maximum().value(), std::size_t{32});
    RUVIA_CHECK(!requestBodyByteLimit(RequestBodyMode::kStream, std::nullopt, 1024, 0)
            .maximum()
            .has_value());

    // And narrows an explicit stream limit the same way.
    const auto tightenedStream =
        requestBodyByteLimit(RequestBodyMode::kStream, std::optional<std::size_t>{2048}, 1024, 64);
    RUVIA_CHECK_EQ(tightenedStream.maximum().value(), std::size_t{64});
    const auto clampedStream = requestBodyByteLimit(
        RequestBodyMode::kStream, std::optional<std::size_t>{2048}, 1024, 8192);
...
RUVIA_TEST(request_state_keep_alive_by_connection_header) {
    ...
    RUVIA_CHECK(parser.parseMessage("GET / HTTP/1.1\r\nHost: x\r\nConnection: keep-alive\r\n\r\n")
                    .connectionPlan.disposition() == Http1ClosePolicy::kAllowReuse);
}
...
RUVIA_TEST(request_state_keep_alive_default_by_version) {
    ...
    RUVIA_CHECK(http11.disposition() == Http1ClosePolicy::kAllowReuse);
    ...
    RUVIA_CHECK(http10Default.disposition() == Http1ClosePolicy::kCloseAfterResponse);
    ...
    const auto http10KeepAlive =
        parser.parseMessage("GET / HTTP/1.0\r\nConnection: keep-alive\r\n\r\n").connectionPlan;
    RUVIA_CHECK(http10KeepAlive.disposition() == Http1ClosePolicy::kAllowReuse);
    ...
    RUVIA_CHECK(incompleteHttp10.connectionPlan.disposition() == Http1ClosePolicy::kAllowReuse);
    RUVIA_CHECK(
        incompleteHttp10.connectionPlan.protocolVersion() == ruvia::HttpProtocolVersion::kHttp10);
    ...
    RUVIA_CHECK(failedHttp10.connectionPlan.disposition() == Http1ClosePolicy::kCloseAfterResponse);
    RUVIA_CHECK(
        failedHttp10.connectionPlan.protocolVersion() == ruvia::HttpProtocolVersion::kHttp10);
}
...
RUVIA_TEST(request_state_wants_continue) {
    ...
    const auto emptyPlan =
        empty.bodyPlan.expectationPlan(HttpUnsupportedExpectationPolicy::kReject);
    ...
    const auto bodyExpectationPlan =
        body.bodyPlan.expectationPlan(HttpUnsupportedExpectationPolicy::kReject);
    ...
    const auto http10 = parser.parseMessage(
        "POST / HTTP/1.0\r\nHost: x\r\n"
        "Expect: 100-continue\r\nContent-Length: 0\r\n\r\n");
    const auto http10Plan =
        http10.bodyPlan.expectationPlan(HttpUnsupportedExpectationPolicy::kReject);
    ...
    const auto extensionPlan =
        extension.bodyPlan.expectationPlan(HttpUnsupportedExpectationPolicy::kReject);
...
RUVIA_TEST(http1_response_stream_plan_owns_version_body_and_persistence_semantics) {
    ...
    RUVIA_CHECK(http11.requestConnectionPlan().disposition() == Http1ClosePolicy::kAllowReuse);
    RUVIA_CHECK(http11.closePolicy() == Http1ClosePolicy::kAllowReuse);
    RUVIA_CHECK(
        http11.requestConnectionPlan().protocolVersion() == ruvia::HttpProtocolVersion::kHttp11);

    const auto limitedHttp11 =
        http1PlanResponseStream(parser.parseMessage("GET / HTTP/1.1\r\nHost: x\r\n\r\n"),
            Http1ClosePolicy::kCloseAfterResponse);
    RUVIA_CHECK(limitedHttp11.framing() == ResponseStreamFraming::kHttp1Chunked);
    RUVIA_CHECK(
        limitedHttp11.requestConnectionPlan().disposition() == Http1ClosePolicy::kAllowReuse);
    RUVIA_CHECK(limitedHttp11.closePolicy() == Http1ClosePolicy::kCloseAfterResponse);
    ...
    RUVIA_CHECK(requestBodyPending.requestConnectionPlan().disposition() ==
                Http1ClosePolicy::kCloseAfterResponse);
    ...
    RUVIA_CHECK(
        http10KeepAlive.requestConnectionPlan().disposition() == Http1ClosePolicy::kAllowReuse);
    RUVIA_CHECK(http10KeepAlive.closePolicy() == Http1ClosePolicy::kAllowReuse);
    RUVIA_CHECK(http10KeepAlive.requestConnectionPlan().protocolVersion() ==
                ruvia::HttpProtocolVersion::kHttp10);
}

RUVIA_TEST(http1_prepared_stream_head_binds_wire_signal_to_final_connection_disposition) {
    Http1ServerRequestParser parser;
    const auto prepare = [&](std::string_view request, Http1ClosePolicy closePolicy,
        ...
        auto prepared = prepareStream(
            std::move(response), ResponseStreamKind::kGeneric, plan, ResponseTrailerIntent::kNone);
        return std::tuple(prepared.connectionPlan().disposition(),
    ...
    const auto reusable = prepare(http11, Http1ClosePolicy::kAllowReuse, {});
    RUVIA_CHECK(std::get<0>(reusable) == Http1ClosePolicy::kAllowReuse);
    ...
    const auto applicationClose = prepare(http11, Http1ClosePolicy::kAllowReuse, "close");
    RUVIA_CHECK(std::get<0>(applicationClose) == Http1ClosePolicy::kCloseAfterResponse);
    ...
    const auto runtimeClose = prepare(http11, Http1ClosePolicy::kCloseAfterResponse, "keep-alive");
    RUVIA_CHECK(std::get<0>(runtimeClose) == Http1ClosePolicy::kCloseAfterResponse);
    ...
    const auto closeDelimited = prepare("GET / HTTP/1.0\r\nConnection: keep-alive\r\n\r\n",
        Http1ClosePolicy::kAllowReuse, "keep-alive");
    RUVIA_CHECK(std::get<0>(closeDelimited) == Http1ClosePolicy::kCloseAfterResponse);
...
RUVIA_TEST(http1_prepared_stream_head_owns_exact_wire_framing) {
    ...
    const auto prepare = [&](std::string_view request) {
        const auto plan =
            http1PlanResponseStream(parser.parseMessage(request), Http1ClosePolicy::kAllowReuse);
        ruvia::HttpResponse response({.resource = std::pmr::get_default_resource()});
        response.status(ruvia::http_status::kOk);
    ...
    RUVIA_CHECK(http10.connectionPlan().disposition() == Http1ClosePolicy::kCloseAfterResponse);
    ...
    RUVIA_CHECK_EQ(
        std::string(http11.response().header("Transfer-Encoding").value_or(std::string_view{})),
        std::string("chunked"));
...
RUVIA_TEST(http1_prepared_body_suppressed_stream_is_self_delimited) {
    Http1ServerRequestParser parser;
    const auto prepare = [&](std::string_view request, ruvia::HttpStatusCode status,
                             Http1ClosePolicy closePolicy) {
        const auto plan = http1PlanResponseStream(parser.parseMessage(request), closePolicy);
        ruvia::HttpResponse response({.resource = std::pmr::get_default_resource()});
    ...
    RUVIA_CHECK(
        http11.commitPlan().headDisposition() == ResponseStreamHeadDisposition::kMessageEnded);
    ...
    RUVIA_CHECK(http11.connectionPlan().disposition() == Http1ClosePolicy::kAllowReuse);

    auto http10 = prepare("GET / HTTP/1.0\r\nConnection: keep-alive\r\n\r\n",
        ruvia::http_status::kResetContent, Http1ClosePolicy::kAllowReuse);
    RUVIA_CHECK(http10.commitPlan().bodyPlan().bodySuppressed());
    RUVIA_CHECK(!http10.response().header("Transfer-Encoding").has_value());
    RUVIA_CHECK(http10.connectionPlan().disposition() == Http1ClosePolicy::kAllowReuse);
    RUVIA_CHECK_EQ(std::string(http10.response().header("Connection").value_or(std::string_view{})),
        std::string("keep-alive"));

    auto http10Head = prepare("HEAD / HTTP/1.0\r\nConnection: keep-alive\r\n\r\n",
        ruvia::http_status::kOk, Http1ClosePolicy::kAllowReuse);
    RUVIA_CHECK(http10Head.commitPlan().bodyPlan().bodySuppressed());
    RUVIA_CHECK(http10Head.connectionPlan().disposition() == Http1ClosePolicy::kAllowReuse);

    auto limitedHttp10 = prepare("GET / HTTP/1.0\r\nConnection: keep-alive\r\n\r\n",
        ruvia::http_status::kResetContent, Http1ClosePolicy::kCloseAfterResponse);
    RUVIA_CHECK(limitedHttp10.commitPlan().bodyPlan().bodySuppressed());
    RUVIA_CHECK(
        limitedHttp10.connectionPlan().disposition() == Http1ClosePolicy::kCloseAfterResponse);
    RUVIA_CHECK_EQ(
        std::string(limitedHttp10.response().header("Connection").value_or(std::string_view{})),
        std::string("close"));
}
...
RUVIA_TEST(http1_stream_commit_plan_exposes_exact_trailer_capability) {
    ...
    const auto prepare = [&parser](std::string_view request) {
        ...
        ruvia::HttpResponse response({.resource = std::pmr::get_default_resource()});
        response.status(ruvia::http_status::kOk);
        return prepareStream(std::move(response), ResponseStreamKind::kGeneric, plan,
    ...
    RUVIA_CHECK(
        http11.commitPlan().trailerFraming() == ResponseStreamTrailerFraming::kHttp1Chunked);
    ...
    RUVIA_CHECK(
        head.commitPlan().headDisposition() == ResponseStreamHeadDisposition::kMessageEnded);
...
RUVIA_TEST(auto_https_append_port_omits_default) {
    ...
    RUVIA_CHECK_EQ(
        withHttpsPort("https://example.com", 8443), std::string("https://example.com:8443"));
...
RUVIA_TEST(auto_https_redirect_response_is_private_and_well_formed) {
    ...
    RUVIA_CHECK_EQ(std::string(response.header("Location").value_or(std::string_view{})),
    ...
    RUVIA_CHECK_EQ(std::string(response.header("Cache-Control").value_or(std::string_view{})),
        std::string("private"));
    ...
    const auto commit =
        ruvia::detail::http1CommitFinalResponse(response, parsed.connectionPlan.requireClose());
    RUVIA_CHECK(commit.committed() != nullptr);
    if (commit.committed() == nullptr) {
        return;
    }
    RUVIA_CHECK(commit.committed()->disposition() == ruvia::Http1ClosePolicy::kCloseAfterResponse);
