...
concept AcceptsTemporaryPipelineRestore =
    requires(ruvia::detail::Http1ServerConnectionPlan connectionPlan, Pipeline&& pipeline) {
        Http1SessionRequestCompletion::makeBufferedPipelineRestore(
            connectionPlan, std::forward<Pipeline>(pipeline));
    };
...
static_assert(
    std::same_as<decltype(std::declval<const Http1SessionRequestCompletion&>().bufferedResponse()),
        const Http1BufferedResponseReady*>);
static_assert(
    std::same_as<decltype(std::declval<const Http1SessionRequestCompletion&>().committedStream()),
        const Http1CommittedStreamResponse*>);
static_assert(
    std::same_as<decltype(std::declval<const Http1RequestBufferCompletion&>().compaction()),
...
RUVIA_TEST(http1_session_request_completion_owns_wire_and_buffer_outcome) {
    ...
    const auto committed = Http1SessionRequestCompletion::makeCommittedStream(
        reusablePlan, ruvia::http_status::kMultiStatus, 19);
...
RUVIA_TEST(http1_session_request_completion_discriminates_close_and_restore) {
    ...
    const auto unshiftedClosing =
        Http1SessionRequestCompletion::makeBufferedUnrestored(closePlan, 999);
    ...
    const auto committedClosing = Http1SessionRequestCompletion::makeCommittedStream(
        closePlan, ruvia::http_status::kServiceUnavailable, 999);
    ...
    const auto restore = Http1SessionRequestCompletion::makeBufferedPipelineRestore(
        reusableHttp11Plan(), "GET /next HTTP/1.1\r\n\r\n");
    ...
    RUVIA_CHECK_EQ(restore.bufferCompletion().pipelineRestore()->pipeline(),
...
RUVIA_TEST(http1_request_buffer_completion_applies_exactly_one_cleanup) {
    ...
    const auto compacted =
        Http1SessionRequestCompletion::makeBufferedUnrestored(reusableHttp11Plan(), 3);
...
RUVIA_TEST(http1_request_buffer_completion_handles_deterministic_pipeline_remainders) {
    const auto reusablePlan = reusableHttp11Plan();
    std::uint64_t state = 0x6d2b'38a1'f407'c59eULL;
    ...
    for (std::size_t sample = 0; sample < 2048; ++sample) {
        std::string raw(static_cast<std::size_t>(next() % 257U), '\0');
        for (auto& byte : raw) {
            byte = static_cast<char>(next());
        }
        auto readBuffer = buffer(raw);
        std::size_t usedBytes = raw.size();
        const auto consumed = usedBytes == 0 ? std::size_t{0}
                                             : static_cast<std::size_t>(next() % (usedBytes + 1U));
        const auto compacted =
            Http1SessionRequestCompletion::makeBufferedUnrestored(reusablePlan, consumed);
        applyReusableHttp1RequestBufferCompletion(
            compacted.bufferCompletion(), readBuffer, usedBytes);
        RUVIA_CHECK_EQ(usedBytes, raw.size() - consumed);
        RUVIA_CHECK_EQ(live(readBuffer, usedBytes), std::string_view(raw).substr(consumed));

        std::string pipeline(static_cast<std::size_t>(next() % 513U), '\0');
        for (auto& byte : pipeline) {
            byte = static_cast<char>(next());
        }
        const auto restored =
            Http1SessionRequestCompletion::makeBufferedPipelineRestore(reusablePlan, pipeline);
        applyReusableHttp1RequestBufferCompletion(restored.bufferCompletion(), readBuffer, usedBytes);
        RUVIA_CHECK_EQ(usedBytes, pipeline.size());
        RUVIA_CHECK_EQ(live(readBuffer, usedBytes), std::string_view(pipeline));
        RUVIA_CHECK(readBuffer.size() >= pipeline.size());
    }
}
...
RUVIA_TEST(accepted_connection_lease_rolls_back_throwing_initiation) {
    ...
    try {
        ...
        const auto throwingInitiation = [](AcceptedConnectionLease) -> void {
