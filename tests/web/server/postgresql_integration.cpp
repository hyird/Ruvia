...
[[nodiscard]] std::string_view environment(
    const char* name,
...
ruvia::Task<void> withDatabase(
    ...
    const std::array<ruvia::DbValue, 4> params{
        ruvia::DbValue{"hello"}, ruvia::DbValue{-42},
        ruvia::DbValue{true}, ruvia::DbValue{nullptr}};
    auto typed = co_await db.query(
        "SELECT $1::text, $2::bigint, $3::boolean, $4::text IS NULL",
        std::span<const ruvia::DbValue>(params));
    require(typed.rows().size() == 1 && typed.rows()[0].size() == 4,
    ...
    require(reconnected.rows()[0][0].text() == "1",
...
int main() {
    try {
        ...
        runTask([&](asio::io_context& ioContext) {
            return withDatabase(ioContext, config, false);
        });
        runTask([&](asio::io_context& ioContext) {
            return withDatabase(ioContext, config, true);
        });
