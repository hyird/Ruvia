...
void registerRoute(ruvia::detail::RouterImpl& router, ruvia::HttpKnownMethod method, std::string_view path, Handler& handler, std::span<const ruvia::detail::ControllerMiddlewareDescriptor> middlewares) {
    router.registerRoute(method, std::pmr::string(path, std::pmr::get_default_resource()), ruvia::detail::makeCallableRef<ruvia::HttpResponse, ruvia::Context&>(handler), ruvia::detail::RequestBodyMode::kBuffered, {}, middlewares);
}
...
// simply finished".
[[nodiscard]] std::string exchange(const asio::ip::tcp::endpoint& endpoint, std::string_view request, std::chrono::steady_clock::duration& elapsed) {
    asio::io_context ctx;
...
int main() {
    ...
    registerRoute(routerImpl, ruvia::HttpKnownMethod::kPost, "/small", echo, std::span(&smallBody, std::size_t{1}));
    registerRoute(routerImpl, ruvia::HttpKnownMethod::kGet, "/route-deadline", slow, std::span(&shortDeadline, std::size_t{1}));
...

    {
        ...
        const auto ok = exchange(endpoint, "POST /small HTTP/1.1\r\nHost: x\r\nContent-Length: 5\r\n\r\nhello", elapsed);
    ...
    if (result == 0) {
        ...
        const auto tooLarge = exchange(endpoint, "POST /small HTTP/1.1\r\nHost: x\r\nContent-Length: 64\r\n\r\n" + std::string(64, 'x'), elapsed);
    ...
    if (result == 0) {
        const auto routeDeadline = exchange(endpoint, "GET /route-deadline HTTP/1.1\r\nHost: x\r\n\r\n", elapsed);
    ...
    if (result == 0) {
        const auto appDeadline = exchange(endpoint, "GET /app-deadline HTTP/1.1\r\nHost: x\r\n\r\n", elapsed);
