...
RUVIA_RESPONSE_MODEL(SurfaceJsonResponse, RUVIA_OPTIONAL_FIELD(message, ruvia::String), RUVIA_OPTIONAL_FIELD(nullable, ruvia::String, RUVIA_EMIT_NULL));

RUVIA_RESPONSE_MODEL(RequiredFieldSurface, RUVIA_REQUIRED_FIELD(id, ruvia::UInt32), RUVIA_OPTIONAL_FIELD(label, ruvia::String));

struct DirectRequestModel final : ruvia::RequestModel<DirectRequestModel, RUVIA_OPTIONAL_FIELD(message, ruvia::String)> {
    using RuviaModelBase::RuviaModelBase;
};

struct DirectResponseModel final : ruvia::ResponseModel<DirectResponseModel, RUVIA_OPTIONAL_FIELD(message, ruvia::String)> {
    using RuviaModelBase::RuviaModelBase;
};
...
template <typename T>
concept HasPublicParseHook = requires { T::ruviaParseJsonBody(std::string_view{}, static_cast<std::pmr::memory_resource*>(nullptr)); };

template <typename T>
concept HasPublicWriterHook = requires(const T& model, std::pmr::string& output) { model.ruviaAppendJson(output); };

template <typename T>
concept HasPublicFieldStateHook = requires(const T& model) { model.template ruviaFieldState<"message">(); };

template <typename String>
concept AcceptsTemporaryRuleMessage = requires(String&& value) { ruvia::detail::model::Required{std::forward<String>(value)}; };

template <typename T>
concept HasModelOptionsConstructor = requires(std::pmr::memory_resource* resource) { T(ruvia::ModelOptions{.resource = resource}); };

template <typename T>
concept HasPositionalModelResourceConstructor = requires(std::pmr::memory_resource* resource) { T(resource); };

template <typename T>
concept HasPositionalStringValueResourceConstructor = requires(std::pmr::memory_resource* resource) { T(std::string_view{}, resource); };

template <typename T>
concept HasModelStringOptionsConstructor = requires(std::pmr::memory_resource* resource) { T(std::string_view{}, ruvia::ModelOptions{.resource = resource}); };

template <typename Parser>
concept HasPositionalModelObjectParseResource = requires(std::pmr::memory_resource* resource) { Parser::parse(std::string_view{}, resource); };

template <typename Parser>
concept HasModelObjectParseOptions = requires(std::pmr::memory_resource* resource) { Parser::parse(std::string_view{}, ruvia::ModelParseOptions{.resource = resource}); };
...
static_assert(std::same_as<decltype(std::declval<const RequiredFieldSurface&>().template get<"id">()), const ruvia::UInt32&>);
static_assert(std::same_as<decltype(std::declval<const RequiredFieldSurface&>().template get<"label">()), const std::optional<ruvia::String>&>);
