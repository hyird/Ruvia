...
concept HasRvalueWebSocketClientOperation =
    ...
    requires(Client&& client) { std::move(client).read(); } ||
    requires(Client&& client) { std::move(client).text(std::string_view{}); } ||
    requires(Client&& client) { std::move(client).binary(std::string_view{}); } ||
    requires(Client&& client) { std::move(client).ping(); } ||
    requires(Client&& client) { std::move(client).pong(std::string_view{}); } ||
    requires(Client&& client) { std::move(client).close(ruvia::WebSocketCloseOptions{}); };
...
static_assert(std::same_as<decltype(ruvia::WebSocketClientConfig{}.heartbeat),
...
static_assert(
    std::same_as<decltype(std::declval<ruvia::WebSocketClient&>().shutdown()), ruvia::Task<void>>);
