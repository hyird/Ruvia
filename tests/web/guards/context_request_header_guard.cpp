...
concept ExposesRvalueRequestFormObjectGroups =
    requires { std::declval<const Object&&>().groups(); };
...
concept AcceptsRequestFieldName = requires(Input&& input) {
    ruvia::detail::RequestNameValueViewAccess::make(std::forward<Input>(input), std::string_view{});
};

template <typename Input>
concept AcceptsRequestFieldValue = requires(Input&& input) {
    ruvia::detail::RequestNameValueViewAccess::make(std::string_view{}, std::forward<Input>(input));
};

static_assert(sizeof(ruvia::ContextRequest) == sizeof(void*));
static_assert(std::same_as<decltype(std::declval<const ruvia::ContextRequest&>().method()),
    std::string_view>);
static_assert(std::same_as<decltype(std::declval<const ruvia::ContextRequest&>().parseBody()),
