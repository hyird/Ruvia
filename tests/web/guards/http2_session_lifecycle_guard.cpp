...
bool cleanShutdown() {
    ...
    if (lifecycle.phase() != Http2SansIoSessionPhase::kRunning || lifecycle.stopping() || lifecycle.writeFailed() || lifecycle.writerDone()) {
        return false;
    }
    lifecycle.beginStopping();
    if (lifecycle.phase() != Http2SansIoSessionPhase::kStopping || !lifecycle.stopping() || lifecycle.writerDone()) {
        return false;
    }
...
bool failedWriteThenShutdown() {
    ...
    if (lifecycle.phase() != Http2SansIoSessionPhase::kWriteFailed || !lifecycle.writeFailed() || lifecycle.stopping()) {
        return false;
    }
    lifecycle.beginStopping();
    if (lifecycle.phase() != Http2SansIoSessionPhase::kStoppingAfterWriteFailure || !lifecycle.writeFailed() || !lifecycle.stopping()) {
        return false;
    }
    lifecycle.markWriterDone();
    lifecycle.markWriteFailed();
