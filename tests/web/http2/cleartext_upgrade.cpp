...
RUVIA_TEST(http2_cleartext_startup_accepts_only_prior_knowledge_preface) {
    RUVIA_CHECK(probeCleartextHttp2Preface("GET / HTTP/1.1\r\n", false) == CleartextHttp2Probe::kHttp1);
    RUVIA_CHECK(probeCleartextHttp2Preface("PRI * HT", false) == CleartextHttp2Probe::kNeedMorePreface);
    RUVIA_CHECK(probeCleartextHttp2Preface(kHttp2ClientPreface, false) == CleartextHttp2Probe::kCompletePreface);
    RUVIA_CHECK(probeCleartextHttp2Preface("PRI /not-http2\r\n", false) == CleartextHttp2Probe::kDropConnection);
    ...
    RUVIA_CHECK(probeCleartextHttp2Preface(kHttp2ClientPreface, true) == CleartextHttp2Probe::kHttp1);
}
...
RUVIA_TEST(drop_invalid_cleartext_by_version_token) {
    ...
    RUVIA_CHECK(!shouldDropInvalidCleartextHttp1Input("no-line-break", Http1ServerRequestParseFailureSource::kRequestLine));
