...
static_assert(std::same_as<decltype(std::declval<const ruvia::detail::Http2BufferedResponseWriteResult&>().committedStatus()), std::optional<ruvia::HttpStatusCode>>);
...
RUVIA_TEST(http2_buffered_response_write_result_preserves_terminal_cause) {
    ...
    RUVIA_CHECK_EQ(completed.committedStatus(), std::optional<ruvia::HttpStatusCode>{ruvia::http_status::kMultiStatus});
    ...
    RUVIA_CHECK_EQ(peerAfter.committedStatus(), std::optional<ruvia::HttpStatusCode>{ruvia::http_status::kAlreadyReported});
    ...
    RUVIA_CHECK_EQ(failedAfter.committedStatus(), std::optional<ruvia::HttpStatusCode>{ruvia::HttpStatusCode::fromValue(209)});
}
