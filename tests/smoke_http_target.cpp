...
int main() {
    ...
    if (parsed == nullptr || parsed->request().path() != "/" ||
        !parsed->wireBody().empty()) {
        return 1;
    }
    ...
    const auto requestPlan = ruvia::Http1ClientRequestWriter().prepare(
        origin, outboundRequest, requestHead);
    ...
    const auto responseResult = responseParser.parse(
