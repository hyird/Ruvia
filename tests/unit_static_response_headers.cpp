...
RUVIA_TEST(static_header_content_type_variants_split_correctly) {
    checkInterned(ruvia_ctx, ruvia::detail::kResponseHeaderContentType, "application/json", "Content-Type");
    checkInterned(ruvia_ctx, ruvia::detail::kResponseHeaderContentType, "text/plain; charset=UTF-8", "Content-Type");
    checkInterned(ruvia_ctx, ruvia::detail::kResponseHeaderContentType, "text/html; charset=UTF-8", "Content-Type");
    checkInterned(ruvia_ctx, ruvia::detail::kResponseHeaderContentType, "text/event-stream", "Content-Type");
    checkInterned(ruvia_ctx, ruvia::detail::kResponseHeaderContentType, "image/svg+xml", "Content-Type");
    // Last entry in the chain.
    checkInterned(ruvia_ctx, ruvia::detail::kResponseHeaderContentType, "application/octet-stream", "Content-Type");
}
...
RUVIA_TEST(static_header_single_value_names) {
    ...
    checkInterned(ruvia_ctx, ruvia::detail::kResponseHeaderContentEncoding, "gzip", "Content-Encoding");
    checkInterned(ruvia_ctx, ruvia::detail::kResponseHeaderTransferEncoding, "chunked", "Transfer-Encoding");
    checkInterned(ruvia_ctx, ruvia::detail::kResponseHeaderCacheControl, "no-store", "Cache-Control");
}
...
RUVIA_TEST(static_header_unknown_value_or_bit_is_nullopt) {
    ...
    RUVIA_CHECK(!builtinStaticResponseHeader(ruvia::detail::kResponseHeaderContentType, "text/markdown").has_value());
    RUVIA_CHECK(!builtinStaticResponseHeader(ruvia::detail::kResponseHeaderConnection, "keep-alive").has_value());
    RUVIA_CHECK(!builtinStaticResponseHeader(ruvia::detail::kResponseHeaderVary, "User-Agent").has_value());
