...
RUVIA_TEST(ws_heartbeat_disabled_stays_idle) {
    ...
    RUVIA_CHECK(decide(
        options(0, 0), WsLivenessMode::kOpen, WebSocketLivenessIdle{}, false, 0, 10000) ==
        WebSocketLivenessDecision::kIdle);
    RUVIA_CHECK(decide(
        options(1000, 500), WsLivenessMode::kInactive, WebSocketLivenessIdle{}, false, 0, 10000) ==
        WebSocketLivenessDecision::kIdle);
}
...
RUVIA_TEST(ws_heartbeat_sends_ping_when_idle) {
    ...
    RUVIA_CHECK(decide(
        options(1000, 500), WsLivenessMode::kOpen, WebSocketLivenessIdle{}, false, 0, 2000) ==
        WebSocketLivenessDecision::kSendPing);
    // Recent activity keeps it idle.
    RUVIA_CHECK(decide(
        options(1000, 500), WsLivenessMode::kOpen, WebSocketLivenessIdle{}, false, 1500, 2000) ==
        WebSocketLivenessDecision::kIdle);
    // A write in flight defers the ping.
    RUVIA_CHECK(decide(
        options(1000, 500), WsLivenessMode::kOpen, WebSocketLivenessIdle{}, true, 0, 2000) ==
        WebSocketLivenessDecision::kIdle);
}
...
RUVIA_TEST(ws_heartbeat_pong_timeout) {
    ...
    RUVIA_CHECK(decide(
        options(1000, 500), WsLivenessMode::kOpen, WebSocketAwaitingPong(1000), false, 0, 1600) ==
        WebSocketLivenessDecision::kAbortTransport);
    // Still within the pong timeout -> idle.
    RUVIA_CHECK(decide(
        options(1000, 500), WsLivenessMode::kOpen, WebSocketAwaitingPong(1000), false, 0, 1400) ==
        WebSocketLivenessDecision::kIdle);
    // The one-argument policy uses the ping interval as the pong timeout.
    RUVIA_CHECK(decide(
        options(1000, 0), WsLivenessMode::kOpen, WebSocketAwaitingPong(1000), false, 0, 2200) ==
        WebSocketLivenessDecision::kAbortTransport);
    RUVIA_CHECK(decide(
        options(1000, 0), WsLivenessMode::kOpen, WebSocketAwaitingPong(1000), false, 0, 1500) ==
        WebSocketLivenessDecision::kIdle);
}
...
RUVIA_TEST(ws_liveness_bounds_local_close_handshake) {
    ...
    RUVIA_CHECK(decide(
        opts, WsLivenessMode::kAwaitingPeerClose, WebSocketAwaitingPeerClose(1000), false, 0, 2999) ==
        WebSocketLivenessDecision::kIdle);
    RUVIA_CHECK(decide(
        opts, WsLivenessMode::kAwaitingPeerClose, WebSocketAwaitingPeerClose(1000), false, 0, 3000) ==
        WebSocketLivenessDecision::kAbortTransport);
    RUVIA_CHECK(decide(
        options(1000, 500, 0), WsLivenessMode::kAwaitingPeerClose,
        WebSocketAwaitingPeerClose(1000), false, 0, 100000) == WebSocketLivenessDecision::kIdle);
}
