...
RUVIA_TEST(rate_limiter_allows_up_to_max_then_denies) {
    ...
    RUVIA_CHECK(rateLimitAllowed(limiter.allowGlobal("10.0.0.1")));
    RUVIA_CHECK(rateLimitAllowed(limiter.allowGlobal("10.0.0.1")));
    RUVIA_CHECK(rateLimitAllowed(limiter.allowGlobal("10.0.0.1")));
    const auto denied = limiter.allowGlobal("10.0.0.1");
...
RUVIA_TEST(rate_limiter_keys_are_independent) {
    ...
    RUVIA_CHECK(rateLimitAllowed(limiter.allowGlobal("1.1.1.1")));
    RUVIA_CHECK(rateLimitAllowed(limiter.allowGlobal("2.2.2.2")));   // distinct key, own budget
    RUVIA_CHECK(!rateLimitAllowed(limiter.allowGlobal("1.1.1.1")));  // first key now exhausted
    RUVIA_CHECK(!rateLimitAllowed(limiter.allowGlobal("2.2.2.2")));
}
...
RUVIA_TEST(rate_limiter_resets_after_window) {
    ...
    RUVIA_CHECK(rateLimitAllowed(limiter.allowGlobal("k")));
    RUVIA_CHECK(!rateLimitAllowed(limiter.allowGlobal("k")));
    ManualRateLimiterClock::set(1'019);
    RUVIA_CHECK(!rateLimitAllowed(limiter.allowGlobal("k")));
    ManualRateLimiterClock::set(1'020);
    RUVIA_CHECK(rateLimitAllowed(limiter.allowGlobal("k")));  // new fixed window admits again
}
...
RUVIA_TEST(rate_limiter_route_scope_independent_of_global) {
    ...
    RUVIA_CHECK(rateLimitAllowed(limiter.allowGlobal("ip")));
...
RUVIA_TEST(rate_limiter_route_enforced_when_app_rule_disabled) {
    ...
    RateLimiter limiter(std::nullopt);
    RUVIA_CHECK(!limiter.enabled());
    RUVIA_CHECK(rateLimitAllowed(limiter.allowGlobal("ip")));   // global off -> always allowed
    RUVIA_CHECK(rateLimitAllowed(limiter.allowGlobal("ip")));
    const RateLimitRule routeRule = ruleWith(1);
...
RUVIA_TEST(rate_limiter_oversized_key_follows_fail_closed) {
    ...
    RUVIA_CHECK(!rateLimitAllowed(closed.allowGlobal(oversized)));  // fail closed -> deny
    RateLimiter open(ruleWith(1, /*failClosed=*/false));
    RUVIA_CHECK(rateLimitAllowed(open.allowGlobal(oversized)));  // fail open -> allow
}
...
RUVIA_TEST(rate_limiter_workers_own_independent_budgets) {
    ...
    RUVIA_CHECK(rateLimitAllowed(firstWorker.allowGlobal("shared.key")));
    RUVIA_CHECK(!rateLimitAllowed(firstWorker.allowGlobal("shared.key")));
    RUVIA_CHECK(rateLimitAllowed(secondWorker.allowGlobal("shared.key")));
    RUVIA_CHECK(!rateLimitAllowed(secondWorker.allowGlobal("shared.key")));
}

RUVIA_TEST(rate_limiter_full_worker_table_honors_fail_policy) {
    RateLimiter closed(ruleWith(1, /*failClosed=*/true), 1);
    RUVIA_CHECK(rateLimitAllowed(closed.allowGlobal("first")));
    RUVIA_CHECK(!rateLimitAllowed(closed.allowGlobal("second")));

    RateLimiter open(ruleWith(1, /*failClosed=*/false), 1);
    RUVIA_CHECK(rateLimitAllowed(open.allowGlobal("first")));
    RUVIA_CHECK(rateLimitAllowed(open.allowGlobal("second")));
}
...
RUVIA_TEST(rate_limiter_reclaims_expired_worker_slot) {
    ...
    RUVIA_CHECK(rateLimitAllowed(limiter.allowGlobal("first")));
    RUVIA_CHECK(!rateLimitAllowed(limiter.allowGlobal("second")));
