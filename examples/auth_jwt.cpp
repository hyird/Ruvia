...
public:
    ruvia::Task<void> handle(ruvia::Context& c, const ruvia::Next& next) {
        const auto token = ruvia::jwtBearerToken(c.req().header("Authorization"));
        if (!token) {
            c.res(c.jsonError(401, "missing_token", "missing bearer token"));
            co_return;
        }
        ...
        try {
            ...
            c.res(c.jsonError(401, "invalid_token", "invalid bearer token"));
            co_return;
        }
