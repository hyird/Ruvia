...
| `ruvia_example_models_validation` | `web/models_validation.cpp` | Unified JSON models, nullable PATCH fields with public presence/null states, form bodies, nested models, arrays, recursive lists, defaults, validation middleware and rules, `jsonIf`/`formIf` fallbacks on media-type mismatch. |
...
| `ruvia_example_workers_blocking` | `web/workers_blocking.cpp` | Worker-local state (`useWorkerState<T>`/`workerState<T>`), cross-worker task posting (`App::workers`, `WebWorkerHandle::post`), and bounded blocking offload through `Context::runBlocking()`/`tryRunBlocking()`. |
