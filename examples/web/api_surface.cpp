...
int main() {
    ruvia::app().listen({.address = "0.0.0.0", .http = 8088}).server({.workerCount = 2, .processSignalHandlers = ruvia::ProcessSignalHandlerPolicy::kInstall}).onNotFound(&surfaceNotFound).run();
}
