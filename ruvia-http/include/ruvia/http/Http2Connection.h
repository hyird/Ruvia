...
private:
    friend class Http2RequestContent;
    explicit constexpr Http2KnownLengthRequestContent(std::uint64_t length) noexcept : length_(length) {}
...
public:
    [[nodiscard]] static constexpr Http2RequestContent none() noexcept { return Http2RequestContent(Http2RequestWithoutContent()); }
    [[nodiscard]] static constexpr Http2RequestContent knownLength(std::uint64_t length) noexcept { return Http2RequestContent(Http2KnownLengthRequestContent(length)); }
    [[nodiscard]] static constexpr Http2RequestContent streaming() noexcept { return Http2RequestContent(Http2StreamingRequestContent()); }
    [[nodiscard]] constexpr const Http2RequestWithoutContent* withoutContent() const& noexcept { return std::get_if<Http2RequestWithoutContent>(&value_); }
    const Http2RequestWithoutContent* withoutContent() const&& = delete;
    [[nodiscard]] constexpr const Http2KnownLengthRequestContent* knownLengthContent() const& noexcept { return std::get_if<Http2KnownLengthRequestContent>(&value_); }
    const Http2KnownLengthRequestContent* knownLengthContent() const&& = delete;
    [[nodiscard]] constexpr const Http2StreamingRequestContent* streamingContent() const& noexcept { return std::get_if<Http2StreamingRequestContent>(&value_); }
...
private:
    ...
    explicit constexpr Http2RequestContent(Http2RequestWithoutContent value) noexcept : value_(value) {}
    explicit constexpr Http2RequestContent(Http2KnownLengthRequestContent value) noexcept : value_(value) {}
    explicit constexpr Http2RequestContent(Http2StreamingRequestContent value) noexcept : value_(value) {}
    Value value_;
};
...
private:
    ...
    explicit constexpr Http2SubmittedRequestHead(std::uint32_t streamId) noexcept : streamId_(streamId) {}
    std::uint32_t streamId_;
};

class Http2RequestHeadSubmitFailure final {
public:
    [[nodiscard]] constexpr Http2RequestHeadSubmitError error() const noexcept { return error_; }

private:
    friend class Http2RequestHeadSubmitResult;
    explicit constexpr Http2RequestHeadSubmitFailure(Http2RequestHeadSubmitError error) noexcept : error_(error) {}
    Http2RequestHeadSubmitError error_;
};

class Http2RequestHeadSubmitResult final {
public:
    [[nodiscard]] constexpr const Http2SubmittedRequestHead* submitted() const& noexcept { return std::get_if<Http2SubmittedRequestHead>(&value_); }
    const Http2SubmittedRequestHead* submitted() const&& = delete;
    [[nodiscard]] constexpr const Http2RequestHeadSubmitFailure* failure() const& noexcept { return std::get_if<Http2RequestHeadSubmitFailure>(&value_); }
...
private:
    ...
    explicit constexpr Http2RequestHeadSubmitResult(Http2SubmittedRequestHead value) noexcept : value_(value) {}
    explicit constexpr Http2RequestHeadSubmitResult(Http2RequestHeadSubmitFailure value) noexcept : value_(value) {}
    [[nodiscard]] static constexpr Http2RequestHeadSubmitResult makeSubmitted(std::uint32_t streamId) noexcept { return Http2RequestHeadSubmitResult(Http2SubmittedRequestHead(streamId)); }
    [[nodiscard]] static constexpr Http2RequestHeadSubmitResult makeFailure(Http2RequestHeadSubmitError error) noexcept { return Http2RequestHeadSubmitResult(Http2RequestHeadSubmitFailure(error)); }
    Value value_;
};
...
public:
    ...
    [[nodiscard]] bool valid() const noexcept { return endpoint_ != nullptr && streamId_ != 0 && bytes_ != 0; }
...
public:
    [[nodiscard]] std::uint32_t streamId() const noexcept { return streamId_; }
    [[nodiscard]] const HttpClientResponseHead& head() const& noexcept { return head_; }
    const HttpClientResponseHead& head() const&& = delete;
    [[nodiscard]] std::optional<HttpClientRequestContentSignal> requestContentSignal() const noexcept { return signal_; }
private:
    friend class Http2Event;
    Http2InformationalHeadEvent(std::uint32_t streamId, HttpClientResponseHead head, std::optional<HttpClientRequestContentSignal> signal) noexcept : streamId_(streamId), head_(std::move(head)), signal_(signal) {}
    std::uint32_t streamId_;
...
public:
    ...
    [[nodiscard]] HttpServerExpectationPlan expectationPlan(HttpUnsupportedExpectationPolicy policy) const noexcept { return expectations_.serverPlan(content_, policy); }
private:
    friend class Http2Connection;
    friend class Http2Event;
...
private:
    friend class Http2Event;
    Http2ResponseHeadEvent(std::uint32_t streamId, HttpClientResponseHead head, std::optional<HttpClientRequestContentSignal> signal) noexcept : streamId_(streamId), head_(std::move(head)), signal_(signal) {}
    std::uint32_t streamId_;
    HttpClientResponseHead head_;
    std::optional<HttpClientRequestContentSignal> signal_;
};
...
public:
    ...
    [[nodiscard]] Http2ReceivedDataCredit takeCredit() & noexcept { return std::move(credit_); }
private:
    friend class Http2Event;
    Http2MessageBodyChunkEvent(detail::Http2ConnectionOwnerEndpoint* endpoint, std::uint32_t streamId, std::string_view bytes, std::uint32_t credit) noexcept : streamId_(streamId), bytes_(bytes), credit_(endpoint, streamId, credit) {}
    std::uint32_t streamId_;
...
private:
    friend class Http2Event;
    explicit constexpr Http2MessageEndEvent(std::uint32_t streamId) noexcept : streamId_(streamId) {}
    std::uint32_t streamId_;
};
...
private:
    friend class Http2Event;
    Http2TunnelDataEvent(detail::Http2ConnectionOwnerEndpoint* endpoint, std::uint32_t streamId, std::string_view bytes, std::uint32_t credit) noexcept : streamId_(streamId), bytes_(bytes), credit_(endpoint, streamId, credit) {}
    std::uint32_t streamId_;
    std::string_view bytes_;
    Http2ReceivedDataCredit credit_;
};
...
private:
    friend class Http2Event;
    explicit constexpr Http2TunnelEndEvent(std::uint32_t streamId) noexcept : streamId_(streamId) {}
    std::uint32_t streamId_;
};
...
public:
    [[nodiscard]] std::uint32_t streamId() const noexcept { return streamId_; }
    [[nodiscard]] Http2StreamCloseSource source() const noexcept { return source_; }
    [[nodiscard]] Http2ErrorCode error() const noexcept { return error_; }
private:
    friend class Http2Event;
    constexpr Http2StreamClosedEvent(std::uint32_t streamId, Http2StreamCloseSource source, Http2ErrorCode error) noexcept : streamId_(streamId), source_(source), error_(error) {}
    std::uint32_t streamId_;
...
private:
    friend class Http2Event;
    explicit constexpr Http2RequestUnprocessedEvent(std::uint32_t streamId) noexcept : streamId_(streamId) {}
    std::uint32_t streamId_;
};

class Http2GoawayEvent final {
public:
    [[nodiscard]] std::uint32_t lastStreamId() const noexcept { return lastStreamId_; }
    [[nodiscard]] Http2ErrorCode error() const noexcept { return error_; }
private:
    friend class Http2Event;
    constexpr Http2GoawayEvent(std::uint32_t lastStreamId, Http2ErrorCode error) noexcept : lastStreamId_(lastStreamId), error_(error) {}
...
public:
    ...
    [[nodiscard]] const Http2InformationalHeadEvent* informationalHead() const& noexcept { return std::get_if<Http2InformationalHeadEvent>(&value_); }
    const Http2InformationalHeadEvent* informationalHead() const&& = delete;
    [[nodiscard]] Http2RequestHeadEvent* requestHead() & noexcept { return std::get_if<Http2RequestHeadEvent>(&value_); }
    [[nodiscard]] const Http2RequestHeadEvent* requestHead() const& noexcept { return std::get_if<Http2RequestHeadEvent>(&value_); }
    const Http2RequestHeadEvent* requestHead() const&& = delete;
    [[nodiscard]] const Http2ResponseHeadEvent* responseHead() const& noexcept { return std::get_if<Http2ResponseHeadEvent>(&value_); }
    const Http2ResponseHeadEvent* responseHead() const&& = delete;
    [[nodiscard]] Http2MessageBodyChunkEvent* messageBodyChunk() & noexcept { return std::get_if<Http2MessageBodyChunkEvent>(&value_); }
    [[nodiscard]] const Http2MessageBodyChunkEvent* messageBodyChunk() const& noexcept { return std::get_if<Http2MessageBodyChunkEvent>(&value_); }
    const Http2MessageBodyChunkEvent* messageBodyChunk() const&& = delete;
    [[nodiscard]] const Http2MessageEndEvent* messageEnd() const& noexcept { return std::get_if<Http2MessageEndEvent>(&value_); }
    const Http2MessageEndEvent* messageEnd() const&& = delete;
    [[nodiscard]] Http2TunnelDataEvent* tunnelData() & noexcept { return std::get_if<Http2TunnelDataEvent>(&value_); }
    [[nodiscard]] const Http2TunnelDataEvent* tunnelData() const& noexcept { return std::get_if<Http2TunnelDataEvent>(&value_); }
    const Http2TunnelDataEvent* tunnelData() const&& = delete;
    [[nodiscard]] const Http2TunnelEndEvent* tunnelEnd() const& noexcept { return std::get_if<Http2TunnelEndEvent>(&value_); }
    const Http2TunnelEndEvent* tunnelEnd() const&& = delete;
    [[nodiscard]] const Http2StreamClosedEvent* streamClosed() const& noexcept { return std::get_if<Http2StreamClosedEvent>(&value_); }
    const Http2StreamClosedEvent* streamClosed() const&& = delete;
    [[nodiscard]] const Http2RequestUnprocessedEvent* requestUnprocessed() const& noexcept { return std::get_if<Http2RequestUnprocessedEvent>(&value_); }
    const Http2RequestUnprocessedEvent* requestUnprocessed() const&& = delete;
    [[nodiscard]] const Http2GoawayEvent* goaway() const& noexcept { return std::get_if<Http2GoawayEvent>(&value_); }
...
private:
    ...
    explicit Http2Event(Event event) noexcept : value_(std::move(event)) {}
    [[nodiscard]] static Http2Event informationalHead(std::uint32_t id, HttpClientResponseHead head, std::optional<HttpClientRequestContentSignal> signal) noexcept { return Http2Event(Http2InformationalHeadEvent(id, std::move(head), signal)); }
    [[nodiscard]] static Http2Event requestHead(detail::Http2ConnectionOwnerEndpoint* endpoint, std::uint32_t id, HttpRequest request, HttpRequestExpectations expectations, HttpRequestContentIndication content) noexcept { return Http2Event(Http2RequestHeadEvent(endpoint, id, std::move(request), expectations, content)); }
    [[nodiscard]] static Http2Event responseHead(std::uint32_t id, HttpClientResponseHead head, std::optional<HttpClientRequestContentSignal> signal) noexcept { return Http2Event(Http2ResponseHeadEvent(id, std::move(head), signal)); }
    [[nodiscard]] static Http2Event messageBodyChunk(detail::Http2ConnectionOwnerEndpoint* endpoint, std::uint32_t id, std::string_view bytes, std::uint32_t credit) noexcept { return Http2Event(Http2MessageBodyChunkEvent(endpoint, id, bytes, credit)); }
    [[nodiscard]] static Http2Event messageEnd(std::uint32_t id) noexcept { return Http2Event(Http2MessageEndEvent(id)); }
    [[nodiscard]] static Http2Event tunnelData(detail::Http2ConnectionOwnerEndpoint* endpoint, std::uint32_t id, std::string_view bytes, std::uint32_t credit) noexcept { return Http2Event(Http2TunnelDataEvent(endpoint, id, bytes, credit)); }
    [[nodiscard]] static Http2Event tunnelEnd(std::uint32_t id) noexcept { return Http2Event(Http2TunnelEndEvent(id)); }
    [[nodiscard]] static Http2Event streamClosed(std::uint32_t id, Http2StreamCloseSource source, Http2ErrorCode error) noexcept { return Http2Event(Http2StreamClosedEvent(id, source, error)); }
    [[nodiscard]] static Http2Event requestUnprocessed(std::uint32_t id) noexcept { return Http2Event(Http2RequestUnprocessedEvent(id)); }
    [[nodiscard]] static Http2Event goaway(std::uint32_t lastStreamId, Http2ErrorCode error) noexcept { return Http2Event(Http2GoawayEvent(lastStreamId, error)); }
    Value value_;
};
