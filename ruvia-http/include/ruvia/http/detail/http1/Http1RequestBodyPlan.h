...
private:
    ...
    explicit constexpr Http1KnownLengthRequestBody(std::size_t contentLength) noexcept
...
public:
    [[nodiscard]] constexpr const Http1RequestWithoutBody* withoutBody() const& noexcept {
        return std::get_if<Http1RequestWithoutBody>(&framing_);
    }
    [[nodiscard]] constexpr const Http1RequestWithoutBody* withoutBody() const&& = delete;

    [[nodiscard]] constexpr const Http1KnownLengthRequestBody* knownLength() const& noexcept {
        return std::get_if<Http1KnownLengthRequestBody>(&framing_);
    }
    [[nodiscard]] constexpr const Http1KnownLengthRequestBody* knownLength() const&& = delete;

    [[nodiscard]] constexpr const Http1ChunkedRequestBody* chunked() const& noexcept {
        return std::get_if<Http1ChunkedRequestBody>(&framing_);
    }
    [[nodiscard]] constexpr const Http1ChunkedRequestBody* chunked() const&& = delete;
