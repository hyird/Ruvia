...
private:
    ...
    constexpr Http1ChunkDecodeBodyChunk(std::size_t consumedBytes, std::string_view bytes) noexcept
        : consumedBytes_(consumedBytes),
          bytes_(bytes) {}
...
private:
    ...
    explicit constexpr Http1ChunkDecodeComplete(std::size_t consumedBytes, std::string_view trailers) noexcept
        : consumedBytes_(consumedBytes),
          trailers_(trailers) {}
...
private:
    ...
    [[nodiscard]] static Http1ChunkDecodeResult makeComplete(std::size_t consumedBytes, std::string_view trailers = {}) noexcept {
        ...
        for (;;) {
            switch (std::get<ProgressState>(state_)) {
                case ProgressState::kSizeLine: {
                    ...
                    if (chunkSize == 0) {
                        ...
                        if (bodyLimit_.additionExceeds(decodedBytes_, chunkSize)) {
                            return fail(cursor, Http1ChunkDecodeError::kBodyTooLarge);
                        }
                ...
                case ProgressState::kTrailers: {
                    ...
                    state_ = ProgressState::kComplete;
                    return Http1ChunkDecodeResult::makeComplete(cursor + trailerBytes, trailers.substr(0, trailerEnd));
                }
