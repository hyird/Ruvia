...
enum class Http1ConnectionDisposition : std::uint8_t { kReuse, kClose };
...
public:
    ...
    [[nodiscard]] static constexpr Http1ServerConnectionPlan http11Close() noexcept {
        return Http1ServerConnectionPlan(HttpProtocolVersion::kHttp11, Http1ConnectionDisposition::kClose);
    }
    ...
    [[nodiscard]] constexpr Http1ServerConnectionPlan requireClose() const noexcept {
        return Http1ServerConnectionPlan(protocolVersion_, Http1ConnectionDisposition::kClose);
    }
...
private:
    ...
    constexpr Http1ServerConnectionPlan(HttpProtocolVersion protocolVersion, Http1ConnectionDisposition disposition) noexcept
...
[[nodiscard]] inline Http1ServerConnectionPlan http1PlanHttp10RequestConnection(const HttpConnectionOptions& options) noexcept {
    return Http1ServerConnectionPlan(HttpProtocolVersion::kHttp10, !options.close() && options.keepAlive() ? Http1ConnectionDisposition::kReuse : Http1ConnectionDisposition::kClose);
}

[[nodiscard]] inline Http1ServerConnectionPlan http1PlanHttp11RequestConnection(const HttpConnectionOptions& options) noexcept {
    return Http1ServerConnectionPlan(HttpProtocolVersion::kHttp11, options.close() ? Http1ConnectionDisposition::kClose : Http1ConnectionDisposition::kReuse);
}
