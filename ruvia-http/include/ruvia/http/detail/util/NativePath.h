...
[[nodiscard]] inline HttpNativePathView httpNativePathView(const std::filesystem::path& path) noexcept {
...
[[nodiscard]] inline std::filesystem::path makePathFromHttpNativePath(const HttpNativePathChar* path) {
    return path == nullptr ? std::filesystem::path{} : std::filesystem::path(path);
}

[[nodiscard]] inline std::filesystem::path makePathFromHttpNativePath(const HttpNativePathString& path) {
