...
struct HttpResponseHeadersAccess final {
    ...
    [[nodiscard]] static HttpResponseHeader& add(HttpResponseHeaders& headers,
        std::string_view name, std::string_view value, std::uint32_t knownBit) {
        return headers.add(name, value, knownBit);
    }

    static void assign(HttpResponseHeaders& headers, HttpResponseHeader& header,
        std::string_view name, std::string_view value, std::uint32_t knownBit) {
        headers.assign(header, name, value, knownBit);
    }

    [[nodiscard]] static HttpResponseHeader& addUninitializedValue(HttpResponseHeaders& headers,
    ...
    [[nodiscard]] static HttpResponseHeader& addStableView(HttpResponseHeaders& headers,
        std::string_view name, std::string_view value, std::uint32_t knownBit) {
        return headers.addStableView(name, value, knownBit);
    }

    static void assignStableView(HttpResponseHeaders& headers, HttpResponseHeader& header,
        std::string_view name, std::string_view value, std::uint32_t knownBit) {
