...
[[nodiscard]] inline bool isValidHttpTrailerFieldValue(std::string_view value, HttpFieldListRole role, ForbiddenName&& forbiddenName) noexcept {
    const bool emptyField = httpTrimOws(value).empty();
    bool valid = true;
    httpVisitCommaSeparatedQuotedItems(value, [&valid, emptyField, role, &forbiddenName](std::string_view item) noexcept {
        if (item.empty()) {
...
[[nodiscard]] inline bool isValidHttpRequestTrailerFieldValue(std::string_view value, HttpFieldListRole role) noexcept {
    return isValidHttpTrailerFieldValue(value, role, [](std::string_view name) noexcept { return isForbiddenHttpRequestTrailerName(name); });
}
