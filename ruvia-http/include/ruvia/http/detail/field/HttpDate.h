...
[[nodiscard]] inline std::optional<std::time_t> httpCivilToTimeT(
    ...
    const auto maxDay =
        daysPerMonth[static_cast<std::size_t>(month - 1)] + (month == 2 && leapYear ? 1 : 0);
    if (day > maxDay) {
        return std::nullopt;
    }
    const auto days =
        httpDaysFromCivil(year, static_cast<unsigned>(month), static_cast<unsigned>(day));
...
[[nodiscard]] inline std::optional<std::time_t> httpParseImfFixdate(
    ...
    if (value.size() != 29 || !httpIsShortWeekday(value.substr(0, 3)) || value[3] != ',' ||
