...
[[nodiscard]] inline Http2WindowUpdateResult http2ApplyStreamWindowUpdate(Http2StreamState& stream, std::uint32_t increment) noexcept {
    if (increment == 0) {
        return Http2WindowUpdateResult::kZeroIncrement;
    }
    return stream.addSendWindow(static_cast<std::int64_t>(increment)) ? Http2WindowUpdateResult::kOk : Http2WindowUpdateResult::kOverflow;
}
...
// transactional.
[[nodiscard]] inline Http2ReceiveWindowDebitStatus http2DebitConnectionReceiveWindow(std::int32_t& connectionWindow, std::int32_t bytes) noexcept {
...
[[nodiscard]] inline Http2ReceiveWindowDebitStatus http2DebitStreamReceiveWindow(Http2StreamState& stream, std::int32_t bytes) noexcept {
    return stream.consumeReceiveWindow(bytes) ? Http2ReceiveWindowDebitStatus::kAccepted : Http2ReceiveWindowDebitStatus::kExceeded;
}

inline void http2CreditConnectionReceiveWindow(std::int32_t& connectionWindow, std::int32_t bytes) noexcept {
    connectionWindow += bytes;
}
...
[[nodiscard]] inline bool http2SendWindowAvailable(std::int32_t connectionWindow, const Http2StreamState& stream) noexcept {
    return connectionWindow > 0 && stream.sendWindow() > 0;
}

[[nodiscard]] inline std::size_t http2AvailableSendWindow(std::int32_t connectionWindow, const Http2StreamState& stream) noexcept {
...
inline void http2ConsumeSendWindow(std::int32_t& connectionWindow, Http2StreamState& stream, std::size_t bytes) noexcept {
