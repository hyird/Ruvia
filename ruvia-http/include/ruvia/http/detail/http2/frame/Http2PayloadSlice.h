...
[[nodiscard]] inline Http2PayloadSlice http2SliceTwoPartPayload(std::string_view first,
    ...
    if (offset < first.size()) {
        ...
        return Http2PayloadSlice{
            .first = firstSlice, .second = size == 0 ? std::string_view{} : second.substr(0, size)};
    }
...
Http2PayloadSlice http2SliceTwoPartPayload(
    First&&, std::string_view, std::size_t, std::size_t) = delete;

template <HttpTemporaryOwningCharString Second>
Http2PayloadSlice http2SliceTwoPartPayload(
    std::string_view, Second&&, std::size_t, std::size_t) = delete;
