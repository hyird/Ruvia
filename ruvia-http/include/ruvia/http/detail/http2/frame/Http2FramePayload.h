...
[[nodiscard]] inline Http2FramePayloadStatus http2StripPadAndPriority(
    const Http2FrameHeader& header, std::string_view payload, bool allowPriority,
    ...
    if (allowPriority && (header.flags & kHttp2FlagPriority) != 0) {
        ...
        if (dependency != nullptr) {
            *dependency =
                http2Read31(reinterpret_cast<const unsigned char*>(payload.data() + offset));
        }
        offset += 5;
    }
...
Http2FramePayloadStatus http2StripPadAndPriority(
    const Http2FrameHeader&, Payload&&, bool, std::string_view&, std::uint32_t* = nullptr) = delete;

[[nodiscard]] inline Http2FramePayloadStatus http2DecodeHeadersPayload(
    const Http2FrameHeader& header, std::string_view payload, std::string_view& fragment) noexcept {
    return http2StripPadAndPriority(header, payload, true, fragment);
}

template <HttpTemporaryOwningCharString Payload>
Http2FramePayloadStatus http2DecodeHeadersPayload(
    const Http2FrameHeader&, Payload&&, std::string_view&) = delete;

[[nodiscard]] inline Http2FramePayloadStatus http2HeadersPriorityDependency(
    const Http2FrameHeader& header, std::string_view payload, std::uint32_t& dependency) noexcept {
    dependency = 0;
...
[[nodiscard]] inline bool http2DecodeDataPayload(
    const Http2FrameHeader& header, std::string_view payload, std::string_view& data) noexcept {
