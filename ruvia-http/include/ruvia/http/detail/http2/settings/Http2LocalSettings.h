...
struct Http2LocalSettings final {
    ...
    static constexpr std::uint32_t kMaxHeaderListSize = static_cast<std::uint32_t>(kMaxHttpHeaderBytes);
    ...
    static constexpr std::size_t kFrameBytes = kHttp2FrameHeaderBytes + kPayloadBytes;
};
...
inline char* http2WriteLocalSettingsFrame(char* out) noexcept {
    out = http2WriteFrameHeader(out, Http2LocalSettings::kPayloadBytes, Http2FrameType::kSettings, 0, 0);
    out = http2WriteSettingsEntry(out, Http2SettingId::kHeaderTableSize, Http2LocalSettings::kHeaderTableSize);
    out = http2WriteSettingsEntry(out, Http2SettingId::kEnablePush, Http2LocalSettings::kEnablePush);
    out = http2WriteSettingsEntry(out, Http2SettingId::kMaxConcurrentStreams, Http2LocalSettings::kMaxConcurrentStreams);
    out = http2WriteSettingsEntry(out, Http2SettingId::kInitialWindowSize, Http2LocalSettings::kInitialWindowSize);
    out = http2WriteSettingsEntry(out, Http2SettingId::kMaxFrameSize, Http2LocalSettings::kMaxFrameSize);
    out = http2WriteSettingsEntry(out, Http2SettingId::kMaxHeaderListSize, Http2LocalSettings::kMaxHeaderListSize);
    return http2WriteSettingsEntry(out, Http2SettingId::kEnableConnectProtocol, Http2LocalSettings::kEnableConnectProtocol);
}
