...
public:
    ...
    [[nodiscard]] constexpr const Http2LocalRequestContentOpen* requestContentOpen()
        const& noexcept {
        return std::get_if<Http2LocalRequestContentOpen>(&state_);
    }
    [[nodiscard]] constexpr const Http2LocalRequestContentOpen* requestContentOpen() const&& =
        delete;

    [[nodiscard]] constexpr const Http2LocalResponseContentOpen* responseContentOpen()
        const& noexcept {
        return std::get_if<Http2LocalResponseContentOpen>(&state_);
    }
    [[nodiscard]] constexpr const Http2LocalResponseContentOpen* responseContentOpen() const&& =
        delete;

    [[nodiscard]] constexpr const Http2LocalResponseTrailersOnly* responseTrailersOnly()
        const& noexcept {
        return std::get_if<Http2LocalResponseTrailersOnly>(&state_);
    }
    [[nodiscard]] constexpr const Http2LocalResponseTrailersOnly* responseTrailersOnly() const&& =
        delete;
    ...
    [[nodiscard]] constexpr const Http2LocalEndStreamCommitted* endStreamCommitted()
        const& noexcept {
        return std::get_if<Http2LocalEndStreamCommitted>(&state_);
    }
    [[nodiscard]] constexpr const Http2LocalEndStreamCommitted* endStreamCommitted() const&& =
        delete;
...
private:
    [[nodiscard]] bool contentOrTrailersOpen() const noexcept {
        return requestContentOpen() != nullptr || responseContentOpen() != nullptr ||
               responseTrailersOnly() != nullptr || tunnelOpen() != nullptr;
    }

    using State = std::variant<Http2LocalHeadPending, Http2LocalRequestContentOpen,
        Http2LocalResponseContentOpen, Http2LocalResponseTrailersOnly, Http2LocalConnectPending,
        Http2LocalTunnelOpen, Http2LocalEndStreamQueued, Http2LocalEndStreamCommitted,
        Http2StreamAborted>;
