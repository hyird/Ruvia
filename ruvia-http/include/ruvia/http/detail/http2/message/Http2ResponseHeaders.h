...
[[nodiscard]] inline Http2InterimResponseHeaderEncodeStatus validateHttp2InterimResponseHeaders(
    ...
    if (commonValidation != HttpInterimResponseHeaderValidationStatus::kOk) {
        return Http2InterimResponseHeaderEncodeStatus::kInvalidHeader;
    }
    const auto statusToken = httpStatusCodeToken(response.status());
    HttpHeaderSectionSize sectionSize;
    if (!sectionSize.add(":status", httpStatusCodeTokenView(statusToken))) {
        return Http2InterimResponseHeaderEncodeStatus::kInvalidHeader;
    }
...
[[nodiscard]] inline bool appendHttp2ResponseHeaders(Http2StreamState& stream,
    ...
    const auto statusToken = httpStatusCodeToken(plan.bodyPlan().responseStatus());
    HttpHeaderSectionSize sectionSize;
    if (!sectionSize.add(":status", httpStatusCodeTokenView(statusToken))) {
        return false;
    }
