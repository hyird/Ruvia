...
private:
    ...
    using Value = std::variant<
        Http2PeerSettingApplied,
        Http2PeerInitialWindowChange,
    ...
    [[nodiscard]] static constexpr Http2PeerSettingApplyResult
    makeInitialWindowChange(std::int64_t delta) noexcept {
        return Http2PeerSettingApplyResult(Http2PeerInitialWindowChange(delta));
    }

    [[nodiscard]] static constexpr Http2PeerSettingApplyResult
    makeFailure(Http2PeerSettingError error) noexcept {
...
[[nodiscard]] inline Http2ErrorCode http2PeerSettingErrorCode(Http2PeerSettingError error) noexcept {
    return error == Http2PeerSettingError::kInvalidInitialWindow
