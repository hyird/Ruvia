...
inline constexpr auto kHttpWebSocketSwitchingProtocolsPrefix = [] {
    ...
    constexpr auto status = httpStatusCodeToken(http_status::kSwitchingProtocols);
...
inline constexpr std::string_view kHttpWebSocketSubprotocolHeaderPrefix = "Sec-WebSocket-Protocol: ";
inline constexpr std::string_view kHttpWebSocketExtensionsHeaderPrefix = "Sec-WebSocket-Extensions: ";
...
public:
    ...
    HttpWebSocketServerHandshake& operator=(const HttpWebSocketServerHandshake&) = delete;
    HttpWebSocketServerHandshake(HttpWebSocketServerHandshake&&) noexcept = default;
    HttpWebSocketServerHandshake& operator=(HttpWebSocketServerHandshake&&) = delete;

    [[nodiscard]] const WebSocketServerNegotiation& negotiation() const& noexcept {
        return negotiation_;
    }
    [[nodiscard]] const WebSocketServerNegotiation& negotiation() const&& = delete;

    template <typename Visitor>
    void forEachResponsePart(Visitor&& visitor) const {
        visitor(std::string_view(kHttpWebSocketSwitchingProtocolsPrefix.data(), kHttpWebSocketSwitchingProtocolsPrefix.size()));
...
private:
    friend HttpWebSocketServerHandshake makeHttpWebSocketServerHandshake(const HttpRequest&, std::string_view, std::pmr::memory_resource*);

    HttpWebSocketServerHandshake(WebSocketAcceptKey accept, WebSocketServerNegotiation&& negotiation) noexcept
        : accept_(accept),
...
[[nodiscard]] inline HttpWebSocketServerHandshake makeHttpWebSocketServerHandshake(const HttpRequest& request, std::string_view supportedSubprotocols, std::pmr::memory_resource* resource = nullptr) {
    WebSocketAcceptKey accept;
    encodeWebSocketAccept(accept, requestKnownHeader(request, RequestKnownHeader::kSecWebSocketKey));
    return HttpWebSocketServerHandshake(accept, makeWebSocketServerNegotiation(request, supportedSubprotocols, resource));
}
