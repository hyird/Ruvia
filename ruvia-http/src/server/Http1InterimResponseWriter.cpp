...
struct Http1InterimResponsePrepareResultAccess final {
    [[nodiscard]] static constexpr Http1InterimResponsePrepareResult
    bufferTooSmall(std::size_t requiredHeadBytes) noexcept {
    ...
    [[nodiscard]] static constexpr Http1InterimResponsePrepareResult failure(
        ...
        return Http1InterimResponsePrepareResult(
            Http1InterimResponsePrepareFailure(error));
    }
...
[[nodiscard]] bool analyzeHttp1Fields(
    ...
    if (facts.upgradeProtocols.hasField() &&
        ...
        error = Http1InterimResponsePrepareError::
...
Http1InterimResponsePrepareResult Http1InterimResponseWriter::prepare(
    ...
    const auto commonValidation =
        detail::validateHttpInterimResponseHeaders(response);
    if (commonValidation !=
    ...
    if (headBuffer.size() < facts.wireBytes) {
        return detail::Http1InterimResponsePrepareResultAccess::bufferTooSmall(
            facts.wireBytes);
    }
