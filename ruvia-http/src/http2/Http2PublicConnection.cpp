...
public:
    ...
    Http2ConnectionOwnerEndpoint(
        ...
        : target_(target), abandonRequest_(abandonRequest), abandonCredit_(abandonCredit) {}

    void retain() noexcept { ++references_; }
    void release() noexcept {
        if (references_ == 0) std::terminate();
        if (--references_ == 0) delete this;
    }
    void detach() noexcept { target_ = nullptr; }
    void abandonRequest(std::uint32_t streamId) noexcept {
        if (target_ != nullptr) abandonRequest_(target_, streamId);
    }
    void abandonCredit(std::uint32_t streamId, std::uint32_t bytes) noexcept {
        if (target_ != nullptr) abandonCredit_(target_, streamId, bytes);
    }
...
[[nodiscard]] detail::Http2RequestContent toInternal(Http2RequestContent content) noexcept {
    if (content.withoutContent() != nullptr) return detail::Http2RequestContent::none();
    if (const auto* known = content.knownLengthContent()) return detail::Http2RequestContent::knownLength(known->length());
    return detail::Http2RequestContent::streaming();
}

[[nodiscard]] detail::Http2Role toInternal(Http2Role role) {
    switch (role) {
        case Http2Role::kServer: return detail::Http2Role::kServer;
        case Http2Role::kClient: return detail::Http2Role::kClient;
    }
...
[[nodiscard]] detail::Http2EndStream toInternal(Http2EndStream endStream) {
    switch (endStream) {
        case Http2EndStream::kKeepOpen: return detail::Http2EndStream::kKeepOpen;
        case Http2EndStream::kEndStream: return detail::Http2EndStream::kEndStream;
    }
...
[[nodiscard]] detail::Http2ErrorCode toInternal(Http2ErrorCode error) {
    switch (error) {
        case Http2ErrorCode::kNoError: return detail::Http2ErrorCode::kNoError;
        case Http2ErrorCode::kProtocolError: return detail::Http2ErrorCode::kProtocolError;
        case Http2ErrorCode::kInternalError: return detail::Http2ErrorCode::kInternalError;
        case Http2ErrorCode::kFlowControlError: return detail::Http2ErrorCode::kFlowControlError;
        case Http2ErrorCode::kSettingsTimeout: return detail::Http2ErrorCode::kSettingsTimeout;
        case Http2ErrorCode::kStreamClosed: return detail::Http2ErrorCode::kStreamClosed;
        case Http2ErrorCode::kFrameSizeError: return detail::Http2ErrorCode::kFrameSizeError;
        case Http2ErrorCode::kRefusedStream: return detail::Http2ErrorCode::kRefusedStream;
        case Http2ErrorCode::kCancel: return detail::Http2ErrorCode::kCancel;
        case Http2ErrorCode::kCompressionError: return detail::Http2ErrorCode::kCompressionError;
        case Http2ErrorCode::kConnectError: return detail::Http2ErrorCode::kConnectError;
        case Http2ErrorCode::kEnhanceYourCalm: return detail::Http2ErrorCode::kEnhanceYourCalm;
        case Http2ErrorCode::kInadequateSecurity: return detail::Http2ErrorCode::kInadequateSecurity;
        case Http2ErrorCode::kHttp11Required: return detail::Http2ErrorCode::kHttp11Required;
    }
...
[[nodiscard]] Http2ErrorCode toPublic(detail::Http2ErrorCode error) noexcept {
    switch (error) {
        case detail::Http2ErrorCode::kNoError: return Http2ErrorCode::kNoError;
        case detail::Http2ErrorCode::kProtocolError: return Http2ErrorCode::kProtocolError;
        case detail::Http2ErrorCode::kInternalError: return Http2ErrorCode::kInternalError;
        case detail::Http2ErrorCode::kFlowControlError: return Http2ErrorCode::kFlowControlError;
        case detail::Http2ErrorCode::kSettingsTimeout: return Http2ErrorCode::kSettingsTimeout;
        case detail::Http2ErrorCode::kStreamClosed: return Http2ErrorCode::kStreamClosed;
        case detail::Http2ErrorCode::kFrameSizeError: return Http2ErrorCode::kFrameSizeError;
        case detail::Http2ErrorCode::kRefusedStream: return Http2ErrorCode::kRefusedStream;
        case detail::Http2ErrorCode::kCancel: return Http2ErrorCode::kCancel;
        case detail::Http2ErrorCode::kCompressionError: return Http2ErrorCode::kCompressionError;
        case detail::Http2ErrorCode::kConnectError: return Http2ErrorCode::kConnectError;
        case detail::Http2ErrorCode::kEnhanceYourCalm: return Http2ErrorCode::kEnhanceYourCalm;
        case detail::Http2ErrorCode::kInadequateSecurity: return Http2ErrorCode::kInadequateSecurity;
        case detail::Http2ErrorCode::kHttp11Required: return Http2ErrorCode::kHttp11Required;
    }
    std::terminate();
}
...
[[nodiscard]] Http2StreamCloseSource toPublic(detail::Http2StreamCloseSource source) noexcept {
    switch (source) {
        case detail::Http2StreamCloseSource::kLocal: return Http2StreamCloseSource::kLocal;
        case detail::Http2StreamCloseSource::kPeer: return Http2StreamCloseSource::kPeer;
        case detail::Http2StreamCloseSource::kPeerGoaway: return Http2StreamCloseSource::kPeerGoaway;
    }
    std::terminate();
}
...
[[nodiscard]] Http2OutputConsumeStatus toPublic(detail::Http2OutputConsumeStatus status) noexcept {
    switch (status) {
        case detail::Http2OutputConsumeStatus::kPending: return Http2OutputConsumeStatus::kPending;
        case detail::Http2OutputConsumeStatus::kDrained: return Http2OutputConsumeStatus::kDrained;
        case detail::Http2OutputConsumeStatus::kOutOfRange: return Http2OutputConsumeStatus::kOutOfRange;
    }
    std::terminate();
}
...
[[nodiscard]] Http2SubmitStatus toPublic(detail::Http2SubmitStatus status) noexcept {
    switch (status) {
        case detail::Http2SubmitStatus::kAccepted: return Http2SubmitStatus::kAccepted;
        case detail::Http2SubmitStatus::kClosed: return Http2SubmitStatus::kClosed;
        case detail::Http2SubmitStatus::kInvalidState: return Http2SubmitStatus::kInvalidState;
        case detail::Http2SubmitStatus::kInvalidMessage: return Http2SubmitStatus::kInvalidMessage;
        case detail::Http2SubmitStatus::kPeerCapabilityUnavailable: return Http2SubmitStatus::kPeerCapabilityUnavailable;
    }
    std::terminate();
}
...
[[nodiscard]] Http2DataSubmitStatus toPublic(detail::Http2DataSubmitStatus status) noexcept {
    switch (status) {
        case detail::Http2DataSubmitStatus::kAccepted: return Http2DataSubmitStatus::kAccepted;
        case detail::Http2DataSubmitStatus::kQueued: return Http2DataSubmitStatus::kQueued;
        case detail::Http2DataSubmitStatus::kBackpressured: return Http2DataSubmitStatus::kBackpressured;
        case detail::Http2DataSubmitStatus::kExpectationPending: return Http2DataSubmitStatus::kExpectationPending;
        case detail::Http2DataSubmitStatus::kClosed: return Http2DataSubmitStatus::kClosed;
        case detail::Http2DataSubmitStatus::kInvalidState: return Http2DataSubmitStatus::kInvalidState;
        case detail::Http2DataSubmitStatus::kContentLengthExceeded: return Http2DataSubmitStatus::kContentLengthExceeded;
        case detail::Http2DataSubmitStatus::kContentLengthIncomplete: return Http2DataSubmitStatus::kContentLengthIncomplete;
    }
    std::terminate();
}
...
[[nodiscard]] Http2RequestContentReleaseStatus toPublic(detail::Http2RequestContentReleaseStatus status) noexcept {
    switch (status) {
        case detail::Http2RequestContentReleaseStatus::kReleased: return Http2RequestContentReleaseStatus::kReleased;
        case detail::Http2RequestContentReleaseStatus::kNotPending: return Http2RequestContentReleaseStatus::kNotPending;
        case detail::Http2RequestContentReleaseStatus::kClosed: return Http2RequestContentReleaseStatus::kClosed;
    }
    std::terminate();
}
...
[[nodiscard]] Http2RequestHeadSubmitError toPublic(detail::Http2RequestHeadSubmitError error) noexcept {
    switch (error) {
        case detail::Http2RequestHeadSubmitError::kInvalidState: return Http2RequestHeadSubmitError::kInvalidState;
        case detail::Http2RequestHeadSubmitError::kConnectionNotStarted: std::terminate();
        case detail::Http2RequestHeadSubmitError::kConnectionUnavailable: return Http2RequestHeadSubmitError::kConnectionUnavailable;
        case detail::Http2RequestHeadSubmitError::kPeerStreamLimitReached: return Http2RequestHeadSubmitError::kPeerStreamLimitReached;
        case detail::Http2RequestHeadSubmitError::kLocalStreamCapacityReached: return Http2RequestHeadSubmitError::kLocalStreamCapacityReached;
        case detail::Http2RequestHeadSubmitError::kPeerCapabilityUnavailable: return Http2RequestHeadSubmitError::kPeerCapabilityUnavailable;
        case detail::Http2RequestHeadSubmitError::kInvalidMessage: return Http2RequestHeadSubmitError::kInvalidMessage;
    }
    std::terminate();
}
...
[[nodiscard]] RequestHeadSubmitOutcome pinSubmittedRequest(
    ...
    const detail::Http2RequestHeadSubmitResult& result) {
...
public:
    ...
    [[nodiscard]] DeferredRelease* deferred(std::uint32_t streamId) noexcept {
        for (std::size_t i = 0; i < deferredReleaseCount; ++i) {
            if (deferredReleases[i].streamId == streamId) return &deferredReleases[i];
        }
        return nullptr;
    }
    ...
    void defer(std::uint32_t streamId, DeferredReleaseKind kind) noexcept {
        if (auto* existing = deferred(streamId)) {
            if (kind == DeferredReleaseKind::kAbandon) existing->kind = kind;
            return;
        }
        if (deferredReleaseCount == deferredReleases.size()) std::terminate();
        deferredReleases[deferredReleaseCount++] = DeferredRelease{streamId, kind};
    }

    void eraseDeferred(std::uint32_t streamId) noexcept {
        for (std::size_t i = 0; i < deferredReleaseCount; ++i) {
            if (deferredReleases[i].streamId != streamId) continue;
    ...
    [[nodiscard]] DeferredCredit* deferredCredit(std::uint32_t streamId) noexcept {
        for (std::size_t i = 0; i < deferredCreditCount; ++i) {
            if (deferredCredits[i].streamId == streamId) return &deferredCredits[i];
        }
        return nullptr;
    }
    ...
    void retryDeferredRelease(std::uint32_t streamId) noexcept {
        ...
        if (pending->kind == DeferredReleaseKind::kAfterCredits) {
            ...
            if (stream != nullptr && stream->windowDebt() != 0) return;
        }
        try {
            connection.unpinStream(streamId);
            eraseDeferred(streamId);
...
Http2ReceivedDataCredit::Http2ReceivedDataCredit(detail::Http2ConnectionOwnerEndpoint* endpoint,
    ...
    : endpoint_(endpoint), streamId_(streamId), bytes_(bytes) {
    endpoint_->retain();
}
...
Http2RequestHeadEvent::Http2RequestHeadEvent(detail::Http2ConnectionOwnerEndpoint* endpoint,
    ...
    : streamId_(streamId), request_(std::move(request)), expectations_(expectations),
      content_(content), endpoint_(endpoint) {
    endpoint_->retain();
}
...
Http2RequestHeadEvent::Http2RequestHeadEvent(Http2RequestHeadEvent&& other) noexcept
    : streamId_(std::exchange(other.streamId_, 0)), request_(std::move(other.request_)),
      expectations_(other.expectations_), content_(other.content_),
      endpoint_(std::exchange(other.endpoint_, nullptr)) {}
...
Http2FeedResult Http2Connection::feed(std::string_view input) {
    ...
    switch (impl_->connection.feed(input)) {
        case detail::Http2FeedResult::kConnectionNotStarted: std::terminate();
        case detail::Http2FeedResult::kEventsPending: return Http2FeedResult::kEventsPending;
        case detail::Http2FeedResult::kAccepted: return Http2FeedResult::kAccepted;
        case detail::Http2FeedResult::kNeedInput: return Http2FeedResult::kNeedInput;
        case detail::Http2FeedResult::kProtocolFailure: return Http2FeedResult::kProtocolFailure;
    }
    std::terminate();
}
...
std::optional<Http2Event> Http2Connection::nextEvent() {
    ...
    if (auto* value = event->informationalHead()) {
        ...
        auto result = Http2Event::informationalHead(
            streamId, std::move(*value).takeHead(), signal);
    ...
    if (const auto* value = event->messageHead()) {
        if (impl_->role == Http2Role::kServer) {
            ...
            const auto build = detail::Http2RequestBuilder::build(
                *stream, request, impl_->resource, {});
            ...
            auto result = Http2Event::requestHead(
                impl_->endpoint, value->streamId(), std::move(request),
...
Http2RequestHeadSubmitResult Http2Connection::submitRequestHead(const Http2RegularRequestHeadView& request) {
    ...
    if (request.authority) authority = request.authority->view();
...
Http2RequestHeadSubmitResult Http2Connection::submitRequestHead(const Http2ConnectRequestHeadView& request) {
    const auto result = impl_->connection.submitConnectRequestHead(request.authority.view(),
...
Http2RequestContentReleaseStatus Http2Connection::releaseRequestContent(std::uint32_t streamId) noexcept { return toPublic(impl_->connection.releaseRequestContent(streamId)); }
Http2SubmitStatus Http2Connection::submitInterimResponseHead(std::uint32_t streamId, const HttpInterimResponseHead& response) { return toPublic(impl_->connection.submitInterimResponseHead(streamId, response)); }
...
Http2ReceivedDataAcknowledgeStatus Http2Connection::acknowledge(Http2ReceivedDataCredit&& credit) {
    if (!credit.valid() || credit.endpoint_ != impl_->endpoint) return Http2ReceivedDataAcknowledgeStatus::kInvalidCredit;
    ...
    credit.bytes_ = 0;
    endpoint->release();
    if (acknowledged) impl_->retryDeferredRelease(streamId);
...
Http2ServerRequestReleaseStatus Http2Connection::release(Http2RequestHeadEvent&& request) {
    if (request.endpoint_ == nullptr || request.streamId_ == 0 || request.endpoint_ != impl_->endpoint) return Http2ServerRequestReleaseStatus::kInvalidLease;
