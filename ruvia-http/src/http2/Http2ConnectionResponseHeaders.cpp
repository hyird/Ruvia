...
bool http2OnDecodedResponseTrailer(void* target, std::string_view name, std::string_view value) {
    ...
    return context.acceptRegularField() && http2IsValidDecodedResponseHeader(name, value) && !isForbiddenResponseTrailerName(name) && context.stream.appendRemoteHeader(name, value, classifyRequestHeader(name));
}
