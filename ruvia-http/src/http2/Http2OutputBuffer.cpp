...
void Http2OutputBuffer::appendGoawayFrame(
    ...
    auto* const end = http2WriteGoawayPayload(
        payload.data(), lastStreamId, error);
    appendFrame(
        Http2FrameType::kGoaway,
        0,
        0,
        std::string_view(
            payload.data(), static_cast<std::size_t>(end - payload.data())),
        debug);
}

void Http2OutputBuffer::appendRstStream(
