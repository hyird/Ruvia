...
[[nodiscard]] bool nextNormalizedHostUnit(std::string_view value, std::size_t& cursor, NormalizedHostUnit& output) noexcept {
    ...
    output = NormalizedHostUnit{.byte = byte, .encodedReserved = encodedReserved};
    return true;
}
...
bool isValidRequestTargetBytes(std::string_view target) noexcept {
    ...
    for (std::size_t i = 0; i < target.size(); ++i) {
        ...
        if (!isUriPchar(byte) && byte != '/' && byte != '?' && byte != '[' && byte != ']') {
            return false;
        }
    }
    return true;
...
bool isValidOriginOrAsteriskFormTarget(HttpKnownMethod method, std::string_view target) noexcept {
    return target == "*" ? method == HttpKnownMethod::kOptions : isValidOriginFormTarget(target);
}
...
std::optional<HttpAuthorityView> parseHttpAuthority(std::string_view value) noexcept {
    ...
    if (!hasPortDelimiter) {
        return HttpAuthorityViewAccess::make(host, HttpAuthorityPortKind::kAbsent);
    }
    if (portText.empty()) {
...
bool isValidUriAuthority(std::string_view value) noexcept {
    ...
    if (!hostAndPort.empty() && hostAndPort.front() == '[') {
        ...
        host = delimiter == std::string_view::npos ? hostAndPort : hostAndPort.substr(0, delimiter);
    ...
    return (host.empty() || isValidHttpHost(host)) && (!hasPort || isValidUriPort(port));
}
...
[[nodiscard]] bool parseAbsoluteTarget(HttpKnownMethod method, std::string_view target, RequestTargetView& output) noexcept {
    ...
    if (schemeEnd == std::string_view::npos || !isValidUriScheme(target.substr(0, schemeEnd))) {
        return false;
    }
    ...
    if (hierarchy.starts_with("//")) {
        ...
        if (httpScheme && (authority.empty() || userinfoDelimiter != std::string_view::npos)) {
            return false;
        }
    } else {
...
bool authorityMatchesHost(std::string_view authority, std::string_view host, std::uint16_t defaultPort) noexcept {
    ...
    if (defaultPort == 0) {
        const bool authorityHasPort = authorityParts->portKind() == HttpAuthorityPortKind::kValue;
        const bool hostHasPort = hostParts->portKind() == HttpAuthorityPortKind::kValue;
