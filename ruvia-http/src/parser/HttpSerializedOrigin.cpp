...
[[nodiscard]] bool isSerializedOriginIpv4Number(std::string_view value) noexcept {
    ...
    if (value.size() >= 2 && value.front() == '0' && (value[1] == 'x' || value[1] == 'X')) {
        return std::ranges::all_of(value.substr(2), [](char digit) noexcept { return isDecimalDigit(digit) || (digit >= 'a' && digit <= 'f') || (digit >= 'A' && digit <= 'F'); });
    }
    if (value.size() >= 2 && value.front() == '0') {
        return std::ranges::all_of(value.substr(1), [](char digit) noexcept { return digit >= '0' && digit <= '7'; });
    }
    return false;
}
...
[[nodiscard]] bool isValidSerializedOriginDomain(std::string_view domain) noexcept {
    ...
    return std::ranges::all_of(domain, [](char byte) noexcept { return isSerializedOriginDomainByte(static_cast<unsigned char>(byte)); });
}
