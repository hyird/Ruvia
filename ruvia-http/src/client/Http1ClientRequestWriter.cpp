...
struct Http1ClientRequestPrepareResultAccess final {
    [[nodiscard]] static Http1ClientExchangeState exchangeState(HttpKnownMethod method, HttpConnectionOptions connectionOptions, Http1ClosePolicy closePolicy, Http1ClientInitialContentState contentState, std::pmr::string offeredUpgradeProtocols) noexcept {
        return Http1ClientExchangeState(method, connectionOptions, closePolicy, contentState, std::move(offeredUpgradeProtocols));
    }
    ...
    [[nodiscard]] static Http1ClientRequestPrepareResult preparedWithoutContent(std::string_view head, Http1ClientExchangeState exchangeState) noexcept {
        return Http1ClientRequestPrepareResult(PreparedHttp1ClientRequest(head, Http1ClientRequestContentPlan(Http1ClientRequestWithoutContent()), std::move(exchangeState)));
    }

    [[nodiscard]] static Http1ClientRequestPrepareResult preparedImmediateContent(std::string_view head, std::string_view contentBytes, Http1ClientExchangeState exchangeState) noexcept {
        return Http1ClientRequestPrepareResult(PreparedHttp1ClientRequest(head, Http1ClientRequestContentPlan(Http1ClientImmediateRequestContent(contentBytes)), std::move(exchangeState)));
    }

    [[nodiscard]] static Http1ClientRequestPrepareResult preparedContinueGatedContent(std::string_view head, std::string_view contentBytes, Http1ClientExchangeState exchangeState) noexcept {
        return Http1ClientRequestPrepareResult(PreparedHttp1ClientRequest(head, Http1ClientRequestContentPlan(Http1ClientContinueGatedRequestContent(contentBytes)), std::move(exchangeState)));
    }
};
...
[[nodiscard]] std::pmr::string ownOfferedUpgradeProtocols(std::span<const HttpHeaderView> headers, std::pmr::memory_resource* resource) {
...
[[nodiscard]] Http1ClientRequestPrepareResult prepareRequest(const HttpOriginView& origin, std::string_view method, std::string_view target, bool connect, std::span<const HttpHeaderView> headers, HttpClientRequestContentView content, std::span<char> headBuffer, Http1ClientRequestWirePolicy policy, std::pmr::memory_resource* resource) {
    ...
    auto offeredUpgradeProtocols = headerFacts.upgradeProtocols.hasProtocol() ? ownOfferedUpgradeProtocols(headers, resource) : std::pmr::string(resource);
    ...
    *cursor++ = ' ';
    ...
    const auto contentState = expectContinue ? detail::Http1ClientInitialContentState::kAwaitingContinue : (explicitContent && !contentBytes->value().empty() ? detail::Http1ClientInitialContentState::kPending : detail::Http1ClientInitialContentState::kComplete);
    auto exchangeState = detail::Http1ClientRequestPrepareResultAccess::exchangeState(connect ? HttpKnownMethod::kConnect : classifyHttpMethod(method), headerFacts.connectionOptions, effectiveClosePolicy, contentState, std::move(offeredUpgradeProtocols));
    ...
    if (expectContinue) {
        return detail::Http1ClientRequestPrepareResultAccess::preparedContinueGatedContent(head, contentBytes->value(), std::move(exchangeState));
    }
    return detail::Http1ClientRequestPrepareResultAccess::preparedImmediateContent(head, contentBytes->value(), std::move(exchangeState));
}
...
Http1ClientRequestPrepareResult Http1ClientRequestWriter::prepare(const HttpOriginView& origin, const HttpClientRequestView& request, std::span<char> headBuffer, Http1ClientRequestWirePolicy policy) const {
    ...
    return prepareRequest(origin, request.method, request.target, false, static_cast<std::span<const HttpHeaderView>>(request.headers), request.content, headBuffer, policy, resource_);
}

Http1ClientRequestPrepareResult Http1ClientRequestWriter::prepareConnect(const HttpOriginView& tunnelOrigin, std::span<const HttpHeaderView> headers, std::span<char> headBuffer, Http1ClientRequestWirePolicy policy) const {
    if (!isValidHttp1ClosePolicy(policy.closePolicy)) {
        return detail::Http1ClientRequestPrepareResultAccess::failure(Http1ClientRequestPrepareError::kInvalidClosePolicy);
    }
    ...
    return prepareRequest(tunnelOrigin, "CONNECT", {}, true, headers, HttpClientRequestContentView::none(), headBuffer, policy, resource_);
}
