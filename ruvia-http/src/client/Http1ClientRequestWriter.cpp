...
[[nodiscard]] bool hasWriterManagedConnectionOption(
    ...
    while (start <= value.size()) {
        ...
        const auto end = comma == std::string_view::npos
            ? value.size()
            : comma;
        const auto option = detail::httpTrimOws(
            value.substr(start, end - start));
        // These end-to-end fields are generated by the writer. Listing one as
        // a connection option would require an intermediary to remove it,
        // changing routing, framing, or Expect: 100-continue semantics.
        if (detail::httpAsciiEqualsIgnoreCase(option, "Host") ||
            detail::httpAsciiEqualsIgnoreCase(option, "Content-Length") ||
            detail::httpAsciiEqualsIgnoreCase(option, "Expect")) {
            return true;
        }
    ...
    for (const auto& header : headers) {
        ...
        if (detail::httpAsciiEqualsIgnoreCase(name, "Connection")) {
            if (facts.connectionOptions.parseField(
                ...
                hasWriterManagedConnectionOption(value)) {
