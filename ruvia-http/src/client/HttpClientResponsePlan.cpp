...
[[nodiscard]] bool requestOffersProtocol(const Http1ClientExchangeState& exchangeState, const HttpUpgradeProtocol& selected) noexcept {
    ...
    if (protocols.parseField(offeredProtocols, HttpFieldListRole::kSender, [&selected, &offered](const HttpUpgradeProtocol& candidate) noexcept {
...
[[nodiscard]] bool requestAllowsProtocolSwitch(const Http1ClientExchangeState& exchangeState, const Http1ClientParsedResponseHead& response, Http1ClientRequestContentPhase requestContentPhase) noexcept {
    ...
    for (std::size_t i = 0; i < response.headerCount; ++i) {
        ...
        if (selectedProtocols.parseField(header.value(), HttpFieldListRole::kRecipient, [&exchangeState](const HttpUpgradeProtocol& selected) noexcept { return requestOffersProtocol(exchangeState, selected); }) != HttpFieldListParseStatus::kOk) {
            return false;
        }
    }
...
[[nodiscard]] Http1ClosePolicy finalResponsePersistence(const Http1ClientExchangeState& exchangeState, const Http1ClientParsedResponseHead& response) noexcept {
...
Http1ClientResponsePlanningResult planHttp1ClientResponse(const Http1ClientExchangeState& exchangeState, const Http1ClientParsedResponseHead& response, Http1ClientRequestContentPhase requestContentPhase) noexcept {
    const auto contentSemantics = httpResponseContentSemantics(Http1ClientExchangeStateAccess::method(exchangeState), response.statusCode);
