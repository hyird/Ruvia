...
struct HttpResponseHeadersAccess final {
    ...
    static void truncate(HttpResponseHeaders& headers, iterator begin, iterator write) {
        if (headers.spilled_) {
            headers.heap_.erase(
                headers.heap_.begin() + static_cast<std::ptrdiff_t>(write - begin),
                headers.heap_.end());
            return;
        }
    ...
    [[nodiscard]] static HttpResponseHeader& add(
    ...
    [[nodiscard]] static HttpResponseHeader& addUninitializedValue(
        HttpResponseHeaders& headers,
        std::string_view name,
    ...
    [[nodiscard]] static HttpResponseHeader& addStableView(
        HttpResponseHeaders& headers,
        std::string_view name,
        std::string_view value,
        std::uint32_t knownBit) {
