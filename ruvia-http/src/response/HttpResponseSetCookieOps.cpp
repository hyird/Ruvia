...
[[nodiscard]] bool setCookieValueMatchesStorageKey(std::string_view value, std::string_view wirePrefix, std::string_view cookieName, bool hasPath, std::string_view path, std::string_view domain) noexcept {
    const auto parsed = parseSetCookie(value);
    return parsed.has_value() && isValidHttpHeaderName(parsed->name()) && setCookieWireNameMatches(parsed->name(), wirePrefix, cookieName) && parsed->has(HttpSetCookieAttribute::kPath) == hasPath && (!hasPath || parsed->path() == path) && detail::httpAsciiEqualsIgnoreCase(parsed->domain(), domain);
}

}  // namespace

HttpResponseHeader& HttpResponse::upsertSetCookieHeaderUninitializedValue(std::string_view wirePrefix, std::string_view cookieName, std::string_view path, std::string_view domain, std::size_t valueSize) {
...
HttpResponseHeader* HttpResponse::findSetCookieHeader(std::string_view wirePrefix, std::string_view cookieName, bool hasPath, std::string_view path, std::string_view domain) noexcept {
    for (auto& header : headers_) {
        if (detail::responseHeaderKnownBit(header) == detail::kResponseHeaderSetCookie && setCookieValueMatchesStorageKey(header.value(), wirePrefix, cookieName, hasPath, path, domain)) {
            return &header;
        }
    }
    return nullptr;
...
void HttpResponse::eraseLaterSetCookieHeaders(HttpResponseHeader& retained, std::string_view wirePrefix, std::string_view cookieName, bool hasPath, std::string_view path, std::string_view domain) noexcept {
    ...
    for (auto* read = &retained + 1; read != end; ++read) {
        if (detail::responseHeaderKnownBit(*read) == detail::kResponseHeaderSetCookie && setCookieValueMatchesStorageKey(read->value(), wirePrefix, cookieName, hasPath, path, domain)) {
