...
concept MoveOnlyFunctionTarget = (std::is_void_v<Result> && std::same_as<std::invoke_result_t<std::decay_t<Fn>&, Args...>, void>) || (!std::is_void_v<Result> && std::is_invocable_r_v<Result, std::decay_t<Fn>&, Args...>);
