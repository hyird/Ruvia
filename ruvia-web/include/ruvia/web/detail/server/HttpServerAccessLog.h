...
inline void recordHttpAccess(const AccessLogSink& accessLog, const HttpRequest& request,
    ...
    const AccessLogRecord record = AccessLogRecordAccess::make(
        request, remoteAddress, status, micros < 0 ? 0 : static_cast<std::uint64_t>(micros));
