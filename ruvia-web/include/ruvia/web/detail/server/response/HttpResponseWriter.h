...
[[nodiscard]] inline bool tryPlainTcpSyncWrite(asio::ip::tcp::socket& socket,
    const ConstBufferSequence& buffers, std::size_t totalBytes, std::error_code& ec,
...
Task<Http1BufferedResponseWriteResult> writeResponseWithScratch(Stream& stream,
    ...
    if (const auto fileBody = responseContent.file()) {
        ...
        if (ec) {
            co_return classifyHttp1BufferedResponseWrite(
                responsePlan, responseHeadBytes, ec, bytesTransferred);
        }
        if (!responsePlan.sendBody()) {
            co_return classifyHttp1BufferedResponseWrite(
                responsePlan, responseHeadBytes, {}, bytesTransferred);
        }

        const auto fileError =
            co_await writeHttpResponseFile(stream, memory, fileChunkBuffer, *fileBody);
        co_return classifyHttp1BufferedResponseWrite(
            responsePlan, responseHeadBytes, fileError, responseHeadBytes);
    }
    ...
    if (body.empty()) {
        ...
        co_return classifyHttp1BufferedResponseWrite(
            responsePlan, responseHeadBytes, writeEc, writtenBytes);
    }
    const auto headView = head.view();
    ...
    if (!writeDone) {
        const std::array<asio::const_buffer, 2> remaining =
            writtenBytes < headView.size()
                ? std::array<asio::const_buffer, 2>{asio::buffer(headView.substr(writtenBytes)),
                      asio::buffer(body)}
...
Task<Http1BufferedResponseWriteResult> writeResponse(Stream& stream, WorkerMemory& memory,
    ...
    if (reusableHead != nullptr) {
        return writeResponseWithScratch(
            stream, memory, *reusableHead, fileChunkBuffer, response, responsePlan);
    }
