...
[[nodiscard]] inline ProtocolByteLimit requestBodyByteLimit(RequestBodyMode bodyMode, const std::optional<std::size_t>& maxStreamBodyBytes, std::size_t maxBufferedBodyBytes, std::size_t routeLimit = 0) {
    if (bodyMode != RequestBodyMode::kStream) {
        const auto limit = routeLimit != 0 && routeLimit < maxBufferedBodyBytes ? routeLimit : maxBufferedBodyBytes;
        return ProtocolByteLimit::limited(limit);
    }
    if (!maxStreamBodyBytes.has_value()) {
        return routeLimit != 0 ? ProtocolByteLimit::limited(routeLimit) : ProtocolByteLimit::unlimited();
    }
    const auto limit = routeLimit != 0 && routeLimit < *maxStreamBodyBytes ? routeLimit : *maxStreamBodyBytes;
    return ProtocolByteLimit::limited(limit);
}
