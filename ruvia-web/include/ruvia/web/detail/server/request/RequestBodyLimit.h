...
[[nodiscard]] inline ProtocolByteLimit requestBodyByteLimit(RequestBodyMode bodyMode, const std::optional<std::size_t>& maxStreamBodyBytes, std::size_t maxBufferedBodyBytes) {
    if (bodyMode != RequestBodyMode::kStream) {
        return ProtocolByteLimit::limited(maxBufferedBodyBytes);
    }
    return maxStreamBodyBytes.has_value() ? ProtocolByteLimit::limited(*maxStreamBodyBytes) : ProtocolByteLimit::unlimited();
}
