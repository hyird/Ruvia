...
[[nodiscard]] inline std::uint64_t windowsFileTimeToken(
    LARGE_INTEGER value) noexcept {
    return static_cast<std::uint64_t>(value.QuadPart);
}

[[nodiscard]] inline std::time_t windowsFileTimeSeconds(
    LARGE_INTEGER value) noexcept {
    ...
    return static_cast<std::time_t>(
        (ticks - kWindowsToUnixEpoch100ns) / UINT64_C(10000000));
}
...
[[nodiscard]] inline ResponseFileSnapshot snapshotNativeFileHandle(
    ...
    if (::GetFileInformationByHandleEx(
            handle, FileIdInfo, &id, sizeof(id)) == 0 ||
        ::GetFileInformationByHandleEx(
            handle, FileStandardInfo, &standard, sizeof(standard)) == 0 ||
        ::GetFileInformationByHandleEx(
    ...
    std::memcpy(
        &fileIdHigh,
        id.FileId.Identifier + sizeof(fileIdLow),
        sizeof(fileIdHigh));
    const std::array<std::uint64_t, 4> words{
        static_cast<std::uint64_t>(id.VolumeSerialNumber),
        fileIdLow,
        fileIdHigh,
        windowsFileTimeToken(basic.ChangeTime)};
    ec = {};
    return ResponseFileSnapshot{
        ResponseFileIdentity::checked(words),
        static_cast<std::uint64_t>(standard.EndOfFile.QuadPart),
        windowsFileTimeToken(basic.LastWriteTime),
        windowsFileTimeSeconds(basic.LastWriteTime)};
}
