...
public:
    ...
    [[nodiscard]] static constexpr Http1ClosingRejection error(HttpErrorInfo error) noexcept {
    ...
    [[nodiscard]] constexpr const HttpErrorInfo* error() const& noexcept {
        ...
        if (const auto* rateLimit = std::get_if<Http1ClosingRateLimitRejection>(&value_)) {
    ...
    [[nodiscard]] constexpr const RateLimitRejection* rateLimit() const& noexcept {
        const auto* rejection = std::get_if<Http1ClosingRateLimitRejection>(&value_);
...
private:
    ...
    explicit constexpr Http1ClosingRejection(Http1ClosingRateLimitRejection rejection) noexcept
        : value_(rejection) {}

    std::variant<std::monostate, Http1ClosingError, Http1ClosingRateLimitRejection> value_;
};
