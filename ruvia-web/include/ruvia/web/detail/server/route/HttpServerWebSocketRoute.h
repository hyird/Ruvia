...
Task<std::optional<Http1SessionRequestCompletion>> dispatchHttpWebSocketRoute(Http1RouteDispatch<Stream> d, const ResolvedRoute& resolved, std::string_view pendingFrames) {
    ...
    if (const auto* failure = handshakeValidation.failure()) {
        ...
        const auto connectionPlan = requireHttp1FinalResponseCommit(d.response, d.parsed.connectionPlan.requireClose());
        co_return Http1SessionRequestCompletion::makeBufferedClosing(connectionPlan);
    }
    const auto& webSocketEndpoint = *resolved.route().endpoint().webSocket();
    ...
    auto upgradeAndRun = [&](Context& context) -> Task<void> {
        const auto handshake = makeWebSocketServerHandshake(d.parsed.request, {.supportedSubprotocols = webSocketEndpoint.subprotocols(), .resource = d.memory.resource()});
    ...
    try {
        buffered = co_await d.routes.dispatchWebSocket(d.parsed.request, resolved, d.requestMemory, terminal, d.baseRouteServices);
    } catch (...) {
        exception = std::current_exception();
    }
    if (webSocketConnection.has_value()) {
        co_await finishWebSocketSession(*webSocketConnection, exception, d.options.connectionFailure, d.baseRouteServices.connInfo().remote().address());
