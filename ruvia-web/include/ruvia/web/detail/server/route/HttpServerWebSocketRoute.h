...
Task<std::optional<Http1SessionRequestCompletion>> dispatchHttpWebSocketRoute(
    Http1RouteDispatch<Stream> d, const ResolvedRoute& resolved, std::string_view pendingFrames) {
    const auto handshakeValidation =
        validateWebSocketHandshake(d.parsed.request, d.parsed.bodyPlan);
    if (const auto* failure = handshakeValidation.failure()) {
        d.response = co_await d.routes.handleError(d.parsed.request, d.requestMemory,
            copyHttpProtocolErrorInfo(d.requestMemory.resource(), failure->protocolError()),
            d.baseRouteServices);
    ...
    try {
        buffered = co_await d.routes.dispatchWebSocket(
            d.parsed.request, resolved, d.requestMemory, terminal, d.baseRouteServices);
    } catch (...) {
        exception = std::current_exception();
    }
    if (webSocketConnection.has_value()) {
        co_await finishWebSocketSession(*webSocketConnection, exception,
            d.options.connectionFailure, d.baseRouteServices.connInfo().remote().address());
