...
struct HttpLazyBufferedBodyRouteState final {
    ...
    void emplace(Stream& stream, std::pmr::polymorphic_allocator<char> workerAllocator, std::pmr::memory_resource* requestResource, std::string_view bodyAndPipeline, Http1RequestBodyPlan bodyPlan, ProtocolByteLimit bodyLimit, ConnectionScanner::Entry& scannerEntry) {
        body.emplace(stream, workerAllocator, requestResource, bodyAndPipeline, bodyPlan, bodyLimit, scannerEntry);
    }
...
[[nodiscard]] inline std::string_view httpBodyAndPipeline(const Http1ServerRequestHeadReady& requestHead, const std::pmr::string& readBuffer, std::size_t usedBytes) noexcept {
    ...
    return std::string_view(readBuffer.data() + headerBytes, usedBytes - headerBytes);
}

inline Task<Http1SessionRequestCompletion> completeFailedHttpBodyRoute(ConnectionScanner::Entry& scannerEntry, std::exception_ptr exception, const Http1ServerRequestParseState& parsed, const RouteTable& routes, RequestMemory& requestMemory, ContextServices exceptionServices, HttpResponse& response) {
    response = co_await routes.handleException(parsed.request, requestMemory, exception, exceptionServices);
...
[[nodiscard]] inline Http1SessionRequestCompletion completeSuccessfulHttpBodyRoute(ConnectionScanner::Entry& scannerEntry, HttpResponse& response, Http1ServerConnectionPlan connectionPlan, Http1RequestSequence& requestSequence, Http1RequestBodyConsumption bodyConsumption, std::pmr::string& pipelineStash, TakePipeline takePipeline) {
    ...
    scannerEntry.touch();
    return Http1SessionRequestCompletion::makeBufferedClosing(connectionPlan);
}
