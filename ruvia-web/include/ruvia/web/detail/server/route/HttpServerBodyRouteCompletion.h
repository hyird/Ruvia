...
[[nodiscard]] inline std::string_view httpBodyAndPipeline(
    const Http1ServerRequestHeadReady& requestHead, const std::pmr::string& readBuffer,
...
inline Task<Http1SessionRequestCompletion> completeFailedHttpBodyRoute(
    ...
    response = co_await routes.handleException(
        parsed.request, requestMemory, exception, exceptionServices);
...
[[nodiscard]] inline Http1SessionRequestCompletion completeSuccessfulHttpBodyRoute(
    ...
    connectionPlan =
        finalizeBodyRouteResponse(response, connectionPlan, requestSequence, bodyConsumption);
    if (connectionPlan.disposition() == Http1ClosePolicy::kAllowReuse) {
        ...
        return Http1SessionRequestCompletion::makeBufferedPipelineRestore(
            connectionPlan, std::string_view(pipelineStash));
    }
    scannerEntry.touch();
