    ...
    try {
        bodyReader.emplace(
            ...
            requestBodyByteLimit(
                ...
                d.options.maxStreamBodyBytes,
                d.options.maxBufferedBodyBytes),
            d.scannerEntry);
        d.response = co_await d.routes.dispatch(
            d.parsed.request,
            routeResolution,
            d.requestMemory,
            d.baseRouteServices.withStreamingRequestBody(
                bodyReader->facade()));
    } catch (...) {
