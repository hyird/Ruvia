...
[[nodiscard]] inline detail::ContextServices withRouteHandlers(detail::ContextServices services,
    const detail::RouteTable& routes, detail::HttpErrorHandlerRef errorHandler,
    detail::HttpNotFoundHandlerRef notFoundHandler) noexcept {
    return services.withRoutes(routes)
...
[[nodiscard]] inline Context makeRouteContext(RequestMemory& memory, const HttpRequest& request,
    ...
    if (values.empty()) {
        return detail::ContextAccess::make(
            memory, request, route.path(), routeRateLimitScope, services);
    }

    return detail::ContextAccess::make(memory, request, route.path(), route.paramNames().data(),
        values.data(), values.size(), routeRateLimitScope, services);
}
