...
[[nodiscard]] inline Context makeRouteContext(RequestMemory& memory, const HttpRequest& request, const detail::ResolvedRoute& resolved, detail::ContextServices services) noexcept {
    ...
    return detail::ContextAccess::make(memory, request, route.path(), route.paramNames().data(), values.data(), values.size(), routeRateLimitScope, services);
}
