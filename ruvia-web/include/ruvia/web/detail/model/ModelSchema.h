...
constexpr void visitModelFieldsImpl(ModelT& model, ModelSchema<DescriptorTs...>, VisitorT&& visitor, std::index_sequence<Indices...>) {
...
constexpr void visitModelFields(ModelT& model, ModelSchema<DescriptorTs...> schema, VisitorT&& visitor) {
    visitModelFieldsImpl(model, schema, std::forward<VisitorT>(visitor), std::index_sequence_for<DescriptorTs...>{});
}

template <std::size_t Index, typename DescriptorT, typename... RemainingTs, typename ModelT, typename VisitorT>
[[nodiscard]] bool visitModelFieldByWireNameImpl(ModelT& model, std::uint64_t wireHash, std::string_view wireName, bool& visitResult, VisitorT& visitor) {
    ...
    if constexpr (sizeof...(RemainingTs) > 0) {
        return visitModelFieldByWireNameImpl<Index + 1, RemainingTs...>(model, wireHash, wireName, visitResult, visitor);
    } else {
        return false;
    }
}
...
template <typename ModelT, typename... DescriptorTs, typename VisitorT>
[[nodiscard]] bool visitModelFieldByWireName(ModelT& model, ModelSchema<DescriptorTs...>, std::uint64_t wireHash, std::string_view wireName, bool& visitResult, VisitorT&& visitor) {
    if constexpr (sizeof...(DescriptorTs) == 0) {
        return false;
    } else {
        auto& visitorRef = visitor;
        return visitModelFieldByWireNameImpl<0, DescriptorTs...>(model, wireHash, wireName, visitResult, visitorRef);
    }
}
