...
void validateRule(const ValueT& value, std::string_view path, ValidatorT& validator, const Required&) {
    (void)value;
    (void)path;
    (void)validator;
}
...
template <typename ValueT, typename ValidatorT>
void validateRule(const ValueT& value, std::string_view path, ValidatorT& validator, const Min& rule) {
...
template <typename ValueT, typename ValidatorT>
void validateRule(const ValueT& value, std::string_view path, ValidatorT& validator, const Max& rule) {
    if constexpr (modelHasSizeRule<ValueT>()) {
        if (modelSize(value) > static_cast<std::size_t>(rule.value)) {
...
void validateRule(const ValueT& value, std::string_view path, ValidatorT& validator, const OneOf<Values...>& rule) {
...
template <typename ValueT, typename ValidatorT>
void validateRule(const ValueT& value, std::string_view path, ValidatorT& validator, const Email& rule) {
...
void validateRule(const ValueT& value, std::string_view path, ValidatorT& validator, const PatternRule<Pattern>& rule) {
    const auto actual = modelString(value);
...
template <typename ValueT, typename ValidatorT, FixedString Pattern>
void validateRule(const ValueT& value, std::string_view path, ValidatorT& validator, const RegexRule<Pattern>& rule) {
    const auto actual = modelString(value);
...
void validateRule(const ValueT& value, std::string_view path, ValidatorT& validator, const Custom<PredicateT>& rule) {
...
void validateRule(const ValueT& value, std::string_view path, ValidatorT& validator, const Nested<ValidationSchemaT>&) {
    static_assert(
        requires(const ValidationSchemaT& schema, const ValueT& nested, std::string_view nestedPath, ValidatorT& nestedValidator) { schema.validateNested(nested, nestedPath, nestedValidator); },
...
void validateRule(const ValueT& value, std::string_view path, ValidatorT& validator, const RuleT&)
    requires(isDefaultRule<RuleT>() || std::is_same_v<std::remove_cvref_t<RuleT>, OmitEmpty> || std::is_same_v<std::remove_cvref_t<RuleT>, EmitNull>)
{
    (void)value;
    (void)path;
    (void)validator;
