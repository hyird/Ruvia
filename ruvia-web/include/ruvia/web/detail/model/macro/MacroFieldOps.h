...
#define RUVIA_MODEL_PARSE_FORM_FIELD(T, field)                                                                                                                                                             \
    if constexpr (::ruvia::detail::isFormField<typename decltype(ruviaField_##field##_)::value_type>) {                                                                                                    \
        if (key == ruviaField_##field##_.wireName()) {                                                                                                                                                     \
            if (ruviaField_##field##_.state() != ::ruvia::detail::ModelFieldState::kMissing) {                                                                                                             \
                ruviaField_##field##_.markDuplicate();                                                                                                                                                     \
                return;                                                                                                                                                                                    \
            }                                                                                                                                                                                              \
            const auto ruviaEncoding = ruviaInput.kind() == ::ruvia::detail::ModelInputKind::kFormFields ? ::ruvia::detail::FormValueEncoding::kDecoded : ::ruvia::detail::FormValueEncoding::kUrlEncoded; \
            using RuviaValueT = typename decltype(ruviaField_##field##_)::value_type;                                                                                                                      \
            auto ruviaValue = ::ruvia::detail::parseFormValue<RuviaValueT>(::ruvia::detail::ResolvedPmrResourceTag{}, value, ruviaEncoding, ruviaResource);                                                \
            if (ruviaValue) {                                                                                                                                                                              \
                ruviaField_##field##_.emplaceParsed(::std::move(*ruviaValue));                                                                                                                             \
            } else {                                                                                                                                                                                       \
                ruviaField_##field##_.markInvalidType();                                                                                                                                                   \
...
#define RUVIA_MODEL_APPLY_DEFAULT_FIELD(T, field) ruviaField_##field##_.applyDefault(ruviaResource);
...
#define RUVIA_MODEL_APPEND_JSON_FIELD(T, field)                                                            \
    {                                                                                                      \
        ...
        if (ruviaValue && !(ruviaSlot.omitEmpty() && ::ruvia::detail::model::isEmptyValue(*ruviaValue))) { \
    ...
    {                                                                                        \
        ...
        if constexpr (::std::remove_cvref_t<decltype(ruviaSlot)>::required) {                \
            if (ruviaSlot.state() == ::ruvia::detail::ModelFieldState::kMissing) {           \
                ...
                ::ruvia::detail::model::appendPath(ruviaPath, prefix, ruviaSlot.wireName()); \
