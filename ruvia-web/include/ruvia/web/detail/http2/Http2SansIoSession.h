...
[[nodiscard]] Task<void> runHttp2SansIoSession(asio::ip::tcp::socket& stream,
    const RouteTable& routes, WorkerMemory& worker, Http2SansIoSessionContext session,
    std::string_view initialBytes = {});

[[nodiscard]] Task<void> runHttp2SansIoSession(asio::ssl::stream<asio::ip::tcp::socket&>& stream,
    const RouteTable& routes, WorkerMemory& worker, Http2SansIoSessionContext session,
    std::string_view initialBytes = {});
