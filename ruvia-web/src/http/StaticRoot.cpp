...
[[nodiscard]] detail::StaticRootState* makeStaticRootState(detail::StaticRootConfigStorage config) {
    ...
    return detail::constructPmrObject<detail::StaticRootState>(resource, resource, std::move(config));
}
...
[[nodiscard]] bool staticContentTypeEligibleForPrecompression(std::string_view contentType) noexcept {
    ...
    return mediaTypeStartsWith(mediaType, "text/") || detail::httpAsciiEqualsIgnoreCase(mediaType, "application/json") || detail::httpAsciiEqualsIgnoreCase(mediaType, "application/javascript") || detail::httpAsciiEqualsIgnoreCase(mediaType, "application/x-javascript") || detail::httpAsciiEqualsIgnoreCase(mediaType, "application/wasm") || detail::httpAsciiEqualsIgnoreCase(mediaType, "application/xml") || detail::httpAsciiEqualsIgnoreCase(mediaType, "application/xhtml+xml") || detail::httpAsciiEqualsIgnoreCase(mediaType, "image/svg+xml") || mediaTypeEndsWith(mediaType, "+json") || mediaTypeEndsWith(mediaType, "+xml");
}
...
[[nodiscard]] bool sameStaticRootFileSnapshot(const detail::StaticRootEntry& entry, const detail::ResponseFileSnapshot& snapshot) noexcept {
    return entry.size == snapshot.size && entry.identity == snapshot.identity && entry.modifiedToken == snapshot.modifiedToken && entry.modifiedSeconds == snapshot.modifiedSeconds;
}

[[nodiscard]] bool sameStaticRootEntryMetadata(const detail::StaticRootEntry& left, const detail::StaticRootEntry& right) noexcept {
    return left.relativePath == right.relativePath && left.filePath == right.filePath && left.contentType == right.contentType && left.size == right.size && left.identity == right.identity && left.modifiedToken == right.modifiedToken && left.modifiedSeconds == right.modifiedSeconds && left.etag == right.etag && left.lastModified == right.lastModified && left.directlyServable == right.directlyServable;
}
...
detail::StaticRootConfigStorage detail::StaticRootAccess::copyConfig(const StaticRoot& root, std::pmr::memory_resource* resource) {
...
std::optional<detail::StaticRootEntryView> detail::StaticRootAccess::findVariant(const StaticRoot& root, std::string_view relativePath) noexcept {
    ...
    return detail::StaticRootEntryView(entry->filePath.c_str(), entry->contentType, state.config.cacheControl, entry->etag, entry->lastModified, entry->size, entry->identity, entry->modifiedToken, entry->modifiedSeconds, state.config.rangeRequests, state.config.responseValidators, entry->directlyServable, &entry->memoryVariants);
}
...
void detail::StaticRootAccess::installPrecompressedVariants(StaticRoot& root, const StaticRoot* previous, const StaticRootPrecompressionOptions& options) {
    ...
    const bool emitResponseValidators = state.config.responseValidators == StaticResponseValidatorPolicy::kEmit;

    for (auto& entry : state.entries) {
        if (!entry.directlyServable || entry.size < options.minBytes || entry.size > options.maxBytes || !staticContentTypeEligibleForPrecompression(entry.contentType)) {
            continue;
        }
...
bool detail::StaticRootAccess::sameSnapshot(const StaticRoot& left, const StaticRoot& right) noexcept {
    ...
    if (lhs.root != rhs.root || lhsConfig.indexFile != rhsConfig.indexFile || lhsConfig.cacheControl != rhsConfig.cacheControl || lhsConfig.defaultContentType != rhsConfig.defaultContentType || lhsConfig.fileTypeKind != rhsConfig.fileTypeKind || lhsConfig.rangeRequests != rhsConfig.rangeRequests || lhsConfig.responseValidators != rhsConfig.responseValidators || lhsConfig.dotfiles != rhsConfig.dotfiles || lhsConfig.fileTypeExtensions != rhsConfig.fileTypeExtensions || lhs.directories != rhs.directories || lhsConfig.mimeTypes.size() != rhsConfig.mimeTypes.size() || lhs.entries.size() != rhs.entries.size()) {
        return false;
    }
    for (std::size_t i = 0; i < lhsConfig.mimeTypes.size(); ++i) {
        if (lhsConfig.mimeTypes[i].extension != rhsConfig.mimeTypes[i].extension || lhsConfig.mimeTypes[i].contentType != rhsConfig.mimeTypes[i].contentType) {
            return false;
        }
    }
...
StaticRoot::PreparedConstruction StaticRoot::prepareConstruction(const std::filesystem::path& root, StaticRootOptions options) {
...
StaticRoot::PreparedConstruction StaticRoot::prepareConstruction(const std::filesystem::path& root, const detail::StaticRootConfigStorage& config) {
    return PreparedConstruction{
        .canonicalRoot = canonicalStaticRootPath(root),
...
StaticRoot::PreparedConstruction StaticRoot::prepareConstruction(const std::filesystem::path& root, detail::StaticRootConfigStorage&& config) {
...
std::unique_ptr<StaticRoot, detail::PmrObjectDeleter<StaticRoot>> detail::StaticRootAccess::construct(std::pmr::memory_resource* objectResource, StaticRoot::PreparedConstruction prepared) {
    ...
    try {
        ...
        return std::unique_ptr<StaticRoot, PmrObjectDeleter<StaticRoot>>(result, PmrObjectDeleter<StaticRoot>{resource});
    } catch (...) {
...
std::unique_ptr<StaticRoot, detail::PmrObjectDeleter<StaticRoot>> detail::StaticRootAccess::make(std::pmr::memory_resource* objectResource, const std::filesystem::path& root, const StaticRootConfigStorage& config) {
    return construct(objectResource, StaticRoot::prepareConstruction(root, config));
}

std::unique_ptr<StaticRoot, detail::PmrObjectDeleter<StaticRoot>> detail::StaticRootAccess::make(std::pmr::memory_resource* objectResource, const std::filesystem::path& root, StaticRootConfigStorage&& config) {
    return construct(objectResource, StaticRoot::prepareConstruction(root, std::move(config)));
}

std::unique_ptr<StaticRoot, detail::PmrObjectDeleter<StaticRoot>> detail::StaticRootAccess::clone(std::pmr::memory_resource* objectResource, const StaticRoot& source) {
...
StaticRoot::StaticRoot(PreparedConstruction prepared)
    ...
    for (; iter != end; iter.increment(ec)) {
        ...
        if (!directlyServable && isPrecompressedSidecarExtension(extension)) {
            usableAsSidecar = detail::fileTypeAllowed(detail::lowerStaticFileExtension(filePath.stem(), upstream), config);
        }
        ...
        const auto emitResponseValidators = config.responseValidators == StaticResponseValidatorPolicy::kEmit;
