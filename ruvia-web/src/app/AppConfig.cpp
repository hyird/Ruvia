...
void applyServerConfig(AppState& state, const ServerConfig& config) {
    ...
    if (config.processSignalHandlers != ProcessSignalHandlerPolicy::kExternalOwner && config.processSignalHandlers != ProcessSignalHandlerPolicy::kInstall) {
    ...
    if (config.maxConnectionsPerWorker) {
        ensurePositiveSize(*config.maxConnectionsPerWorker, "configured connection limit must be greater than zero");
    }
    if (config.maxRequestsPerConnection) {
        ensurePositiveSize(*config.maxRequestsPerConnection, "configured requests-per-connection limit must be greater than zero");
    }
    ensurePositiveSize(config.maxBufferedBodyBytes, "buffered body limit must be greater than zero");
    if (config.maxStreamBodyBytes) {
        ensurePositiveSize(*config.maxStreamBodyBytes, "configured stream body limit must be greater than zero");
    }
    ...
    return detail::mutateStoppedApp(*this, *state_, "cannot change listeners while app is running", [&config](detail::AppState& state) {
        ...
        if (config.http.has_value()) {
            ...
            if (config.autoHttpsRedirect) {
                replacement.emplace_back(endpoint, detail::HttpServerListenerDefinition::RedirectHttpToHttps{*config.https});
            } else {
                replacement.emplace_back(endpoint);
            }
        }
        if (config.https.has_value()) {
            replacement.emplace_back(asio::ip::tcp::endpoint(address, *config.https), detail::normalizeTlsOptions(config.tls, resource));
        }
...
App& App::server(ServerConfig config) {
    return detail::mutateStoppedApp(*this, *state_, "cannot change server config while app is running", [config](detail::AppState& state) { detail::applyServerConfig(state, config); });
}
...
App& App::trustedProxies(std::nullptr_t) {
    return detail::mutateStoppedApp(*this, *state_, "cannot change trusted proxies while app is running", [](detail::AppState& state) { state.options.trustedProxies = detail::TrustedProxySet(detail::appResource()); });
}
