...
DotenvResult detail::loadEnvFromFile(Env& env, const std::filesystem::path& path, DotenvOptions options) {
    if (options.existingVariables != DotenvExistingVariablePolicy::kPreserve && options.existingVariables != DotenvExistingVariablePolicy::kOverride) {
        throw std::invalid_argument("dotenv existing variable policy is invalid");
    }
    if (options.missingFile != DotenvMissingFilePolicy::kIgnore && options.missingFile != DotenvMissingFilePolicy::kRequire) {
