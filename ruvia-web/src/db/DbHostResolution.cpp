...
DbResolvedAddresses collectDbResolvedAddresses(const asio::ip::tcp::resolver::results_type& results, DbDriver driver, std::pmr::memory_resource* resource) {
    ...
    for (const auto& result : results) {
        ...
        if (error) {
            throw DbError(DbError::Code::kResolveFailed, driver, std::system_error(error, "formatting resolved database address failed").what(), error.value());
        }
    ...
    if (addresses.empty()) {
        throw DbError(DbError::Code::kResolveFailed, driver, "database host resolved to no addresses");
    }
    return addresses;
}
