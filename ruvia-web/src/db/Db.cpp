    ...
    try {
        ...
        if (row != nullptr) {
            ...
            for (std::size_t i = 0; i < fieldCount; ++i) {
                outputColumnNames.emplace_back(fields[i].name, static_cast<std::size_t>(fields[i].name_length));
                if (row[i] == nullptr) {
...
Task<DbRows> detail::MariaDbPool::queryOnSlot(ConnectionSlot& slot, std::string_view sql, std::span<const DbValue> params, std::pmr::memory_resource* resource, const OperationTimeout& operationTimeout) {
    ...
    const auto* fields = mysql_fetch_fields(rawResult);
    for (std::size_t i = 0; i < fieldCount; ++i) {
        resultColumnNames.emplace_back(fields[i].name, static_cast<std::size_t>(fields[i].name_length));
    }
    while (auto* row = mysql_fetch_row(rawResult)) {
        ...
        resultRows.push_back(DbResultAccess::borrowedRow(resultFields.data() + rowStart, fieldCount, resultColumnNames.data(), resultColumnNames.size(), resource));
    }
