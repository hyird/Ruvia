...
ScopedOperation<DbRows> DbTransaction::query(
    ...
    return detail::makeScopedOperation(
        operationScope_, queryPrepared(std::move(statement.sql), std::move(statement.params)));
}
...
ScopedOperation<DbExecResult> DbTransaction::execute(
    ...
    auto statement = prepareDbStatement(sql, params, state_.activePayload().resource);
    return detail::makeScopedOperation(
        operationScope_, executePrepared(std::move(statement.sql), std::move(statement.params)));
}
