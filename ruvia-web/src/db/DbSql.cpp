...
DbError mysqlError(const st_mysql& connection, std::string_view operation, DbError::Code errorCode) {
    ...
    return DbError(errorCode, DbDriver::kMariaDb, std::string(error), code == 0 ? std::nullopt : std::optional<std::int64_t>(code), state == nullptr ? std::string{} : std::string(state));
}
