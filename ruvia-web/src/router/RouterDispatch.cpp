...
[[nodiscard]] std::optional<HttpResponse> selectDocumentRootFallback(
    ...
    RequestMemory& memory, detail::StaticFileSelectionMode staticFileMode) {
...
Task<HttpResponse> detail::RouteTable::dispatchRequest(const HttpRequest& request,
    ...
    try {
        ...
        if (resolved == nullptr) {
            if (auto documentResponse =
                    selectDocumentRootFallback(documentRoot, request, memory, staticFileMode)) {
