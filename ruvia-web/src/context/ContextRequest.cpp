...
void Context::ensureRequestQuery() const {
    ...
    for (std::size_t pairIndex = 0; pairIndex < decodedPairCount; ++pairIndex) {
        detail::RequestNameValueListAccess::pushBack(query, detail::RequestNameValueViewAccess::make(detail::storedStringView(storage[pairIndex * 2]), detail::storedStringView(storage[pairIndex * 2 + 1])));
    }
