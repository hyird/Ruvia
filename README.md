...
| `c.req().header(name)` | Read one request header as `std::optional<std::string_view>` without materializing a header table. Framework-level code that needs the raw parser header span can use `c.req().raw().headers()`. |
| `c.req().query(name)` / `c.req().queries(name)` | Read one decoded query value as `std::optional<std::string_view>`, or all decoded values for one query key as a string-view span. |
| `c.req().cookie(name)` | Read one cookie value as `std::optional<std::string_view>`. Framework validation code uses an internal cookie field accessor; application code should keep cookie reads explicit by name. |
...
| `c.req().param(name)` | Read one decoded dynamic route parameter as `std::optional<std::string_view>`. Framework validation code uses an internal param field accessor; application code should keep param reads explicit by name. |
...
| `co_await c.req().blob()` | Lazily read the full buffered request body as a zero-copy Ruvia blob facade with `bytes()` / `text()` access plus its `Content-Type`; it is not a Web `Blob` object. |
...
| `co_await c.req().parseBody()` | Parse URL-encoded or multipart form data into a Ruvia form facade, or return an empty form object for other content types: use `body.get("title").value()` for the selected field, `body.get("tag").fields()` for repeated fields, and `body.groups()` for grouped summaries. With `{.dot = true}`, use `body.at("obj.key").value()`, `body.object("obj").at("key").value()`, `body.object("obj").groups()`, or chained `body.object("obj").object("child")` for dot notation access; nested `__proto__` paths are ignored. |
| `co_await c.req().formData()` | FormData-style form parsing with `get(name)`, `count(name)`, `fields()`, and `groups()` access that keeps file/blob metadata and duplicate-preserving grouped summaries. The return value is a Ruvia form facade, not a Web `FormData` object. |
...
| `c.res()` / `c.res(response)` | Access the current `ruvia::HttpResponse` slot or replace it to short-circuit middleware. `c.res(response)` returns `void`, matching Hono's assignment-style response setter. Reading `c.res()` does not by itself finalize the context; handler returns, downstream responses, and `c.res(response)` do. After `co_await next()`, mutate response headers with `c.res().header(name, value)`, `c.res().header(name, value, {.append = true})`, or `c.res().header(name, std::nullopt)`; `c.res().headers()` is a read-only header view. When replacing the response, existing `c.res().headers()` override the assigned response headers except `Content-Type`. `c.res(response)` does not consume headers prepared only with `c.header()`; use `c.body()` / `c.newResponse()` / `c.text()` / `c.json()` / `c.html()` when prepared headers should be applied while constructing a new response. |
...
| `c.html(...)` / `c.renderer(...)` / `c.layout(...)` / `co_await c.render(body, {.title = ...})` / `co_await c.render(body, {.head = ...})` | Return HTML directly or through middleware-installed renderer/layout hooks with explicit title or head metadata. `c.renderer(...)` installs the renderer; `c.layout(...)` installs or reads the current layout. |
...
Ordinary request bodies are lazy: Ruvia dispatches middleware and handlers after headers, and reads the body only when code explicitly awaits `c.req().text()`, `c.req().arrayBuffer()`, `c.req().blob()`, `c.req().json()`, `c.req().json<T>()`, `c.req().form<T>()`, `c.req().multipart()`, `c.req().parseBody()`, `c.req().formData()`, or `c.req().discardBody()`. If a request declares a body and the route returns without consuming or discarding it, Ruvia closes the connection instead of draining bytes just to preserve keep-alive. `Expect: 100-continue` is answered only when body reading actually starts, so middleware can reject large uploads without encouraging the client to send the body.
