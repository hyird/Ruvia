...
| `co_await c.req().parseBody()` | Parse URL-encoded or multipart form data into an object with `get()`, `getAll()`, `count()`, grouped `entries()`, and raw `fields()`; `foo[]` keeps the `[]` key and is marked as an array entry. |
