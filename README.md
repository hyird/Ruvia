...
| `co_await c.req().parseBody()` | Parse URL-encoded or multipart form data into a Hono-style object facade: use `body["title"].value()` or `body.get("title").toStringView()` for the last field, `body.getAll("tag").values()` for repeated fields, `body["tag[]"].values()` for array fields, and `isArray()` to inspect `[]` fields or multi-value fields retained by `{.all = true}`. `field()` / `fields()` remain available as lower-level field metadata helpers. With `{.dot = true}`, use `body.at("obj.key").value()`, `body.object("obj").value("key")`, `body.object("obj").values("key")`, `body.object("obj").getAll("key").values()`, `body.object("obj").entries()` / `.groups()` / `.keys()`, or chained `body.object("obj").object("child")` for dot notation access. |
