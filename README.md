...
| `co_await c.req().parseBody()` | Parse URL-encoded or multipart form data into a Hono-style object facade: use `body["title"].value()` or `body.get("title").toStringView()` for the last field, `body.getAll("tag").values()` for repeated fields, `body["tag[]"].values()` for array fields, and `body["tag[]"].isArray()` to inspect `[]` fields. `field()` / `fields()` remain available as lower-level field metadata helpers. With `{.dot = true}`, use `body.at("obj.key").value()`, `body.object("obj").value("key")`, `body.object("obj").values("key")`, `body.object("obj").getAll("key").values()`, or `body.object("obj").entries()` / `.groups()` / `.keys()` for dot notation access. |
