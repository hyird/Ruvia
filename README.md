...
| `c.query(name)` / `c.cookie(name)` / `c.param(name)` | Shorthand request readers for common handler input. Use `c.req().header(name)` for request headers; `c.header(name, value)` and `c.setHeader(name, value)` set response headers. |
