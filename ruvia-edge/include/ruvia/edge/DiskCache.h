...
// A persistent, bounded second cache tier backed by a directory on disk. It
// stores the same CachedResponse payloads as the in-memory EdgeCache, one file
// per entry, and survives process restarts: the constructor scans the directory
// and rebuilds its index from the files already there.
//
...
public:
    ...
    // Read the entry stored under `key`, or null if absent or unreadable. On a
    ...
    [[nodiscard]] std::shared_ptr<const CachedResponse> lookup(std::string_view key);
    ...
    // variants of one URL). Returns the number removed.
...
private:
    ...
    void removeLocked(std::unordered_map<std::string, Entry>::iterator it) noexcept;
