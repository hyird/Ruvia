...
[[nodiscard]] std::string encodeResponse(HttpProtocolVersion protocolVersion, std::uint16_t status, const Headers& headers, std::string_view body, std::string_view xCache, std::optional<std::uint64_t> ageOverride, bool omitBody = false, bool keepAlive = false);

// A minimal status-only response for edge-generated errors.
[[nodiscard]] std::string encodeStatusResponse(std::uint16_t status, HttpProtocolVersion protocolVersion = HttpProtocolVersion::kHttp11);
...
[[nodiscard]] std::string encodeStreamingHead(HttpProtocolVersion protocolVersion, std::uint16_t status, const Headers& headers, std::string_view xCache, ClientFraming framing, std::size_t contentLength, bool keepAlive);
