...
std::uint64_t diskRecordHash(std::string_view data) noexcept {
...
std::optional<std::string> encodeDiskRecord(std::string_view key, const CachedResponse& entry) {
    ...
    appendU64(out, diskRecordHash(out));
    return out;
}
...
bool decodeDiskRecord(std::string_view data, std::string& key, CachedResponse* entry) {
    if (data.size() < 8) {
        return false;
    }
