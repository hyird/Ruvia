...
[[nodiscard]] bool readDiskCacheFile(
...
[[nodiscard]] bool isCommittedDiskCacheFileName(std::string_view name) noexcept {
    return name.size() == 20 &&
        std::all_of(name.begin(), name.begin() + 16, isLowerHex) &&
        name.substr(16) == ".rvc";
}

[[nodiscard]] bool isOwnedTransientDiskCacheFileName(std::string_view name) noexcept {
    return name.size() > 24 &&
        std::all_of(name.begin(), name.begin() + 16, isLowerHex) &&
