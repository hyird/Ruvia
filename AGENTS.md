...
- TCP ingress 将新接受的连接只交接一次到目标业务 worker；交接后连接及其协议状态不得再跨线程迁移。
...
- `App::server(ServerConfig)` 原子配置进程内 Web runtime；`workerCount=N` 表示 N 个业务 worker，另外有一个 TCP ingress 线程和一个 UDP ingress 线程。额外线程数不包括 `BlockingPool` 和 signal 线程。每个业务 worker 只创建一份 worker-local DB、Redis、outbound HTTP client 和 user state；listener 数量不得乘增 worker 或数据资源。
...
- listener 通过一个 `App::listen(ListenConfig)` 原子配置统一的 bind address、可选 HTTP/HTTPS TCP 端口、TLS 和自动跳转；未填写的端口表示不开启，自动跳转要求同一配置同时提供 HTTP 与 HTTPS。只有 TCP ingress 绑定全部配置的 HTTP/HTTPS TCP 端口；它接受连接后只向业务 worker 交接一次。交接成功后连接由目标业务 worker 持有并处理，不再迁移。
- `App::run()` 为每个业务 worker 创建一个线程和完整 runtime，并创建 TCP ingress 与 UDP ingress 各一个线程。UDP ingress 目前只运行 `io_context`，没有 UDP port/socket/protocol；不支持 HTTP/3，也没有 QUIC。
- Web 启动必须先完成所有业务 worker 的 prepare 和 TCP ingress listener prepare，再启动并等待所有业务 worker 与 ingress runtimes ready，然后先使业务 worker、再使两个 ingress 进入 serving；准备或启动失败不得留下部分系统对外服务。`onStart` 只在所有 worker 和 ingress 都进入 serving 后执行。
- `App::run()` 的调用线程是 App 生命周期的唯一执行线程，负责 `onStart`、`onStop`、join 和失败重抛。`App::stop()`、信号线程以及 worker/ingress failure 只能提交单调 stop request 并关闭稳定 endpoint，不得在调用方线程执行用户 hook；App 单例的配置、运行和 runtime 借用必须继续受同一生命周期门禁保护。
...
- shutdown 时 TCP ingress 在自己的 `io_context` 上关闭 TCP acceptor，业务 worker 在各自的 `io_context` 上关闭已交接连接和 worker 资源，UDP ingress 在自己的 `io_context` 上停止；每个 owner 线程本地关闭其资源。不等待请求优雅排空。不依赖 `SO_REUSEPORT` 分摊 listener，因为 TCP listener 仅由 ingress 绑定。
