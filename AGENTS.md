...
- 模型绑定只走 schema 路线。`JsonValue`/`JsonObject` 借用完整 JSON 文本，提供 kind 判断、`view()` 和按运行时字段名的 `get<T>()`；不得作为 `c.json()`/`toJson()` 输出面。原始 body 和扁平 multipart 协议访问保留，不提供 form 点路径/分组语言。
